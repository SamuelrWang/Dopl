...
test("session-engine: startSession merges the spec ids into the context for EVERY spawn shape", () => {
  ...
  assert.match(src, /framing\.buildFencedTurn\(\{ side: spec\.side, message: spec\.firstMessage, context: \{ \.\.\.context, profile: spec\.profile, mcpDiscovery: runtimeRegistry\.capability\.mcpDiscoveryVerb\([^}]*\) \}, nonce \}\)/);
