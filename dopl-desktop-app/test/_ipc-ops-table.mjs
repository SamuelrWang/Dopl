...
// 2026-09-07: `OPERATOR_TURN_CAP` / `DEFAULT_TURN_CAP` were re-exported here for the
// `settings:getTurnCap` refusal shape, read off `main/session-state.js` rather than retyped.
// Both constants, both ops and the test that pinned them to one statement are deleted with the
// caps — see `main/session-state.js`'s header.
...
export const OPS = [
  ...
  // ⚠ THE TWO AUTO-SEND ROWS LEFT ON 2026-09-06 (item 8), for the reason the arm's rows left
  // above: the handlers are DELETED from `channel-dir-ipc.js` outright, and a row for an
  // unregistered op fails `every privileged op in the file is registered` on the first case.
  // The axis they covered is the launch posture's `messages`, whose rows are already here.
  ...
  // 🔒 `settings:getTurnCap` / `settings:setTurnCap` STOOD HERE AND ARE DELETED (2026-09-07,
  // Samuel's ruling). Both routes are UNREGISTERED, not stubbed, so there is no row to census.
