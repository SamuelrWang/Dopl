...
const deferredCatalog = (d) => (d && d.prose && d.prose.deferredCatalog) || null;
// CXP-3A (2026-09-22): how a turn must tell the agent to reach DOPL'S deferred tools —
// `{ verb, catalog }` (either may be null), or `null` when nothing is to be said.
...
const mcpDiscovery = (d) => {
  if (d && d.mcp && d.mcp.eagerLoadFlag) return null;
  const verb = toolSearchVerb(d);
  const catalog = deferredCatalog(d);
...
module.exports = {
  ...
  toolSearchVerb, deferredCatalog, mcpDiscovery, entryFile, hasDeepLink, hasInteractiveSignIn, showsLocationPicker,
};
