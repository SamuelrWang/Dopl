...
--
-- **APPLY ORDER: SECOND of the wave's three unapplied migrations** —
-- `knowledge_client_write_id` (`20261014120000`), then this one, then
-- `drop_knowledge_pinned` (`20261016120000`). It depends on nothing structural
-- — it writes three `text` columns and creates and drops one helper function —
-- and nothing depends on it. ⚠ **THE DROP GOES LAST BY GATE, NOT BY TASTE**:
-- `src/features/knowledge/pinning-stays-removed.test.ts` asserts the drop is the
-- LAST file in this directory to mention that column, so nothing after it may
-- even name it.
--
-- ⚠ **THIS FILE WAS RE-STAMPED FROM `20261014120000` AT THE 2026-09-19 MERGE.**
-- THREE branches of this wave each claimed that timestamp independently. The
-- NAME did not change, which is what matters: this directory is applied by NAME
-- (F-304, INVARIANTS §12), and a re-stamp is only ever a re-ordering.
