...
export function ChatMessageBubble({ message, onAddToCanvas }: ChatMessageProps) {
  ...
  if (message.role === "user" && message.type === "text") {
    return (
      <div className="ml-auto max-w-[85%] animate-fade-in">
        <div className="text-sm text-[var(--text-primary)] bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-lg py-2 px-3">
  ...
  if (message.role === "ai" && message.type === "text") {
    return (
      <div className="mr-auto max-w-[95%] animate-fade-in">
  ...
  if (message.role === "ai" && message.type === "streaming") {
    return (
      <div className="mr-auto max-w-[95%]">
        <MarkdownRenderer content={message.content} />
        <span className="inline-block w-1.5 h-4 bg-[var(--accent-primary)] animate-pulse ml-0.5" />
      </div>
    );
  }
  ...
  if (message.role === "ai" && message.type === "tool_activity") {
    return (
      <div className="mr-auto flex items-center gap-2 text-xs text-[var(--text-muted)] py-1 animate-fade-in">
  ...
  if (message.role === "ai" && message.type === "entry_cards") {
    return (
      <div className="mr-auto w-full space-y-2 animate-fade-in">
