...
export async function getAccountStatus(
  ...
  return t.request<AccountStatus>(
    `/api/channels/account/status${query ? `?${query}` : ""}`,
    { toolName: "channel_status" }
  );
}
...
export async function readAccountMessages(
  ...
  return t.request<AccountMessagesPage>(
    `/api/channels/account/messages?${params.toString()}`,
    { toolName: "channel_read_account" }
  );
}
