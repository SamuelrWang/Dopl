...
export interface ChannelMessage {
  ...
  /**
   * The artifact this message is folded into (#1220 §2, 2026-09-06).
   * ⚠ OPTIONAL *AND* NULLABLE, like the delivery keystone above: `undefined` is
   * an older server that has never heard of artifacts, `null` is a message in
   * none. Neither is a reason to hide the row.
   */
  artifactId?: string | null;
}

/**
 * ⚠ **THE ARTIFACT SHAPES MOVED TO `channel-artifact-types.ts` (2026-09-14)** and are
 * re-exported here unchanged, so **NO CONSUMER IMPORT CHANGED** — the same discipline
 * `escalation-types.ts`, `launch-types.ts` and `info-card-types.ts` already follow. The
 * seam is a REASON TO CHANGE (one feature's shapes), not a line count; what forced it
 * was this file passing the 500-line cap (§1, the `size-check` CI job, F-689).
 */
export type {
  ChannelArtifact,
...
} from "./channel-artifact-types.js";
...
export interface ChannelThreadCreateInput {
  ...
  mode?: ThreadMode;
  body: string;
  toUserId?: string; // omitted: the opener's thread alone, and its opener wakes nobody
  intent?: MessageIntent; // "chat" opens it as a record; refused with toUserId
  /**
