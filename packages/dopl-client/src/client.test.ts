...
describe("DoplClient retries", () => {
  ...
  it("does NOT retry on POST (non-idempotent)", async () => {
    ...
    const client = new DoplClient(BASE, "k");
    await expect(client.createOntology({ name: "x" })).rejects.toBeInstanceOf(DoplApiError);
...
/**
 * `updateAgentIdentity`'s TRI-STATE (F-747, 2026-09-18).
 *
 ...
 * ⚠ AND IT NEEDS NO EXISTENCE PROBE where the KB one does — an identity update
 * always overwrites, so there is no create arm to distinguish.
 */
describe("updateAgentIdentity — optimistic concurrency", () => {
  let mock: ReturnType<typeof installFetchMock>;
  ...
  beforeEach(() => {
    mock = installFetchMock([
      () => jsonResponse(200, { identity: { id: TPL, updatedAt: VERSION } }),
    ]);
  });
  afterEach(() => mock.restore());
  ...
  it("a version rides as `X-Updated-At`", async () => {
    await client().updateAgentIdentity(TPL, { name: "Renamed" }, VERSION);
  ...
  it("`null` is FORCE — the request goes, with no precondition on it", async () => {
    await client().updateAgentIdentity(TPL, { name: "Renamed" }, null);
    const headers = new Headers(mock.calls[0].init.headers as HeadersInit);
  ...
  it("OMITTING it is refused BEFORE the wire, naming what to pass", async () => {
    await expect(
      client().updateAgentIdentity(TPL, { name: "Renamed" })
  ...
  it("…and the refusal carries the code the MCP layer maps on", async () => {
    const failed = client().updateAgentIdentity(TPL, { name: "Renamed" });
