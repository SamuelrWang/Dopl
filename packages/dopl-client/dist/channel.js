 ...
 */
async function readTranscript(t, channelId, opts = {}) {
    const params = new URLSearchParams();
    if (opts.since !== undefined)
        params.set("since", String(opts.since));
    if (opts.limit !== undefined)
    ...
    if (opts.thread !== undefined)
        params.set("thread", opts.thread);
    const qs = params.toString();
    const data = await t.request(`/api/channels/${enc(channelId)}/messages${qs ? `?${qs}` : ""}`, {
 ...
 */
async function writeArtifact(t, channelId, input) {
    return t.request(`/api/channels/${enc(channelId)}/artifacts`, { method: "POST", body: input, toolName: "channel_artifact" });
}
...
async function readArtifact(t, channelId, artifactId) {
    return t.request(`/api/channels/${enc(channelId)}/artifacts?artifact=${enc(artifactId)}`, { toolName: "channel_artifact_read" });
}
