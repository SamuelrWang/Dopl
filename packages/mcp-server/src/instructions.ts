...
function whichTool(loader: string): string {
  return bySet({
    ...
    granular: `WHICH TOOL (long rules are PULLED): dopl_get_map first (a routing view, not a count) · dopl_search when you don't know where it lives · dopl_browse_knowledge / dopl_read_entry / dopl_write_entry knowledge · dopl_get_skill procedures (${callRef("skill.authoring_guide")} before authoring) · dopl_list_agents the user's roles · dopl_browse_ontology the object graph · dopl_list_members who sees what · dopl_save_chat archives a session (${callRef("chats.guide")} first) · dopl_list_workspaces containers · dopl_get_status rooms, sessions, open asks · dopl_send_message reaches a MEMBER or their agent; channel tools may be DEFERRED: load them with ${loader}, then ${callRef("channel.rooms.list")}; law: ${callRef("channel.rooms.help")}. Deletion is app-only.`,
  });
}
