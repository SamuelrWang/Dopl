 ...
 */
export const AWAIT_HOLD_EXTERNAL_MARGIN_MS = 15_000;
 ...
 */
export const AWAIT_HOLD_EXTERNAL_DEFAULT_MS =
  EXTERNAL_CLIENT_ABORT_MS - AWAIT_HOLD_EXTERNAL_MARGIN_MS;
...
export const AWAIT_MIN_POLL_MS = 1_000;
 ...
 */
export const AWAIT_MAX_POLLS = Math.ceil(ENV_CEILING_MS / AWAIT_POLL_MS) + 2;
 ...
 * about getting one.
 */
export const AWAIT_SHORT_HOLD_MS = 60_000;
...
export function awaitHoldMs(
  ...
  return isDesktopRuntime(runtime)
    ? ENV_HOLD_MS
    : Math.min(ENV_HOLD_MS, AWAIT_HOLD_EXTERNAL_DEFAULT_MS);
}
