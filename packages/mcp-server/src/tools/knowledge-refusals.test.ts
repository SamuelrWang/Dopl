...
// ── S43 — pin is app-only, and says so instead of throwing ──────────────────
describe("S43: pin over MCP refuses with reason=session_required retry=no", () => {
  it("names the op, says nothing changed, and closes the door", async () => {
    const res = await opPin(
      clientWith({
        getKbStartupContext: vi.fn().mockResolvedValue({ pinnedChars: 0 }),
        setKbBasePinned: vi
          .fn()
          .mockRejectedValue(apiError(403, "SESSION_REQUIRED")),
      }),
      "my-base",
      undefined,
      true,
