...
const REFUSALS: Record<LaunchRefusalReason, { retry: "once" | "no"; says: string[] }> = {
  ...
  "no-template": { retry: "no", says: ["OPERATOR whose machine this is", "Do not re-issue the same id", "either it no longer exists, or it is not visible"] },
...
describe("the call itself", () => {
  ...
  it("names NO operator — there is no argument that could", async () => {
    ...
    expect(Object.keys(body)).toEqual(["channel", "threadId", "goal", "model", "template"]);
  });
  ...
  it("`no-template` from the MACHINE says WHOSE visibility failed, and does not guess why", async () => {
    ...
    expect(CHANNEL_DOCTRINE).toContain("either it no longer exists, or it is not visible");
  });
