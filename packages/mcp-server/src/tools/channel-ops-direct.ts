...
import { holdRow } from "./channel-directive-hold";
 ...
 */
export async function opDirectAgent(
  client: DoplClient,
  ref: string,
  agentId: string,
  body: string,
  ...
  direction = await holdRow(direction, (id) => client.getAgentDirection(id), opts.waitMs);
...
function renderDirection(d: AgentDirection): string {
  const head = `- \`${d.id}\` → agent \`${d.agentId}\` · **${d.status}**`;
  const body = `  - asked: ${inlineOr(d.body, "(empty)")}`;
  if (d.status === "delivered") {
    return [
      head,
      body,
      d.reply
        ? `  - answered: ${inlineOr(d.reply, "(empty)")}`
 ...
 */
export async function opReadDirections(
  client: DoplClient,
  ...
  if (opts.channel) {
    const channel = await resolveChannelOr(client, opts.channel);
    if (isErr(channel)) return channel;
    channelId = channel.id;
    label = inlineOr(channel.name, NO_NAME);
  }
