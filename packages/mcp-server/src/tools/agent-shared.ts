...
export const TEMPLATE_NOT_FOUND_CODE = "AGENT_TEMPLATE_NOT_FOUND";
...
export async function resolveTemplateRef(
  ...
  if (UUID_RE.test(needle)) {
    ...
    if (byId) return { kind: "found", template: byId };
    try {
      return { kind: "found", template: await client.getAgentTemplate(needle) };
    } catch (e) {
      if (!isApiError(e, 404, TEMPLATE_NOT_FOUND_CODE)) throw e;
