 ...
 */
export function addressedAgentIdsOf(m: ChannelMessage): string[] {
  ...
  const scalar = metaString(m, "to_agent_id");
 ...
 */
export function anyAgentAddressed(messages: ChannelMessage[]): boolean {
 ...
 */
export function agentRef(agentId: string, names: Map<string, string>): string {
  const id = inlineOr(agentId, UNREADABLE_ID);
 ...
 */
export function agentAddressTag(
  m: ChannelMessage,
