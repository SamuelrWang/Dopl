...
import { fileAndHold, pendingLines } from "./channel-ops-agent";
...
const REFUSAL_SENTENCES: Record<LaunchRefusalReason, string> = {
  ...
  "no-sdk":
    "that machine has NO AGENT RUNTIME available. Re-issuing will not change that, and nothing was changed. Tell your operator; it is a setup problem on their side.",
  "auth-hold":
    "the desktop is SIGNED OUT or its credential is being held, so it will act on nothing until a human signs in. Nothing was changed. Tell your operator — this needs them, not another call.",
  "no-counterparty":
    "the machine answered with a word that belongs to starting an agent, not to re-permissioning one. Nothing was changed. Check dopl_channel(op=\"read_sessions\") and report it to your operator if it repeats.",
  "no-template":
    "the machine answered with a word that belongs to starting an agent, not to re-permissioning one. Nothing was changed. Check dopl_channel(op=\"read_sessions\") and report it to your operator if it repeats.",
  ...
  "bad-name":
    "the machine answered with a word that belongs to RENAMING an agent, not to re-permissioning one — so nothing here explains what happened. Nothing was changed. Report it to your operator rather than re-issuing.",
};
