 ...
 */
export async function opEscalate(
  client: DoplClient,
  channelRef: string,
  ...
  if (
    ...
    return err(
      `Nothing was posted. \`recommendation.index\` is ${escalation.recommendation.index}, which is outside your own \`options\` list (${escalation.options.length} of them, indexed 0-${escalation.options.length - 1}). It is 0-based — the first option is 0. Re-send with the index of the option you meant, or omit \`recommendation\` entirely.`,
    );
  }
