...
import { opReadDirections } from "./channel-ops-direct";
...
export async function opStatus(
  ...
  const sessions = scoped
    ? await opReadSessions(client, opts.channel, opts.format)
    : await opReadSessionsAccount(client, directory);
  if (sessions.isError) return sessions;

  const directions = await opReadDirections(client, {
    channel: scoped ? opts.channel : undefined,
    agent: opts.agent,
  });
  if (directions.isError) return directions;

  return ok(`${textOf(sessions)}\n\n${textOf(directions)}`);
}
