 ...
 */
export const RETIRED_OPS = {
  post: 'retired: use send — dopl_channel(op="send", channel=…, body=…, to=…).',
  milestone:
    'retired: use send(kind="milestone") — same body, `thread` required, still one line.',
  escalate:
    'retired: use send(kind="decision") — `summary` is the question, `body` the context, `options` and `recommendation` unchanged.',
  ping: "retired: use send(to=…) — a directed send IS the delivery record, and its `delivery=` is the ack the ping row used to be.",
  pings:
    "retired: use read — a directed send lands in the transcript, so there is no second inbox to page.",
  create_thread:
    'retired: use send(thread="new", to=…, summary=<the title>) — the new thread id comes back on the result.',
  ...
  set_thread_mode:
    'retired: use rooms(action="thread_mode", channel=…, thread=…, mode=…).',
  update: 'retired: use rooms(action="update", channel=…, info_card=…).',
  help: 'retired: use rooms(action="help", section=…).',
  await:
    "retired: use read(since=…, wait_ms=…) — the hold is a knob on the read, not an op of its own.",
  launch_agent: 'retired: use manage(action="launch", channel=…).',
  end_agent: 'retired: use manage(action="end", to=<the agent handle>).',
  rename_agent:
    'retired: use manage(action="rename", to=<the agent handle>, name=…).',
  set_agent_mode:
    'retired: use manage(action="posture", to=<the agent handle>, posture=…).',
  direct_agent:
    'retired: use manage(action="direct", to=<the agent handle>, body=…).',
 ...
 */
export const RETIRED_OP_NAMES = Object.keys(RETIRED_OPS) as [
  RetiredOp,
  ...RetiredOp[],
];

/** True for a name that parses only to be redirected. */
export function isRetiredOp(op: string): op is RetiredOp {
  return Object.prototype.hasOwnProperty.call(RETIRED_OPS, op);
}
 ...
 */
export function retiredRedirect(op: RetiredOp): ToolResponse {
  return ok(`dopl_channel op="${op}" ${RETIRED_OPS[op]}`);
}
