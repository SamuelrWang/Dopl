...
import { err, strictParams, unusedParams, type RegisterTool, type ToolResponse } from "./respond";
...
export function registerOntologyTool(
  ...
  register(
    ...
    (args): Promise<ToolResponse> => {
      ...
      if (args.op === "restore") {
        const bad = strictParams("restore", args, ["object", "revision", "expected_version"]);
