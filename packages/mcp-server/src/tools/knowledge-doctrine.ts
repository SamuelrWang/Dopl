 ...
 */
export const knowledgeDoctrine = () => `# Sections

${bySet({
  legacy: `READ: excerpt (get_tree) → section → body; stop at the first that answers.
get_tree and read_file list an entry's headings; ${callRef("kb.outline", {}, { form: "op" })} adds what each
COSTS. read_file(section=) returns one; an unknown heading answers with the
outline.`,
  granular: `READ: excerpt (${callRef("kb.get_tree")}) → section → body; stop at the
first that answers. The tree and ${toolName("kb.read_file")} list an entry's headings;
${callRef("kb.outline")} adds what each COSTS. ${callRef("kb.read_file", { section: "" })}
...
- entries past ~1.5k chars carry ## headings, one topic each; an unsectioned
  one cannot be read in sections. ${bySet({ legacy: "write_file(section=)", granular: callRef("kb.write_file", { section: "" }) })} replaces one.
