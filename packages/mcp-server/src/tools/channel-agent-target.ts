 ...
 */
export function agentTarget(raw: string): { agent: string } | ToolResponse {
  const agent = bareAgentId(raw);
  if (isAgentId(agent)) return { agent };
  return err(
    [
...
export function isAgentTargetRefusal(
  answer: { agent: string } | ToolResponse,
 ...
 */
export function foreignAgent(agentId: string, verb: string): ToolResponse {
  return err(
    [
