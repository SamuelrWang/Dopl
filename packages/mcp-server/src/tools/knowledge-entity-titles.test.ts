...
describe("read_file flags a stored title that still looks escaped", () => {
  it("renders the signal under the heading, and does not error", async () => {
    const client = {
      ...
      // the unsectioned read lane routes through the part reader since the
      // headings work merged; the shape it returns is `{ entry, outline? }`.
      readKbFilePart: vi
        .fn()
        .mockResolvedValue({ entry: entry({ title: "R&amp;D", body: "hello" }) }),
    } as unknown as DoplClient;
