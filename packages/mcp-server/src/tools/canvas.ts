  ...
  register(
    ...
    async (args): Promise<ToolResponse> => {
      switch (args.op) {
        ...
        case "add_entry": {
          const miss = missingParams("add_entry", args, ["entry"]);
          if (miss) return miss;
          return opAddEntry(client, args.entry as string);
        }
        case "remove_entry": {
          const miss = missingParams("remove_entry", args, ["entry"]);
          if (miss) return miss;
          return opRemoveEntry(client, args.entry as string);
        }
        case "search_and_add": {
          const miss = missingParams("search_and_add", args, ["query"]);
          if (miss) return miss;
          return opSearchAndAdd(client, args.query as string, args.max_results);
        }
        case "rename_chat": {
...
async function opList(client: DoplClient): Promise<ToolResponse> {
  ...
  if (panels.length === 0) {
    return {
      content: [{ type: "text" as const, text: "Your canvas is empty. Use `dopl_canvas(op='add_entry')` to add entries, or `search_setups` to find them first." }],
    };
  }
...
async function opAddEntry(client: DoplClient, entry: string): Promise<ToolResponse> {
...
async function opRemoveEntry(client: DoplClient, entry: string): Promise<ToolResponse> {
