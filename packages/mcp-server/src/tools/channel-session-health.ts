 ...
 * nothing.
 */
export function sessionProgressClauses(s: ChannelSessionHealth): string[] {
  const out: string[] = [];
  const turns = s.turns;
  ...
  if (delta !== null && delta !== undefined) {
    out.push(`+${compactCount(delta)} since it last posted`);
  }
  return out;
}
 ...
 * silence.
 */
function wakeClause(s: ChannelSessionHealth, now: number): string | null {
  const seq = s.lastWakeSeq;
  const age = ageMs(s.lastWakeAt, now);
  const hasSeq = seq !== null && seq !== undefined;
  if (!hasSeq && age === null) return null;
  ...
  const when = age === null ? "" : ` ${coarseAge(age)} ago`;
 ...
 */
function deniedClause(s: ChannelSessionHealth): string | null {
  const n = s.deniedCalls;
  const counted = n !== null && n !== undefined;
  const tool = s.lastDeniedTool;
  if (counted && n === 0 && !tool) return null;
  if (!counted && !tool) return null;
  const head = counted
  ...
  const last = tool ? ` (last: ${inlineOr(tool, "(unnamed tool)")})` : "";
  return `${head}${last}`;
}
 ...
 */
function wedgedClause(s: ChannelSessionHealth): string | null {
  if (s.stale !== true) return null;
 ...
 */
export function sessionHealthClauses(
  s: ChannelSessionHealth,
  now: number,
): string[] {
  return [wakeClause(s, now), deniedClause(s), wedgedClause(s)].filter(
