...
// Match the local ToolResponse shape used by registerTool in server.ts.
type ToolResponse = {
...
export function registerKnowledgeTools(
  ...
  register(
    "kb_get_tree",
    "Get the full folder/entry tree for a knowledge base — metadata only, bodies stripped. Use as the first call when exploring a base. For an entry's body, follow up with `kb_read_file`.",
  ...
  register(
    "kb_delete_base",
    "Soft-delete a knowledge base. The base + its folders + entries become invisible but remain restorable from trash via `kb_restore_base`. Confirm with the user before calling — this is destructive from the user's perspective.",
    { base: z.string().describe("Base slug or id") },
    async ({ base: ref }) => {
      const base = await resolveBase(client, ref);
      if (!base) return err(`Knowledge base not found: ${ref}`);
  ...
  register(
    "kb_list_dir",
    "List the immediate folders + entries at a path inside a knowledge base. Empty/omitted path = the base root. Returns metadata only.",
    {
      base: z.string().describe("Base slug or id"),
      path: z.string().optional().describe("'/' or '' for root, else 'foo/bar'"),
    },
    async ({ base: ref, path }) => {
      const base = await resolveBase(client, ref);
      if (!base) return err(`Knowledge base not found: ${ref}`);
      ...
      if (listing.folders.length === 0 && listing.entries.length === 0) {
        ...
        for (const f of listing.folders) lines.push(`📁 ${f.name}/`);
        for (const e of listing.entries) lines.push(`📄 ${e.title}`);
      }
      return ok(lines.join("\n"));
    }
  );
  ...
  register(
    "kb_create_folder",
    "Create a folder at a path. Idempotent: existing folders are no-op'd; missing parent folders are created (mkdir -p semantics). Errors only if the path already resolves to an entry of the same name.",
    {
      base: z.string().describe("Base slug or id"),
      path: z.string().min(1).describe("Folder path, e.g. 'projects/foo'"),
    },
    async ({ base: ref, path }) => {
      const base = await resolveBase(client, ref);
      if (!base) return err(`Knowledge base not found: ${ref}`);
  ...
  register(
    "kb_delete_folder",
    "Soft-delete the folder at the given path. Children remain in the DB but stop appearing in active listings. Restorable from trash. Confirm with the user before calling.",
    {
      base: z.string().describe("Base slug or id"),
      path: z.string().min(1),
    },
  ...
  register(
    "kb_move_folder",
    "Move + rename a folder in one shot. The target path's leaf becomes the new folder name; missing parent folders along the new path are created. Cycles (moving a folder under its own descendant) return an error.",
    {
      base: z.string().describe("Base slug or id"),
  ...
  register(
    "kb_read_file",
    "Read an entry's full markdown body by path. The path must resolve to an entry, not a folder.",
    {
      base: z.string().describe("Base slug or id"),
      path: z.string().min(1).describe("Entry path, e.g. 'notes/q4-plan.md'"),
    },
    async ({ base: ref, path }) => {
      const base = await resolveBase(client, ref);
      if (!base) return err(`Knowledge base not found: ${ref}`);
  ...
  register(
    "kb_delete_file",
    "Soft-delete an entry at the given path. Restorable from trash. Confirm with the user before calling.",
    {
      base: z.string().describe("Base slug or id"),
      path: z.string().min(1),
    },
  ...
  register(
    "kb_move_file",
    "Move + rename an entry. Target path's parents are mkdir-p'd; leaf becomes the new title.",
    {
      base: z.string().describe("Base slug or id"),
      from_path: z.string().min(1),
      to_path: z.string().min(1),
  ...
  register(
    "kb_list_trash",
    "List soft-deleted bases, folders, and entries. Scope to one base with the optional arg, or omit to see workspace-wide trash.",
    { base: z.string().optional().describe("Optional: base slug or id") },
    async ({ base: ref }) => {
  ...
  register(
    "kb_restore_folder",
    "Restore a soft-deleted folder by id (paths don't apply post-deletion). Get the id from `kb_list_trash`.",
  ...
  register(
    "kb_restore_file",
    "Restore a soft-deleted entry by id. Get the id from `kb_list_trash`.",
  ...
  register(
    "kb_search",
    "Full-text search across the workspace's knowledge bases. Returns ranked entries with title, snippet (with `<b>` tags around matches), and the path you can pass to `kb_read_file` to fetch the body. Optional `base` slug narrows the search to one base.",
    {
      query: z.string().min(1).describe("Free-text query, e.g. 'cold open template'"),
      base: z.string().optional().describe("Optional base slug to narrow results"),
