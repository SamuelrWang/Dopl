 ...
 */
export const READ_DESCRIPTION_MAX_CHARS = 450;
...
export function renderExamples(calls: readonly object[]): string {
  ...
  if (block.length > EXAMPLES_MAX_CHARS) {
    throw new Error(
      `examples are ${block.length} chars, over the ${EXAMPLES_MAX_CHARS} cap — drop one, or shorten a value:\n${block}`,
    );
  }
  return block;
}
...
export function boundsOf(shape: ZodRawShape): Bound[] {
  ...
  for (const [name, prop] of Object.entries(schema.properties ?? {})) {
    ...
    if (phrase) bounds.push({ name, phrase: `${name} ${phrase}` });
  }
  return bounds;
}

/** One property's bound as a phrase, or "" when it declares none. */
function boundPhrase(prop: Record<string, unknown>): string {
  const num = (k: string): number | null =>
    typeof prop[k] === "number" ? (prop[k] as number) : null;
...
export interface DescriptionSpec {
  ...
  limits?: { shape: ZodRawShape; only?: readonly string[] };
 ...
 */
export function composeDescription(spec: DescriptionSpec): string {
  if (spec.headline.length > HEADLINE_MAX_CHARS) {
    throw new Error(
      `headline is ${spec.headline.length} chars, over the ${HEADLINE_MAX_CHARS} window a truncating client guarantees: ${spec.headline}`,
    );
  }
  if (spec.routing.length > ROUTING_MAX_LINES) {
    throw new Error(
      `${spec.routing.length} routing lines, over the ${ROUTING_MAX_LINES}-line ceiling — routing is a signpost, not a table`,
    );
  }
  const blocks: string[] = [
  ...
  if (prose.length > spec.cap) {
    throw new Error(
      `prose is ${prose.length} chars, over its ${spec.cap} cap — move standing doctrine into an MCP resource rather than raising the number:\n${prose}`,
    );
  }
  ...
  if (description.length > HARD_DESCRIPTION_CEILING) {
    throw new Error(
      `description is ${description.length} chars, past the ${HARD_DESCRIPTION_CEILING} truncation line no tool may cross:\n${description}`,
    );
  }
  return description;
}
