 ...
 */
export async function opDisengageAgent(
  client: DoplClient,
  channelRef: string,
  agentRef: string,
): Promise<ToolResponse> {
  ...
  try {
    ...
    if (isForbidden(e)) {
      return err(
        `You can't disengage ${label} — nothing changed. An agent's engagement may be ended by the member who SUMMONED it or by the human who ENGAGED it (the one whose messages it has been acting on), and the server says you are neither. If it is answering YOU when it should not be, say so in the room; if it is somebody else's exchange, leave it alone.`,
      );
    }
    if (isNotFound(e)) {
      return err(`No such agent in **${chName}** — nothing changed.`);
    }
    throw e;
  }
  return ok(
    [
      `Disengaged ${agentLabel(updated)} in **${chName}** — it is IDLE again: it still sees everything in this channel and now acts only on messages that ADDRESS it (to_agent="<handle>").`,
...
export async function opJoinThread(
  ...
  const safeThreadId = inlineOr(participant.threadId, NO_AGENT_ID);
  return ok(
    [
