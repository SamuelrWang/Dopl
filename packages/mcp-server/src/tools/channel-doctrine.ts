...
- REPLY WHERE YOU WERE ASKED. Asked in the main room, answer in the main room. Work traffic stays in its thread (thread="<id>"). You MAY also post to the main room unprompted, SPARSELY: that is a capability, not a habit.
...
THREADING: thread="new" opens the exchange and returns its id. A reply with NO \`to\` inside a thread goes to the thread's other party, server-resolved. A legacy \`task-<channel>-<seq>\` id has no thread row behind it; a send onto one reports \`landed=adhoc\`.
...
@-TAGS: \`@\` then the handle, in the BODY — there is no argument for it. A handle is a display name or an email's local part, lowercased, spaces squeezed out (\`@dianataylor\`) or just its first word (\`@diana\`). The result's \`tags=\` count is the verdict, and a 0 has three causes: (1) THE HANDLE WAS IN CODE — a handle inside backticks or a fenced block is quoted text and tags nobody; (2) the spelling missed — matching is EXACT, never a prefix; (3) two members answer to it, which resolves to NOBODY.
WHAT HAPPENS ON THE RECEIVING SIDE IS NOT THAT you are waiting on them: a send simply NOTIFIES them. Nothing you send sits in a queue over there waiting to be approved, so silence means nobody has picked it up YET. Your outgoing call is reviewed on YOUR machine, where you may have to wait for YOUR OWN operator to approve it.
...
Four actions READ (list · members · threads · help) and four WRITE (open · invite · thread_mode · update); a read-only session is refused the four writes BY NAME while the four reads still answer. "open" makes a channel (\`name\`) or a 1:1 (\`to\`, and only with \`name\` omitted — both together is refused, never resolved by precedence); "update" REPLACES the info card whole, so an omitted row is deleted and a blind write clobbers.
op="status" reads your own machine's live sessions and the directions waiting for them. Template, model, context, tokens, current tool and start time are YOUR OWN sessions only — a peer's agent is a handle and a state. A \`—\` cell was NOT REPORTED, and is not a zero.`;
...
\`to\` IS ONE PARTY, RESOLVED AT THE DOOR — a member (email or user id) or an agent (\`@agent-<id>\` or its handle), never two.
\`model\` IS VALIDATED NOWHERE — an id that machine does not recognise FALLS BACK silently.
...
\`info_card\` REPLACES THE WHOLE CARD — omit it entirely to read the card unchanged.
\`recommendation.index\` MUST BE INSIDE \`options\`, or the call is refused.`;
