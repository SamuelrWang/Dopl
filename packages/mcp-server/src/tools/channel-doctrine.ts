...
"launch" starts one, and its \`body\` is the FIRST INSTRUCTION it runs. A \`template\` name matching more than one is refused with every id listed. "end" stops one, and there is no undo — instance ids are never reused. "rename" sets a DISPLAY label — stored on that one machine, it reaches no server, is invisible to every other member and is never addressable from here. "posture" re-permissions a running one. "direct" sends it a private message and reads that turn's final text back.
...
WHAT IS ASKED FOR AND WHAT RUNS ARE NOT THE SAME THING: a \`model\` that machine does not recognize is NOT refused — it silently FALLS BACK, and nothing tells you.`;
...
\`color\` IS AN IDENTITY, NEVER A STATUS — how a reader tells two agents apart in a transcript; a red agent is not a failing one. The sixteen keys are UNIQUE PER CHANNEL ACROSS ALL MEMBERS, so another member's agent may hold the one you name: that is a 409 listing what is free, with NOTHING FILED — re-issue with the SAME \`client_msg_id\`. A key returns to the bank when its agent ENDS. Omit the argument and the first free key is assigned, which is what you want unless the operator named a colour; a room with all sixteen out runs the agent UNCOLOURED, and its posts still read as an agent's.`;
