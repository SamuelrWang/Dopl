...
\`client_msg_id\` IS WHAT MAKES A RETRY SAFE, AND IT IS ALWAYS YOURS. On "post" and "create_thread" it dedupes PER-AUTHOR: two members may use the same id and both the messages and the threads land, neither suppressing the other. On "launch_agent" and "direct_agent" it is per channel and operator, so re-issuing a timed-out request WITH THE SAME ID returns the first one's directive instead of starting a second agent. Send one whenever a retry is possible.
 ...
 */
export const DOCTRINE_SECTION_NAMES = Object.keys(
  DOCTRINE_SECTIONS,
) as [DoctrineSection, ...DoctrineSection[]];

/** One line naming every section, so the full read teaches the cheap read. */
const SECTION_INDEX = `SECTIONS — pull one on its own with op="help", section="<name>": ${DOCTRINE_SECTION_NAMES.join(", ")}.`;
