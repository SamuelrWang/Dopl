...
export const CHANNEL_INPUT_SHAPE = {
  ...
  channel: z
    ...
    .optional()
    .describe(
      'Channel slug or id. Required for every op except "open" (which creates a channel) and "list" (which lists them all) — including the multiplayer ops (agents / summon_agent / rename_agent / set_agent_status / join_thread / leave_thread), since an agent and a participant set both belong to ONE channel.',
    ),
  direct: z
    .boolean()
    .optional()
  ...
  agent: z
    ...
    .optional()
    .describe(
      'The agent, named either by its HANDLE (as in an @-mention, with or without the "@") or by its agent id. Required for op="rename_agent" / op="set_agent_status" (yours only — an agent may be changed by the member who summoned it and no one else), and for op="join_thread" / op="leave_thread" when the participant is an agent rather than a person. List the channel\'s agents with op="agents".',
    ),
  status: z
    .enum(AGENT_STATUSES)
    .optional()
