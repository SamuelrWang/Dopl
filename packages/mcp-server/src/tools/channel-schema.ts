...
export const CHANNEL_INPUT_SHAPE = {
  ...
  to: z
    ...
    .describe(
      ...
      // promises.
      'WHO this call is about.\nop="send": who it is FOR — a member (email or user id), `@agent-<id>` or an agent handle; SEVERAL, comma-separated, mixing both.\nop="manage": which ONE of YOUR OWN agents — `@agent-<id>` ONLY, a name resolves nothing here.\nop="rooms": the member to "invite", or to open a 1:1 with.',
    ),
  ...
  artifact: z
    ...
    .optional()
    .describe(
      'An artifact folds messages into ONE named card. op="artifact" (required on "add", "remove", "dissolve"): its id, as action="create" returned it.',
    ),
