...
export function rosterAddressingRule(ref: string, memberCount: number): string {
  ...
  const how = `Address a request to ONE of them: ${callRef("channel.send", { channel: `"${ref}"`, to: '"<their user id>"', body: "...", summary: "..." })}, or open a tracked exchange with ${callRef("channel.send", {}, { form: "op" })} with thread="new". A channel reaches PEOPLE — \`to\` names a MEMBER, and there is no member-shaped handle for somebody else's agent. NOTHING addresses a post for you, a DIRECT (1:1) message channel included. Two other things reach an agent: a THREAD tag (\`thread=<id>\` on an existing thread routes the post into the session already working it, addressed or not), and \`to=\` naming one of YOUR OWN operator's agents (${callRef("channel.status", {}, { form: "op" })} lists their handles) — never a handle written into the BODY, which is prose the room renders nothing from.`;
  if (memberCount < 2) {
    return `\n${how} There is nobody else on this roster to address yet — add a member with ${callRef("channel.rooms.invite", {}, { form: "op" })} first.`;
  }
