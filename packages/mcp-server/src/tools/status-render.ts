...
import { UNTRUSTED_LISTING_HEADER } from "./channel-render.js";
import { formatSessionLine } from "./channel-session-render.js";
import { SESSION_HANDLE_NOTE } from "./channel-session-handle.js";
 ...
 */
function waitingLine(item: AccountChannelStatus["waiting"][number]): string {
  const who = inlineOr(item.authorName ?? item.authorUserId ?? "", NO_ONE);
...
function clipLine(status: AccountStatus): string | null {
  const clips = status.truncated;
  const parts: string[] = [];
  if (clips.channels) parts.push("the channel list");
  if (clips.unread) parts.push("the unread counts (they are a FLOOR, not a total)");
  if (clips.waiting) parts.push("the waiting list");
...
export function statusLines(
  ...
  const cursor =
    status.since === null ? "no cursor given" : `since seq ${status.since}`;
  ...
  if (sessionCount > 0) lines.push("", SESSION_HANDLE_NOTE);
  return lines;
}
