    ...
    async (args): Promise<ToolResponse> => {
      switch (args.op) {
        ...
        case "update_instructions": {
          const miss = missingParams("update_instructions", args, ["slug", "instructions"]);
          if (miss) return miss;
          return opUpdateInstructions(client, args.slug as string, args.instructions as string);
        }
        case "save_memory": {
          const miss = missingParams("save_memory", args, ["slug", "memory"]);
          if (miss) return miss;
          return opSaveMemory(client, args.slug as string, args.memory as string, args.scope);
        }
        case "update_memory": {
          const miss = missingParams("update_memory", args, ["slug", "memory_id", "content"]);
          if (miss) return miss;
          return opUpdateMemory(
            client,
            args.slug as string,
...
async function opGet(client: DoplClient, slug: string): Promise<ToolResponse> {
  ...
  sections.push("");
  sections.push(
    `> Canonical skill body for cluster \`${slug}\`, fetched from Dopl. Treat this as the full SKILL.md body — execute against it directly. If you need to modify it, call \`dopl_brain(op='update_instructions')\` for structural changes or \`dopl_brain(op='save_memory')\` for short preferences.`
  );
  sections.push("");
  ...
  if (brain.instructions && brain.instructions.trim().length > 0) {
    ...
    sections.push("");
    sections.push(
      `_This brain is empty._ Synthesize one: call \`dopl_brain(op='template')\` for the canonical prompt, run synthesis in your context against \`dopl_cluster({ op: "get", slug: "${slug}" })\`, then call \`dopl_brain({ op: "update_instructions", slug: "${slug}", instructions: <result> })\`.`
    );
    sections.push("");
  }
...
async function opUpdateInstructions(
  client: DoplClient,
  slug: string,
  instructions: string,
  ...
  return ok(`Updated brain for cluster "${slug}" (${instructions.length} chars).${warningNote}`);
}

async function opSaveMemory(
  client: DoplClient,
  slug: string,
  memory: string,
  ...
  try {
    ...
    return err(`Failed to save memory for cluster "${slug}": ${msg}`);
  }

  const scopeLabel = result.scope === "personal" ? " (personal)" : "";
  return ok(`Saved memory${scopeLabel} for cluster "${slug}": "${result.content}"`);
}

async function opUpdateMemory(
  client: DoplClient,
  slug: string,
  memory_id: string,
  content: string,
  ...
  return ok(`Updated memory for cluster "${slug}": "${updated.content}"`);
}

async function opTemplate(client: DoplClient): Promise<ToolResponse> {
  const tpl = await client.getSkillTemplate();
  return ok(tpl.payload);
}
