...
import { agentLabel } from "./channel-agent-refs";
 ...
 */
function addressConflictLines(f: PostAddressFacts): string[] {
  const head = f.toAgents[0];
  if (!head || !f.toUserId || head.ownerUserId === f.toUserId) return [];
  return [
 ...
 */
function multiAddressLines(f: PostAddressFacts): string[] {
  if (f.toAgents.length <= 1) return [];
  return [
 ...
 */
export function agentAttributionNotes(
  toAgents: ChannelAgent[],
  asAgent: ChannelAgent | undefined,
): { toAgentNote: string; asNote: string } {
  const toAgentNote =
    toAgents.length > 0
      ? `, addressed to ${toAgents.length === 1 ? "agent" : `${toAgents.length} agents`} ${toAgents.map(agentLabel).join(", ")}`
      : "";
  return {
    toAgentNote,
    asNote: asAgent ? `, as agent ${agentLabel(asAgent)}` : "",
  };
}
