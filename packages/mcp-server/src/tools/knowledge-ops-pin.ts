 ...
 */
async function pinnedChars(client: DoplClient): Promise<number | null> {
  try {
...
function costLine(chars: number): string {
...
export async function opPin(
  ...
  try {
    if (path === undefined || path === "") {
      ...
      const capped = await enforceCap(client, before, () =>
        client.setKbBasePinned(base.id, false),
      );
      if (capped.refusal) return capped.refusal;
      return ok(
        [
    ...
    const capped = await enforceCap(
      client,
      before,
      () => client.setKbEntryPinned(read.entry.id, false),
      read.outline,
      path,
    );
    if (capped.refusal) return capped.refusal;
...
async function enforceCap(
  ...
  outline?: Outline,
  path?: string,
): Promise<{ warning?: string; refusal?: ToolResponse }> {
  if (before === null) return {};
  const after = await pinnedChars(client);
  if (after === null || after <= KB_PIN_WARN_CHARS) return {};
  if (after <= KB_PIN_MAX_CHARS || after <= before) {
  ...
  const suggestion =
    outline && outline.sections.length > 0
      ? [
          ...
          "",
          `There is no way to pin ONE section — a pinned section would be a stored pointer into prose that a rename empties. Split ${inlineOr(path ?? "", NO_PATH)} into separate entries and pin the one a session actually needs.`,
        ]
      : [
          "",
  ...
  return {
    refusal: err(
      [
        ...
        `${after.toLocaleString("en-US")} chars would exceed the ${KB_PIN_MAX_CHARS.toLocaleString("en-US")}-char ceiling. Past ~8,000 the launch payload ships POINTERS rather than content, so a pin this large does not add what it looks like it adds — it makes it likelier that something already pinned is the thing demoted.`,
        ...suggestion,
      ].join("\n"),
    ),
  };
