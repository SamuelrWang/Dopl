...
export function registerChatTools(
  ...
  register(
    ...
    CHATS_DESCRIPTION,
    {
      ...
      folder: z.string().max(80).optional().describe("op=export / op=update: folder NAME to file the chat under (created if missing). op=update: pass empty string to unfile."),
      visibility: z.enum(["private", "public"]).optional().describe("op=update: share ('public') or unshare ('private') with the workspace. op=export: defaults to private — only set public when the user explicitly says so."),
      ...
      name: z.string().min(1).max(80).optional().describe("op=create_folder (required): folder name."),
    },
...
async function opFolders(client: DoplClient): Promise<ToolResponse> {
  ...
  return ok(`## Chat folders — ${folders.length}\n\n${lines.join("\n")}`);
}
...
async function opCreateFolder(client: DoplClient, name: string): Promise<ToolResponse> {
  try {
    ...
    return ok(`Created folder **${folder.name}** (\`${folder.id}\`).`);
  } catch (e) {
