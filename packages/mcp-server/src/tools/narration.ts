 ...
 * with content.
 */
export function flattenFenced(raw: string, max: number): string | null {
  const flattened = raw
    .replace(/[\u0000-\u001F\u007F\u0085\u2028\u2029]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  if (boundary >= Math.floor(max * 0.6)) return `${window.slice(0, boundary + 1)} …`;
...
export const NO_PATH = "`(unreadable path)`";
