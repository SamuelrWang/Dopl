...
describe("THE LAW is stated, in full, in the tool description", () => {
  ...
  it("says a channel is a ROOM and a participant thread is a BREAKOUT ROOM", () => {
    expect(DESCRIPTION).toContain("A CHANNEL is a ROOM");
    expect(DESCRIPTION).toContain("A THREAD with participants is a BREAKOUT ROOM");
  });

  it("says addressing an agent by handle is what starts it, singly or several at once", () => {
    ...
    expect(DESCRIPTION).toContain("ADDRESS AN AGENT BY HANDLE TO START IT");
    expect(DESCRIPTION).toContain("to_agent=");
    expect(DESCRIPTION).toContain("to_agents=");
  });
  ...
  it("states ENGAGEMENT: a human's tag starts a conversation, not one turn", () => {
    ...
    expect(DESCRIPTION).toContain("BEING ADDRESSED BY A HUMAN ENGAGES YOU");
    expect(DESCRIPTION).toContain(
      "you also act on that person's UNADDRESSED messages in this channel",
    );
    expect(DESCRIPTION).toContain(
      'You go IDLE again on op="disengage_agent", on park or dismiss, or after about an hour',
    );
    expect(DESCRIPTION).toContain(
      "it addresses another agent and engages nothing",
    );
  });
  ...
  it("keeps THE LOOP BRAKE absolute — agents do not wake each other by talking", () => {
    ...
    expect(DESCRIPTION).toContain(
      "an AGENT-authored unaddressed message engages nobody and starts nobody, in a room of two or of ten",
    );
    expect(DESCRIPTION).toContain(
  ...
  it("states the thread handshake: ONE opener, chosen by agent id, with the derived key", () => {
    ...
    expect(DESCRIPTION).toContain("EXACTLY ONE OF YOU OPENS THE THREAD");
    expect(DESCRIPTION).toContain(
      "the addressed agent whose AGENT ID sorts FIRST, lexicographically",
    );
    expect(DESCRIPTION).toContain(
      'client_msg_id="thread-open-<channelId>-<seq>"',
    );
    ...
    expect(DESCRIPTION).toContain("the channel's UUID, NEVER its slug");
    // The other side of the same rule: everyone else WAITS rather than creating.
    expect(DESCRIPTION).toContain('do NOT call "create_thread"');
    expect(DESCRIPTION).toContain(
      "Inside a thread everyone in it hears everything: no tagging between participants",
    );
  });

  it("keys addressing a HUMAN on `as_agent` — the param the desktop actually reads", () => {
    ...
    expect(DESCRIPTION).toContain(
      'ADDRESSING A HUMAN (to="<email or user id>") DOES ONE OF TWO THINGS AND `as_agent` DECIDES WHICH.',
    );
    expect(DESCRIPTION).toContain(
      'Post AS YOURSELF — as_agent="<your handle>" together with to= — and that person is NOTIFIED and no agent of theirs starts.',
    );
    expect(DESCRIPTION).toContain(
      "Post WITHOUT as_agent and the same to= is a REQUEST from your operator: it triggers that member's listener, which is what starts their agent.",
    );
    ...
    expect(DESCRIPTION).toContain("BLOCKED AND NEED A PERSON?");
    expect(DESCRIPTION).toContain(
      'Post with BOTH as_agent="<your handle>" and to=<them>: that notifies the person and starts no agent.',
    );
  });

  it("states `as_agent` on a breakout post as the CONDITIONAL it is, actionably", () => {
    ...
    expect(DESCRIPTION).toContain(
      "ALWAYS pass it on a post into a BREAKOUT THREAD",
    );
    expect(DESCRIPTION).toContain(
      "it is REQUIRED whenever an AGENT ROW is what admits you there",
    );
    // The conditional's other half — and the reason "always" is the actionable
    // instruction rather than a hedge.
    expect(DESCRIPTION).toContain(
      "unless the set ALSO holds you as a person — which you cannot tell from inside the thread",
    );
    // …and the absolute must not come back.
    expect(DESCRIPTION).not.toContain(
      "also REQUIRED to post into a BREAKOUT THREAD you belong to as an agent",
    );
  });

  it("names the three things to act on, and calls everything else ambient context", () => {
    expect(DESCRIPTION).toContain(
      "messages in a breakout thread you are a participant of",
    );
    ...
    // itself once.
    expect(DESCRIPTION).toContain(
      "while you are engaged — the unaddressed messages of the human who engaged you",
    );
    expect(DESCRIPTION).toContain(
  ...
  it("says to reply where you were asked", () => {
    ...
    expect(DESCRIPTION).toContain("Summoned in the main room, answer in the main room");
    expect(DESCRIPTION).toContain("Work traffic stays in your breakout thread");
  });

  it("says an agent has a NAME, posts under it, and may never wear another's", () => {
    expect(DESCRIPTION).toContain("YOU HAVE A NAME");
    expect(DESCRIPTION).toContain('as_agent="<your handle>"');
    expect(DESCRIPTION).toContain("Never post as another agent");
    expect(DESCRIPTION).toContain("the server verifies who owns it and refuses");
  });
  ...
  it("keeps the law SHORT — the budget the bullet count could not enforce", () => {
    ...
    expect(LAW.length).toBeLessThanOrEqual(4000);
...
describe("what the law and the ops around it may NOT say", () => {
   ...
   */
  const KEYED =
    /as_agent|never (spawns|starts|wakes|triggers)|does not (spawn|start|wake|trigger)|starts no agent|no agent of theirs starts|reaches no one's agent|no one's agent wakes|nobody's agent (wakes|woke)|wakes nobody/i;

  it("never states what addressing a PERSON does to their agent without naming `as_agent`", () => {
    const offenders = sentences.filter(
      (s) => OTHER_SIDE.test(s) && STARTS.test(s) && !KEYED.test(s),
    );
    expect(
      offenders,
      `these sentences claim something about another member's agent starting (or not) without keying it on \`as_agent\` or denying it outright:\n- ${offenders.join("\n- ")}`,
    ).toEqual([]);
  });

  it("the guard has teeth — it catches the exact sentence that shipped", () => {
    ...
    const shipped =
      "Pass `to` (an email or user id of a channel member) when your message is a request aimed at one specific person's agent: that member's listener is then the only one triggered.";
    const caught =
      OTHER_SIDE.test(shipped) && STARTS.test(shipped) && !KEYED.test(shipped);
    expect(caught, "the negative guard would not catch the shipped sentence").toBe(
      true,
    );
  });
   ...
   */
  const ENGAGES =
    /\b(re-?engages?|re-?engaging|engages?\s|engaged\s+(you|it|them|by|an agent|that agent)|(is|are|was|were|has|have|been|stays?|keeps?|remains?)\s+engaged)\b/i;
  /**
   ...
   */
  const HUMAN_KEYED =
    /\bhumans?\b|\bpeople\b|\bpersons?\b|engages? (nothing|nobody|no one|no agent)|cannot engage|no re-?engage|NO RE-?ENGAGE/i;

  it("never promises ENGAGEMENT without naming the human credential it requires", () => {
    const offenders = sentences.filter(
      (s) => ENGAGES.test(s) && !HUMAN_KEYED.test(s),
    );
    expect(
      offenders,
      `these sentences say engagement happens without keying it on a HUMAN author (an MCP post is agent-credentialed and can never cause it):\n- ${offenders.join("\n- ")}`,
    ).toEqual([]);
  });

  it("the engagement guard has teeth — it catches the exact sentence that shipped", () => {
    ...
    // description does.
    const shipped =
      "To pick the exchange back up, address it by handle again — that re-engages it.";
    expect(
      ENGAGES.test(shipped) && !HUMAN_KEYED.test(shipped),
      "the engagement guard would not catch the shipped sentence",
    ).toBe(true);
    // …and it must NOT fire on the honest statement of the same fact.
    const honest =
      "Your OWN to_agent post is not that — it addresses another agent and engages nothing, because engagement is stamped only for a HUMAN author.";
    expect(ENGAGES.test(honest) && !HUMAN_KEYED.test(honest)).toBe(false);
    // …nor on ENDING an engagement, which the reader really can do.
    const ending =
      'Parking or dismissing also ENDS any engagement: a process that is not running cannot be listening for anyone.';
    expect(ENGAGES.test(ending)).toBe(false);
  });

  it("does not describe a thread as writable by exactly two members with no exception", () => {
    ...
    expect(DESCRIPTION).toContain(
      "whose participant set (people and agents, see op=\"get_thread\") replaces that pair",
    );
  });
});

describe("the multiplayer ops are documented where an agent will look", () => {
  it("every new op is named in the description (the parity contract, asserted here too)", () => {
    for (const op of [
      "agents",
      "summon_agent",
      "rename_agent",
      ...
      expect(DESCRIPTION, `op="${op}" is undocumented`).toContain(`"${op}"`);
    }
  });

  it("post teaches that as_agent is REQUIRED for agent attribution and is server-verified", () => {
    expect(DESCRIPTION).toContain(
      "it is REQUIRED for a post to be attributed to an agent at all",
    );
    expect(DESCRIPTION).toContain("SERVER-VERIFIED");
  });

  it("create_thread teaches the participants form", () => {
    expect(DESCRIPTION).toContain('"agent:<handle>" / "user:<email>"');
  });

  it("dismissal is described as a status, not a delete", () => {
    expect(DESCRIPTION).toContain('"dismissed" is a status, NOT a delete');
  });
});
