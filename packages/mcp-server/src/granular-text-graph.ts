...
export const GRAPH_TEXT: Readonly<Record<string, ToolText>> = {
  dopl_list_agents: {
    description:
      'Agent identities you can see here, grouped by sharing, your personal ones included. An identity is a role of the user that agents launch from; running agents are dopl_get_channel action="status".',
  },
  dopl_get_agent: {
  ...
  dopl_manage_agent: {
    description:
      "Create or update an agent identity, or grant one you made into a channel or container. An update needs expected_version. Knowledge attaches by reference, and only what you can read.",
    params: {
  ...
  dopl_browse_ontology: {
    description:
      'The object graph without opening objects. action="map" ontologies and their objects with direct items (two levels only), "anchor" your own object, "resolve" objects whose name or subtitle contains `query` (max 20).',
  ...
  dopl_edit_object: {
    description:
      "Edit one thing on the object graph: create or update an object, set or remove one attribute, relationship, action or template field, or claim_anchor (link yourself to it). remove_* strips a field, never the object.",
    params: {
  ...
  dopl_get_member: {
    description:
      'Who someone is: who="me" your id, role, teams and session runtime; "member" a profile and access (another member\'s: admin-only); "team" one team; "my_access" your level per resource.',
    params: { member: "User id, email or display name.", team: "Team id or name." },
  },
  dopl_list_members: {
    description:
      'Rosters: list="members" (invited and deactivated included, so read status), "teams" with their grants, "access_matrix" knowledge bases and skills by access (every one for admins, your view otherwise).',
    params: { fields: 'list="members": comma-separated subset of name,role,status,teams.' },
  },
  dopl_list_chats: {
    description:
      'The chat archive: list="chats" newest first (`query` matches titles and overviews, never transcripts) or "folders" with their sharing.',
  ...
  dopl_save_chat: {
    description:
      'Archive a session: export (a header plus one summarized entry per message), append messages, update the header or sharing, create or update a folder. Read dopl_get_guide topic="chats" first; re-scoping a folder re-scopes its chats.',
    params: {
