 ...
 */
function extractKeywords(entries: ClusterDetailEntry[]): string[] {
 ...
 */
export function renderClusterSkillMd(params: ClusterSkillParams): string {
  ...
  if (brain.instructions) {
    ...
    sections.push("");
    sections.push(
      `To fill this in: call the MCP tool \`get_skill_template\` for the canonical prompt, run synthesis against this cluster's entries (fetch with \`get_cluster("${slug}")\`), then call \`update_cluster_brain("${slug}", <your synthesized body>)\`. Run \`sync_skills\` again afterwards to refresh this file.`
    );
  }
  sections.push("");
  ...
  sections.push("");
  sections.push(`**Do NOT edit this file directly.** The Dopl database is the single source of truth for this cluster's brain and memories. Local edits will be silently overwritten on the next \`sync_skills\` call and won't propagate to other devices, the web UI, or other agents using the same cluster.`);
  sections.push("");
  sections.push(`**To change the skill's instructions** (structural guidance):`);
  sections.push(`Call the MCP tool \`update_cluster_brain\` with slug \`${slug}\`.`);
  sections.push("");
  sections.push(`**To record a preference or correction** (short, environmental, or user-specific):`);
  sections.push(`Call the MCP tool \`save_cluster_memory\` with slug \`${slug}\`.`);
  sections.push("");
  sections.push(`**To refresh this file from the database after a DB change:**`);
  sections.push(`Run \`sync_skills\` (optionally with \`force: true\` to overwrite).`);
  sections.push("");
 ...
 */
export function renderEntryReferenceMd(entry: ClusterDetailEntry): string {
  const sections: string[] = [];
 ...
 */
export function renderGlobalCanvasSkillMd(clusters: ClusterSummary[]): string {
  const sections: string[] = [];
