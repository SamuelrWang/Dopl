...
const OVER_BUDGET_CEILINGS: Record<string, number> = {
  ...
  dopl_members: 1453,
  dopl_ontology: 1922, // ⚠ **1,924 → 1,922 (2026-09-09): BANKED, NOT RAISED** — the home-ontology wave added a FENCE sentence (a lend carries a per-channel LEVEL; an agent gets its operator's) and paid for every character of it out of this same description; `tools/ontology.ts › ONTOLOGY_PROSE_BUDGET` lists what was trimmed. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.**
  dopl_skill: 1593,
};
...
const DOCTRINE_CEILING = 9_979; // ⚠ **9,905 → 9,979 (+74), AND THE FALL THAT LICENSES IT:** 2026-09-06, the artifacts wave: `channel-schema.ts`'s pushed `.describe()` prose fell 762 (9,445 → 8,683 measured) as standing contract prose moved into the PULLED doctrine; the doctrine rose 74 to pay for it. 762 down, 74 up — a 688-char net fall on the surface every client is handed on every connection. Every number below is a MEASUREMENT from a real gate run, not a hand count. // ⚠ **9,923 → 9,905 (2026-09-04, −18): THE ADDRESSING LINE SAYS MORE AND COSTS LESS.** `channel-doctrine.ts › MODEL`'s "WHO A MESSAGE IS FOR" gained the read line's new arrow vocabulary (`→ you` / `→ @<agent>` / `→ <member>` / `→ nobody`, then the delivery word) and the rule that a person who names nobody is still answered — RR3's arms 3 and 4, added the same day because "two live agents and no setting" had been answering NOBODY, which is the ordinary shape of a multiplayer channel. It is paid for by deleting MODEL's restatement of "anything threaded into an exchange you are a party to is yours", which the LAW block already carries. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **9,446 → 9,923 (2026-09-03, +477): A SECOND RESOURCE, `dopl://doctrine/knowledge`, AND THE WHOLE OF IT IS THE RISE.** It is 477 characters against the channels doctrine's ~9,000, because a knowledge base has a filesystem where a channel has a protocol — what no argument could carry is the ORDER (excerpt → outline → section → body) and the write duty that makes the order possible. ⚠ **THE PUSHED SIDE ROSE IN THE SAME WAVE (`SERVED_TOTAL_CEILING` +618), SO THIS IS NOT A LAUNDERING** — the routing sentence is deliberately in BOTH, because several clients read no resources at all, and the saving is on neither ledger: it is per READ. `knowledge-doctrine-budget.test.ts` bounds this document on its own so the cheap one cannot become where evicted paragraphs land. ⚠ **8,960 → 9,446 (2026-09-03): A RISE, RECORDED HERE BECAUSE THIS IS THE GATE THAT WOULD OTHERWISE ABSORB IT.** ~490 pulled once against ~1,400 pushed per empty hold — `channel-doctrine-budget.test.ts` states the trade in full.
