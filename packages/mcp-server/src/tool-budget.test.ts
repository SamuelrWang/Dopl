...
const SCHEMA_CEILINGS: Record<string, number> = {
  ...
  dopl_channel: 8899, // ⚠ **8,901 → 8,899 (2026-09-15, −2): the topic→description RELABEL, which paid for itself — see `channel-schema.ts › SCHEMA_MAX_CHARS`, where the two characters are argued. ⚠ 8,664 → 8,901 (2026-09-13, +237): THE AGENT-COLOUR PARAM.** The full argument — what the `color` field costs, the 134 of pushed prose that was MOVED into the pulled doctrine to fund part of it, and the TWO cheaper shapes that were measured and REFUSED (a `pattern`, 153 cheaper, forbidden by `tool-style.test.ts`; `posture`'s clamp sentence, pinned on its describe by two suites) — is stated once, at `channel-schema.ts › SCHEMA_MAX_CHARS`, and is not restated here. This number is that one plus the registrar's injected `workspace` arg. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.**
  dopl_chats: 3553,
...
const DOCTRINE_CEILING = 10_837; // ⚠ **9,979 → 10,837 (2026-09-13, +858): THE PULLED HALF OF THE AGENT-COLOUR WAVE.** 134 of it is prose MOVED off the pushed describes and the rest is the colour rule itself, in `FIELDS` — the section whose whole subject is an argument whose rule does not fit in a `.describe()`. The argument in full, including why `FIELDS` rather than `MANAGE` (the per-section ratchet chose it), is at `channel-doctrine-budget.test.ts › DOCTRINE_MAX_CHARS`. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.**
