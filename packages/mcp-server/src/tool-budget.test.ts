 ...
 * claims a win.
 */
const SERVED_TOTAL_CEILING = 48_726; // ⚠ **48,791 → 48,726 (2026-09-18, −65): THE ROUND-1 CONSISTENCY BATCH, AND IT PAYS FOR ITSELF WITHOUT SPENDING THE `workspace=` ALIAS.** Eleven facts an agent could not derive were ADDED to this surface and the total still FELL, which is the only shape a wave of additions is allowed to take here. ⚠ **WHAT WAS ADDED**: `to` says its THREE meanings as three lines (S13); `kind` carries Samuel's record CHOOSER (Q5); `artifact` says what an artifact IS (S16); `visibility` on `dopl_agent` stops calling a home channel a "workspace" (S12) and on `dopl_kb` states the PRIVATE audience and ONE WAY (S14), with `dopl_skill` saying the reversible half; `title` says it RENAMES the leaf (S19); `confirm_token` says the flow is TWO CALLS (S16); `level` says a channel grant is READ-ONLY (Q4); `response_format` stops naming ops the tool does not have (S16); and `dopl_status` explains "over-reports" instead of only warning of it (S16(d)). ⚠ **WHAT PAID FOR IT, AND NONE OF IT IS A DISCLOSURE**: the shared `response_format` illustration, which is not the guarantee (−27 × 5 schemas = −135, the largest single source); `expected_version`'s restatement of `force`; four no-fact-twice / no-bound-twice cuts inside `channel-schema.ts`; and TWO RULES MOVED OFF THE PUSHED SURFACE to where they bite — `guest_write`'s reach to `grant.ts › levelReach` (the grant RESULT) and the "waiting on you" rule to `status-render.ts` (the status HEADER), both per-call and both ratcheted by `write-result-budget.test.ts`. ⚠ **THE PULLED DOCTRINE DID NOT MOVE** (12,120, unchanged), so none of this is laundering. ⚠ **AND NOTHING HERE SPENT BATCH A's ALIAS RETIREMENT** — that 189 is still unbanked and still A's to spend. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **RE-DERIVED WHOLE AT INTEGRATION (2026-09-18)** — the home-channel scope fence and the addressing structure both moved this ceiling in the same wave, so neither branch's figure is the merged truth and this one is a fresh sum through the real `listTools()`. ⚠ **48,834 → 48,826 (2026-09-18, −8): THE ADDRESSING STRUCTURE, BANKED.** The whole fall is `dopl_channel`'s schema — `kind` gained `"record"` and `to` gained the recipient LIST, both funded by no-fact-twice cuts inside the same shape; see `SCHEMA_CEILINGS.dopl_channel`. ⚠ **THE RULE WENT TO THE PULLED DOCTRINE** (+1,080 there), which is why this side moves DOWN in a wave that adds a capability. // ⚠ **48,639 → 48,834 (2026-09-17, +195): R-32's CONTAINER ADDRESS, AND THE WHOLE RISE IS THE DEPRECATED ALIAS KEY.** +189 is nine schemas × 21 (`SCHEMA_CEILINGS.dopl_agent` carries the argument), +8 is `dopl_map`'s description gaining the container nodes it now renders, −2 is `dopl_channel`'s relabel. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 195 chars per connection buys a container a FIRST-CLASS ADDRESS — `home` for the caller's own, a slug for a home channel — on every op that takes one, and buys one release in which no caller that still sends `workspace=` breaks. ⚠ **THE TRIM CAME FIRST AND IT IS WHY THE NUMBER IS 195 RATHER THAN ~640**: the alias is published UNDESCRIBED, `container`'s description is 9 chars under the one it replaced while naming one more address form, and the instructions fell 6 in the same change. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,611 → 48,639 (2026-09-15, +28): the whole rise is `dopl_channel`'s schema — see `SCHEMA_CEILINGS.dopl_channel`. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 28 chars per connection buys an orchestrator learning the tag its own agent actually answers to, and the alternative is a post addressed to the wrong agent with nothing to show for it. // ⚠ **48,615 → 48,611 (2026-09-15, −4): THE AGENT-ID-VISIBILITY WAVE, BANKED.** The whole fall is `dopl_channel`'s schema: `name` gained the LAUNCH clause and lost the false one (*"nothing resolves an agent by its name"*). See `SCHEMA_CEILINGS.dopl_channel`. ⚠ **A FALSE SENTENCE IS NOT A BUDGET SAVING TO BE SPENT** — it is why the true one fit. ⚠ **48,605 → 48,615 (2026-09-15, +10): `dopl_kb`'s `ambiguous_slug` row, three de-duplications inside that one description paying ~74 of its ~84. The whole rise is that tool's; see `DESCRIPTION_CEILINGS.dopl_kb` for what was cut, and for the two cuts two gates reverted. ⚠ **48,607 → 48,605 (2026-09-15, −2): the whole fall is `dopl_channel`'s topic→description relabel, banked here in the same change that measured it. ⚠ 48,370 → 48,607 (2026-09-13, +237): THE AGENT-COLOUR PARAM, AND THE WHOLE RISE IS ONE SCHEMA'S.** See `SCHEMA_CEILINGS.dopl_channel`, and `channel-schema.ts › SCHEMA_MAX_CHARS` for the funding and for the two cheaper shapes that were measured and refused. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 237 chars per connection buys a launch lane that can NAME a colour, and the alternative is sixteen agents a reader cannot tell apart in a transcript. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **−35 (2026-09-18): BANKED.** The per-tool rows above account for every character: −2 × 9 schemas for the `container` arg, −5 for the launch `template` describe, −12 for `dopl_channel`'s description. ⚠ **48,639 → 48,834 (2026-09-17, +195): R-32's CONTAINER ADDRESS, AND THE WHOLE RISE IS THE DEPRECATED ALIAS KEY.** +189 is nine schemas × 21 (`SCHEMA_CEILINGS.dopl_agent` carries the argument), +8 is `dopl_map`'s description gaining the container nodes it now renders, −2 is `dopl_channel`'s relabel. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 195 chars per connection buys a container a FIRST-CLASS ADDRESS — `home` for the caller's own, a slug for a home channel — on every op that takes one, and buys one release in which no caller that still sends `workspace=` breaks. ⚠ **THE TRIM CAME FIRST AND IT IS WHY THE NUMBER IS 195 RATHER THAN ~640**: the alias is published UNDESCRIBED, `container`'s description is 9 chars under the one it replaced while naming one more address form, and the instructions fell 6 in the same change. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,611 → 48,639 (2026-09-15, +28): the whole rise is `dopl_channel`'s schema — see `SCHEMA_CEILINGS.dopl_channel`. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 28 chars per connection buys an orchestrator learning the tag its own agent actually answers to, and the alternative is a post addressed to the wrong agent with nothing to show for it. // ⚠ **48,615 → 48,611 (2026-09-15, −4): THE AGENT-ID-VISIBILITY WAVE, BANKED.** The whole fall is `dopl_channel`'s schema: `name` gained the LAUNCH clause and lost the false one (*"nothing resolves an agent by its name"*). See `SCHEMA_CEILINGS.dopl_channel`. ⚠ **A FALSE SENTENCE IS NOT A BUDGET SAVING TO BE SPENT** — it is why the true one fit. ⚠ **48,605 → 48,615 (2026-09-15, +10): `dopl_kb`'s `ambiguous_slug` row, three de-duplications inside that one description paying ~74 of its ~84. The whole rise is that tool's; see `DESCRIPTION_CEILINGS.dopl_kb` for what was cut, and for the two cuts two gates reverted. ⚠ **48,607 → 48,605 (2026-09-15, −2): the whole fall is `dopl_channel`'s topic→description relabel, banked here in the same change that measured it. ⚠ 48,370 → 48,607 (2026-09-13, +237): THE AGENT-COLOUR PARAM, AND THE WHOLE RISE IS ONE SCHEMA'S.** See `SCHEMA_CEILINGS.dopl_channel`, and `channel-schema.ts › SCHEMA_MAX_CHARS` for the funding and for the two cheaper shapes that were measured and refused. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 237 chars per connection buys a launch lane that can NAME a colour, and the alternative is sixteen agents a reader cannot tell apart in a transcript. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,791 → 49,093 (2026-09-18, +302): THE WHOLE RISE IS `dopl_agent`'s SCHEMA** — see `SCHEMA_CEILINGS.dopl_agent` for what `expected_version`/`force` buy, why the cheap version was refused, and why no funding was taken from the one restatement left in that shape. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY**, which is why the rise is recorded rather than absorbed: 302 chars per connection buys a template edit that cannot silently discard another writer's system prompt, and the alternative is a lost write with nothing on the wire that says so. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,801 → 49,115 (2026-09-18, +314): THE WHOLE RISE IS `dopl_kb`'s SCHEMA** — the S53 idempotency key plus `force`'s MOVED refusal. The accounting and the argument are on `SCHEMA_CEILINGS.dopl_kb`; no description, no other schema and the instructions are untouched. ⚠ **THE REST OF THAT BATCH LANDED PER-CALL, DELIBERATELY**: eight refusals gained `field=`, `limit=`, `reason=` and `retry=` and not one of them is served — they are quoted into a 400 and governed by `write-result-budget.test.ts`. **Putting words in the refusal rather than in a describe is this budget's own instruction followed, not worked around** — the key is here only because a parameter has nowhere else to live. ⚠ NEVER QUOTE THIS NUMBER — re-derive it. // ⚠ **48,791 → 48,801 (2026-09-18, +10): THE WHOLE RISE IS `dopl_channel`'s SCHEMA** — the launch goal's cap published on `body`, part-funded by `to`'s half-false handle promise being scoped to the op that keeps it. See `SCHEMA_CEILINGS.dopl_channel` for the accounting and `channel-schema.ts › SCHEMA_MAX_CHARS` for the argument; no description, no other schema and the instructions are untouched. ⚠ **AND THE PER-CALL SIDE IS WHERE THE REST OF THE FIX LANDED, DELIBERATELY**: `channel-errors.ts › FIELD_CAPS_NOTE` now carries all three `body` caps and `name`'s, and that string is quoted only into a 400 — it is governed by `write-result-budget.test.ts` and costs a connection nothing. **Putting words in the refusal rather than in a describe is this budget's own instruction followed, not worked around.** ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.**
/**
