...
const OVER_BUDGET_CEILINGS: Record<string, number> = {
  ...
  dopl_channel: 1603, // ⚠ **1,587 → 1,603 (+16, 2026-09-06), AND IT IS A RISE PAID TO A DIFFERENT
...
const SCHEMA_CEILINGS: Record<string, number> = {
  ...
  dopl_map: 250,
  dopl_members: 845,
  dopl_ontology: 2809, // ⚠ **2,816 → 2,809 (2026-09-11, −7): THE VOCABULARY RULING, BANKED NOT RAISED.** A cluster is an *ontology* to a reader and a column is an *object*, so three `.describe()` strings were respelled rather than added to: `cluster` → "Ontology slug, id, or exact name." (+1), `parent` lost the `column/object` pair for plain `object` (−7), `name` went `(cluster/column/object/action)` → `(ontology/object/item/action)` (−1). The OP NAMES did not move — `create_column` is still the op, and `parity.test.ts` still reads it. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.**
  dopl_search: 1076, // ⚠ 1,080 → 1,076 (2026-09-06): measured after the doctrine move.
  dopl_skill: 3058,
 ...
 * claims a win.
 */
const SERVED_TOTAL_CEILING = 48_370; // ⚠ **48,384 → 48,370 (2026-09-11, −14): THE VOCABULARY RULING, BANKED NOT RAISED** — half is `dopl_ontology`'s schema (−7, see `SCHEMA_CEILINGS.dopl_ontology`) and half is `dopl_map`'s description, where *"ontology clusters"* became *"ontologies"* (−7). `dopl_ontology`'s own description netted ZERO across four respellings, which is the shape this file asks for: the headline's containment ladder and `op="map"`'s bullet paid for the `create_column` gloss and the `cluster_not_found` noun. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,404 → 48,384 (2026-09-10, −20): THE CREDIT-MODEL v2.1 MERGE, BANKED NOT RAISED** — the whole of the move is the credit refusal’s wording (`tool-errors.ts › CREDITS_EXHAUSTED.meaning`, −10 chars served twice), carried in from `feat/credit-model-v2` on top of the ontology wave’s 48,404. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,407 → 48,404 (2026-09-09, −3): BANKED, NOT RAISED** — the whole of the move is `dopl_ontology`'s description, which absorbed the CHANGELOG lane's per-field clause and came out three characters SMALLER. See `OVER_BUDGET_CEILINGS.dopl_ontology`. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,410 → 48,407 (2026-09-09, −3): BANKED, NOT RAISED** — the whole of the move is `dopl_kb`'s description, which absorbed the CHANGELOG clause and came out three characters SMALLER. See `OVER_BUDGET_CEILINGS.dopl_kb` for the trims that paid for it, and for the three that were reverted because `tool-scope-claims.test.ts` owns them. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **48,412 → 48,410 (2026-09-09): BANKED, NOT RAISED** — the whole of the move is `dopl_ontology`'s description, which absorbed a new FENCE sentence and came out two characters SMALLER. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **47,319 → 48,412 (2026-09-08, +1,093): THE KNOWLEDGE-SCOPES WAVE, AND THE RISE IS EXACTLY THE ONE SCHEMA'S** — see `SCHEMA_CEILINGS.dopl_agent` for the measurement and for the two cheaper shapes that were measured and refused. ⚠ **THIS CEILING CANNOT SEE WHAT THE CHARACTERS BUY, WHICH IS WHY THE RISE IS RECORDED RATHER THAN ABSORBED**: it counts what a connection costs before it has done anything, and what a folder scope saves is per LAUNCH — a role that used to point an agent at a whole base now points it at one folder, so the agent's `get_tree` of a 200-entry base becomes a `list_dir` of six. One such launch pays the rise back many times over, and the surface it saves on is a ledger this gate does not keep. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ **47,319 → 47,299 (2026-09-07, −20): THE CREDIT REFUSAL STOPPED CLAIMING A WORKSPACE POOL.** `tool-errors.ts › CREDITS_EXHAUSTED.meaning` went from "this workspace is out of credits …" to "you are out of credits …" (−10 chars, served twice — `dopl_status` and `dopl_search`), because an allocation is per PERSON and never pooled (Samuel, 2026-09-07): a personal wallet, or one member's seat. The old sentence sent a member to an admin who had nothing to refill. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** // ⚠ 47,464 → 47,319 (2026-09-06, −145): the banked half of the doctrine move, after the mandatory third call shape on dopl_channel (+32) was paid back out of it. // ⚠ **46,846 → 47,464 (2026-09-03, +618): THE HEADINGS-AS-ADDRESSES WAVE MADE THIS SURFACE BIGGER, AND SAID SO.** `dopl_kb`'s schema +569 (`section`, `offset`, one op name) and its description +49, against a trim of 233 inside the same description. ⚠ **THIS IS THE ONE CEILING THAT CANNOT SEE THE SAVING, WHICH IS WHY THE RISE IS RECORDED RATHER THAN ABSORBED.** It counts what a connection costs before it has done anything; what these characters buy is per READ — an agent that outlines a 2,559-char entry and pulls one section spends 1,158 (319 outline + 839 section) where it used to spend 2,760, and the ratio only improves with the document. Six such reads pay the rise back. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** ⚠ **46,851 → 46,846 (2026-09-03): THE HOLD-NOT-POLL WAVE MADE THIS SURFACE SMALLER.** The `section=` enum gained `waiting` (+10) and that field's `.describe()` paid for it (−15); the briefing's new WAIT sentence cost nothing here because it was paid for by trimming the contract around it. What the ten characters buy is a caller that HOLDS instead of re-reading on a timer — and a timed poll re-sends that caller's whole context per tick, a cost this ceiling cannot see. Per-call, the hold results fell ~1,400 EACH (`channel-result-budget.test.ts`). // ⚠ **54,934 → 51,996 ON 2026-09-02 (SLICE A14), A FALL OF 2,938.** Re-derived whole through the real `listTools()`, and it was two moves pulling opposite ways: DESCRIPTIONS −4,466 (every one is RENDERED now, and a description carries nothing its own arg descriptions already say), SCHEMAS +1,526 — all of it the response-size knobs, the one rise here that pays for itself per CALL rather than per connection — and the briefing +2, which bought the per-connection IDENTITY block that deletes a round trip from every run. ⚠ **NEVER QUOTE THIS NUMBER — re-derive it.** Every figure a doc has carried about this surface has gone stale inside a day (F-422). ⚠ **49,790 → 47,021 (B13), A FALL OF 2,769, AND IT IS ALMOST ALL TOOL COUNT:** three meta tools became one (`dopl_home`, `current_workspace`, `list_workspaces` → `dopl_workspaces`), the injected `workspace` contract lost 5 chars × 9 tools, and the briefing fell 56. **B15 then took the copy ops, the `shelf` axis and its two labels out and put the grant op and its three args in.**
...
const DOCTRINE_CEILING = 9_979; // ⚠ **9,905 → 9,979 (+74), AND THE FALL THAT LICENSES IT:** 2026-09-06, the artifacts wave: `channel-schema.ts`'s pushed `.describe()` prose fell 762 (9,445 → 8,683 measured) as standing contract prose moved into the PULLED doctrine; the doctrine rose 74 to pay for it. 762 down, 74 up — a 688-char net fall on the surface every client is handed on every connection. Every number below is a MEASUREMENT from a real gate run, not a hand count.
