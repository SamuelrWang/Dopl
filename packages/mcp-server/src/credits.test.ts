...
vi.mock("@modelcontextprotocol/sdk/server/mcp.js", () => ({
  McpServer: class {
    server = { setRequestHandler() {} }; // `unlistTools` wraps it; a double installs nothing.
    // ⚠ THE MCP RESOURCE SEAM (2026-09-02). `createServer` publishes `dopl://doctrine/channels`
    // through `registerResource` (`resources.ts`), so a double without this method throws before
    // a single tool is registered. ⚠ IT IS A NO-OP HERE ON PURPOSE — these suites assert over
    // TOOLS; the resource's content is pinned in `channel-doctrine.test.ts`, its registration in
    // `server.test.ts`.
    registerResource() {}
