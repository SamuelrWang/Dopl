...
import { isConcise, type ResponseFormat } from "./tools/response-size.js";
 ...
 */
export function requestedFormat(args: unknown): ResponseFormat | undefined {
  const value = (args as { response_format?: unknown } | null)?.response_format;
...
export async function appendDoplStatus(
   ...
   */
  binding?: EffectiveWorkspace | null,
): Promise<ToolResponse> {
  const res = response;
  if (res.isError) return res;
  if (isConcise(format)) return note ? appendFooter(res, `\n\n_${note}_`) : res;
...
function bindingLine(
  ...
  if (effective.source !== "per-call arg") return effective.source;
  if (binding === undefined || binding?.id === effective.id) return effective.source;
...
function appendFooter(res: ToolResponse, footer: string): ToolResponse {
