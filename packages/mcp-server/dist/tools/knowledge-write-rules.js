...
exports.excerptRefusal = excerptRefusal;
...
const CROSS_REF_RE = /\b(see the|see our|documented elsewhere|not reproduced here|follows the|described elsewhere|covered elsewhere)\b/i;
/**
 ...
 * writing.
 */
function crossRefNudge(body) {
    if (!CROSS_REF_RE.test(body))
        return null;
    if (NAMES_A_TARGET_RE.test(body))
        return null;
...
const SUPERSEDES_RE = /\b(supersed(e|es|ed|ing)|replaces? (?:the |this )?\w+ in full|replaces the|deprecat(e|es|ed)|no longer applies|obsoletes?)\b/i;
 ...
 */
function supersessionNudge(body) {
    if (!SUPERSEDES_RE.test(body))
        return null;
    if (SUPERSEDES_RE.test(firstLine(body)))
        return null;
