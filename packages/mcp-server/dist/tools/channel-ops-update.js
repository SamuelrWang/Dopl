...
function descriptionLine(topic) {
    const safe = topic ? (0, narration_1.neutralizeInline)(topic) : null;
...
function renderCard(card) {
    ...
    for (const row of rows) {
        ...
        lines.push(`- ${(0, narration_1.inlineOr)(row.label, "`(unlabelled)`")}: ${(0, narration_1.inlineOr)(row.value, "`(empty)`")} (id: \`${row.id}\`)`);
    }
    return lines;
}
/**
...
function toCard(arg) {
    ...
    for (const key of arg.hidden ?? []) {
        if (!BUILT_IN_KEYS.includes(key)) {
            return (0, respond_1.err)(`Refused before sending: \`${(0, narration_1.inlineOr)(key, "(unreadable)")}\` is not a built-in info row, so nothing was changed. The only rows that can be hidden are: ${BUILT_IN_KEYS.join(", ")}. To remove a CUSTOM row, leave it out of \`rows\` — the card is replaced whole.`);
        }
        hidden.push(key);
    }
    const rows = (arg.rows ?? []).map((row) => ({
        ...
        id: row.id ?? (0, node_crypto_1.randomUUID)(),
        label: row.label,
        value: row.value ?? "",
    }));
    const seen = new Set();
    for (const row of rows) {
        if (seen.has(row.id)) {
            return (0, respond_1.err)(`Refused before sending: two info rows share the id \`${row.id}\`, so nothing was changed. Ids must be unique within a card — omit \`id\` on a NEW row and one is minted for you; pass it only to EDIT the row that already has it.`);
        }
        seen.add(row.id);
    }
    return { hidden, rows };
...
function headerFacts(c) {
    const description = c.topic ? (0, narration_1.inlineOr)(c.topic, "`(empty)`") : "`(none)`";
    return `id=\`${c.id}\` · name=${(0, narration_1.inlineOr)(c.name, narration_1.NO_NAME)} · description=${description} · updated=${c.updatedAt}`;
}
/**
 ...
 */
async function opUpdate(client, ref, arg) {
    const { card } = arg;
    // Refused before the resolve: an empty name is a 400 the route explains worse.
    if (arg.name !== undefined && arg.name.trim() === "") {
        return (0, respond_1.err)(`Refused before sending: a channel name cannot be empty, so nothing was changed. Pass the new name, or omit \`name\` to keep it.`);
    }
    ...
    if (arg.description !== undefined)
        patch.topic = arg.description;
    if (card !== undefined) {
        const built = toCard(card);
        if ("isError" in built && built.isError)
            return built;
        patch.infoCard = built;
    }
    let updated;
    try {
    ...
    const lines = [
        `Updated **${(0, narration_1.inlineOr)(updated.name, narration_1.NO_NAME)}**: ${Object.keys(patch).map((k) => (k === "topic" ? "description" : k === "infoCard" ? "info card" : k)).join(", ")}. ${headerFacts(updated)}`,
    ];
    if (patch.infoCard !== undefined) {
        lines.push(...renderCard(updated.infoCard ?? patch.infoCard), "", `⚠ Everyone in this channel sees this card. It changes no permission, no roster and no fact — hiding the Email row hides a ROW, it does not clear anybody's address.`);
    }
