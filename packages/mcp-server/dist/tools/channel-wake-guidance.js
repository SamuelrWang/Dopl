...
exports.WAITING_LINE_MAX_CHARS = void 0;
exports.waitingLine = waitingLine;
exports.channelHoldCall = channelHoldCall;
exports.workspaceHoldCall = workspaceHoldCall;
exports.holdFact = holdFact;
 ...
 */
function waitingLine(call, cursor) {
    return `cursor=${cursor} · hold, never poll: ${call} · ${channel_doctrine_1.DOCTRINE_URI} › Waiting`;
}
/**
 ...
 */
function holdTimedOutLines(ref, since, runtime) {
    if ((0, identity_1.isDesktopRuntime)(runtime)) {
        return [
    ...
    return [waitingLine(channelHoldCall(ref, since), since)];
}
/**
 ...
 */
function workspaceHoldTimedOutLines(since, runtime) {
    if ((0, identity_1.isDesktopRuntime)(runtime)) {
        return [
            `${DESKTOP_OBSERVED} Do NOT re-arm — end your reply here and the next message will be fed to you. If nothing ever arrives, report that to your operator rather than waiting again.`,
        ];
    }
    return [waitingLine(workspaceHoldCall(since), since)];
}
/** The HOLD returned messages: advance the cursor, then re-arm — or don't. */
function holdArrivedLines(ref, lastSeq, runtime) {
    if ((0, identity_1.isDesktopRuntime)(runtime)) {
        return [
