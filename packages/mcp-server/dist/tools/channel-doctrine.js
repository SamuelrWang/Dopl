...
exports.CHANNEL_DOCTRINE = exports.CHANNEL_LAW = exports.DOCTRINE_POINTER = exports.DOCTRINE_URI = void 0;
 ...
 */
const OWN_AGENTS = `YOUR OWN AGENTS. op="launch_agent" ASKS your operator's own machine to start one; it never reaches another member's, and there is no argument that could name one. SEND A \`goal\` if you want it to do anything — a launch WITH one runs that goal as its FIRST INSTRUCTION, and a launch WITHOUT one registers an agent that stands by until something names it, which costs you a second call.
THE HANDLE IS \`@agent-<id>\`, and that \`@agent-<id>\` form is the only one that means anything outside your operator's own machine — it is what the Dopl app writes and tints. A friendly NAME your operator gives an agent (op="rename_agent") is stored on that ONE machine, reaches no server, and is never addressable from here — so "read_sessions" keeps printing the id after a rename, and that is correct rather than a stale read.
