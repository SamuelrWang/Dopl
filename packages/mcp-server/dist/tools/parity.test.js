...
function sourceOf(file) {
    return (0, node_fs_1.readFileSync)(node_path_1.default.join(SRC_DIR, "tools", file), "utf8");
}
function opEnum(t) {
    ...
    (0, vitest_1.it)("every declared schema param is referenced in the tool's source", () => {
        for (const tool of TOOLS) {
            ...
            for (const key of Object.keys(tool.schema)) {
                ...
                (0, vitest_1.expect)(referenced, `${tool.name} declares schema param "${key}" that is never referenced in ${tool.sourceFile} (described-but-dead param)`).toBe(true);
            }
        }
    });
    (0, vitest_1.it)("every param the handler reads (args.X) is a declared schema param", () => {
        ...
        for (const [file, allowed] of keysByFile) {
            ...
            for (const id of accessed) {
                (0, vitest_1.expect)(allowed.has(id), `${file} handler reads args.${id} but no tool in that file declares "${id}" as a schema param (get_tree entry_limit bug class)`).toBe(true);
            }
        }
    });
});
