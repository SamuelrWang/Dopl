...
exports.ACCOUNT_SCOPE = exports.MAX_TRACKED_SUBJECTS = exports.POLL_STRIKE_WINDOW_MS = exports.POLL_STRIKE_LIMIT = void 0;
exports.pollSubject = pollSubject;
exports.noteHold = noteHold;
exports.notePollingRead = notePollingRead;
exports.pollingDetectedLine = pollingDetectedLine;
exports.resetPollDetectorForTests = resetPollDetectorForTests;
...
function keyOf(subject, scope, since) {
...
function sweep(now) {
    for (const [k, v] of tracked) {
        if (now - v.seen > exports.POLL_STRIKE_WINDOW_MS)
            tracked.delete(k);
    }
    if (tracked.size <= exports.MAX_TRACKED_SUBJECTS)
        return;
    const oldest = [...tracked.entries()]
        .sort((a, b) => a[1].seen - b[1].seen)
        .slice(0, tracked.size - exports.MAX_TRACKED_SUBJECTS);
 ...
 */
function noteHold(subject, scope) {
    if (subject === null)
        return;
 ...
 */
function notePollingRead(subject, scope, since, now = Date.now()) {
    if (subject === null)
        return false;
    sweep(now);
    const key = keyOf(subject, scope, since);
    const entry = tracked.get(key) ?? { at: [], seen: now };
    entry.at = entry.at.filter((t) => now - t < exports.POLL_STRIKE_WINDOW_MS);
    entry.at.push(now);
    entry.seen = now;
    tracked.set(key, entry);
    return entry.at.length >= exports.POLL_STRIKE_LIMIT;
}
/**
 ...
 */
function pollingDetectedLine(retry, cursor) {
 ...
 */
function resetPollDetectorForTests() {
    tracked.clear();
}
