...
exports.RETIRED_OP_NAMES = exports.RETIRED_OPS = void 0;
exports.isRetiredOp = isRetiredOp;
exports.retiredRedirect = retiredRedirect;
 ...
 */
exports.RETIRED_OPS = {
    post: 'retired: use send — dopl_channel(op="send", channel=…, body=…, to=…).',
    milestone: 'retired: use send(kind="milestone") — same body, `thread` required, still one line.',
    escalate: 'retired: use send(kind="decision") — `summary` is the question, `body` the context, `options` and `recommendation` unchanged.',
    ping: "retired: use send(to=…) — a directed send IS the delivery record, and its `delivery=` is the ack the ping row used to be.",
    pings: "retired: use read — a directed send lands in the transcript, so there is no second inbox to page.",
    create_thread: 'retired: use send(thread="new", to=…, summary=<the title>) — the new thread id comes back on the result.',
    list: 'retired: use rooms(action="list").',
    open: 'retired: use rooms(action="open", name=…), or rooms(action="open", to=<member>) for a 1:1.',
    invite: 'retired: use rooms(action="invite", channel=…, to=…).',
    members: 'retired: use rooms(action="members", channel=…).',
    list_threads: 'retired: use rooms(action="threads", channel=…).',
    set_thread_mode: 'retired: use rooms(action="thread_mode", channel=…, thread=…, mode=…).',
    update: 'retired: use rooms(action="update", channel=…, info_card=…).',
    help: 'retired: use rooms(action="help", section=…).',
    await: "retired: use read(since=…, wait_ms=…) — the hold is a knob on the read, not an op of its own.",
    launch_agent: 'retired: use manage(action="launch", channel=…).',
    end_agent: 'retired: use manage(action="end", to=<the agent handle>).',
    rename_agent: 'retired: use manage(action="rename", to=<the agent handle>, name=…).',
    set_agent_mode: 'retired: use manage(action="posture", to=<the agent handle>, posture=…).',
    direct_agent: 'retired: use manage(action="direct", to=<the agent handle>, body=…).',
    read_directions: "retired: use status — it reads the whole state machine.",
    read_sessions: "retired: use status — it reads the whole state machine.",
};
/**
 ...
 */
exports.RETIRED_OP_NAMES = Object.keys(exports.RETIRED_OPS);
/** True for a name that parses only to be redirected. */
function isRetiredOp(op) {
    return Object.prototype.hasOwnProperty.call(exports.RETIRED_OPS, op);
}
/**
 ...
 */
function retiredRedirect(op) {
    return (0, respond_1.ok)(`dopl_channel op="${op}" ${exports.RETIRED_OPS[op]}`);
}
