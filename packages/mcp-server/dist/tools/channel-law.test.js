 ...
 */
(0, vitest_1.describe)("no SHIPPED STRING in the channel tool teaches removed vocabulary", () => {
    ...
    const HERE = node_path_1.default.resolve(process.cwd(), "src", "tools");
    /** Every non-test `channel-*.ts` module — the tool's whole authored surface. */
    const sources = (0, node_fs_1.readdirSync)(HERE)
        .filter((f) => (f.startsWith("channel-") || f === "channel.ts") &&
        f.endsWith(".ts") &&
        !f.endsWith(".test.ts"))
        .sort();
    /** Literal TEXT only — comments are not literals, so they are never scanned. */
    function shippedStrings(file) {
        const source = typescript_1.default.createSourceFile(file, (0, node_fs_1.readFileSync)(node_path_1.default.join(HERE, file), "utf8"), typescript_1.default.ScriptTarget.Latest, true);
        const out = [];
        const walk = (node) => {
            if (typescript_1.default.isStringLiteral(node) ||
                typescript_1.default.isNoSubstitutionTemplateLiteral(node) ||
    ...
    (0, vitest_1.it)("finds the modules at all (a scan over nothing is not a guard)", () => {
        (0, vitest_1.expect)(sources.length).toBeGreaterThan(10);
        (0, vitest_1.expect)(sources).toContain("channel-post-linkage.ts");
        (0, vitest_1.expect)(sources).toContain("channel-ops-write.ts");
        (0, vitest_1.expect)(sources).toContain("channel-render-threads.ts");
        // …and it is really reading strings out of them.
        (0, vitest_1.expect)(shippedStrings("channel-post-linkage.ts").length).toBeGreaterThan(3);
    });
    (0, vitest_1.it)("the scan can SEE the defect it was written for", () => {
        ...
        const shipped = 'to_agent="<handle>" starts that agent, to="<member>" triggers their machine.';
        const hit = REMOVED_VOCABULARY.filter(([, re]) => re.test(shipped));
        (0, vitest_1.expect)(hit.map(([label]) => label)).toEqual(["to_agent / to_agents"]);
    });
    for (const file of sources) {
        (0, vitest_1.it)(`${file} ships no removed vocabulary`, () => {
            const found = [];
            ...
            (0, vitest_1.expect)(found, `a shipped string teaches a surface that no longer exists:\n${found.join("\n")}`).toEqual([]);
        });
    }
});
