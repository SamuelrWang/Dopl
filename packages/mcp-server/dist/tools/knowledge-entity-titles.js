 ...
 * (`supabase/migrations/20261014120000_backfill_entity_escaped_knowledge_titles.sql`).
...
function looksEntityEscaped(label) {
 ...
 */
function titleDecodedNote(requested, stored) {
    if (requested === undefined)
        return "";
    if (stored === requested || !looksEntityEscaped(requested))
        return "";
    return ` HTML entities in the title were decoded — address it as ${(0, narration_1.inlineOr)(stored, narration_1.NO_NAME)}.`;
}
/**
 ...
 */
function escapedTitleLine(sample, count = 1) {
    const where = count === 1
