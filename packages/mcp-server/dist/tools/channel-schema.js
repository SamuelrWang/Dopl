...
exports.SCHEMA_MAX_CHARS = 9_155; // Re-derive through `channel-schema-budget.test.ts`, never quote.
exports.PARAM_DESCRIPTION_MAX_CHARS = 400; // Per-`.describe()` cap.
exports.CHANNEL_INPUT_SHAPE = {
    ...
    channel: zod_1.z
        .string()
        .optional()
        .describe('Channel slug or id. Required except on op="rooms" action="list" / "open" / "help"; omitting it WIDENS op="read" and op="status".'),
    ...
    kind: zod_1.z
        ...
        .optional()
        .describe('op="send" (optional, default "message"): "record" takes no `to`; "milestone" marks a step on a thread; "decision" is a card answered in one press.'),
    thread: zod_1.z
        .string()
        .optional()
        .describe(
    ...
    artifact: zod_1.z
        .string()
        .optional()
        .describe('op="artifact" (required on "add", "remove", "dissolve"): its id, as action="create" returned it.'),
    ...
    messages: zod_1.z
        ...
        .max(200)
        .optional()
        .describe('op="artifact" (required on "create", "add" and "remove"): the message `seq`s — the whole set on "create", exactly ONE on "add" and "remove".'),
    ...
    limit: zod_1.z.coerce
        ...
        .max(200)
        .optional()
        .describe('op="read" (optional): max messages to return — with no `since` that is the NEWEST page.'),
    ...
    wait_ms: zod_1.z.coerce
        ...
        .optional()
        .describe('Optional HOLD. op="read": long-poll for messages after `since` instead of returning a page. op="manage" (<=30000): how long to hold for your operator\'s desktop to answer.'),
