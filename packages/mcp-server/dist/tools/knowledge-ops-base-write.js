...
directory) {
    ...
    const verdict = await (0, confirm_token_1.confirmGate)(client, {
        ...
        precheck: async () => {
            try {
                ...
                const ceiling = (0, knowledge_write_shared_1.agentCreateForbidden)(e);
                if (ceiling)
    ...
    const dup = await (0, duplicate_name_1.duplicateNameNoteFor)(base, () => client.listKbBases(), "knowledge base", true);
    ...
    return (0, respond_1.ok)(`Created knowledge base ${(0, narration_1.inlineOr)(base.name, narration_1.NO_NAME)} (slug: \`${base.slug}\`). ${visNote}${dup}`);
}
async function opUpdateBase(client, ref, name, description, slug) {
    ...
    if ((0, channel_shared_1.isErr)(base))
        return base;
    const updated = await (0, knowledge_write_shared_1.writeOr)(() => client.updateKbBase(base.id, { name, description, slug }), knowledge_shared_1.updateBaseValidationError);
...
async function opSetVisibility(client, callerUserId, ref, visibility, confirmToken) {
    ...
    const updated = await (0, knowledge_write_shared_1.writeOr)(() => client.updateKbBase(base.id, {
        visibility: "public",
