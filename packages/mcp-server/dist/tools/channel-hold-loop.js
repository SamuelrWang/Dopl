...
exports.runHold = runHold;
exports.wasCutShort = wasCutShort;
...
async function runHold(poll, since, timeoutMs, runtime, selfSessionId) {
    ...
    const budgetMs = (0, channel_hold_budget_1.holdMsFor)(timeoutMs, runtime);
    ...
    for (let attempt = 0; attempt < channel_hold_budget_1.HOLD_MAX_POLLS; attempt++) {
        ...
        if (attempt > 0 && remaining < channel_hold_budget_1.HOLD_MIN_POLL_MS)
            break;
        let result;
        try {
            result = await poll({
                ...
                timeoutMs: Math.max(1, Math.min(channel_hold_budget_1.HOLD_POLL_MS, remaining)),
            });
        }
        catch (e) {
            if (attempt === 0)
 ...
 */
function wasCutShort(elapsedMs, budgetMs) {
    return elapsedMs < Math.min(channel_hold_budget_1.HOLD_SHORT_MS, budgetMs / 2);
}
