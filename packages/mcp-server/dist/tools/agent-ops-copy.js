...
async function opCopy(client, directory, ref, toWorkspace) {
    ...
    if ((0, agent_shared_js_1.isErr)(found))
        return found;
    const onto = (0, copy_target_js_1.sameWorkspaceRefusal)(target, found.workspaceId, "agent template", `dopl_agent(op="create")`);
    if (onto)
        return onto;
    ...
    let created;
    try {
        created = await client_1.workspaceContext.run(target.id, () => client.createAgentTemplate({
            name: source.name,
        ...
        const mapped = (0, agent_shared_js_1.sharedCredentialPrivateDenied)(e);
        if (mapped)
            return mapped;
        throw e;
    }
    ...
    const lines = [
        `Copied the agent template ${(0, narration_js_1.inlineOr)(source.name, agent_shared_js_1.NO_NAME)} into ${(0, copy_target_js_1.workspaceLabel)(target)} as a NEW template (id: \`${created.id}\`). It is PRIVATE to you there — a copy is never published into the target — and it is a STRANGER to the original: editing one never touches the other.`,
    ...
    if (source.knowledgeBases.length > 0) {
        ...
        lines.push(`⚠ ${n} attached knowledge base${n === 1 ? " was" : "s were"} NOT carried, and the copy has none: a base id belongs to the workspace it was created in, so attaching it there would be a cross-tenancy reference this op refuses to make (and the target would reject it). Copy each across with dopl_kb(op="copy_base", base=…, to_workspace="${handle}"), then attach the NEW ids with dopl_agent(op="update", template="${created.id}", knowledge_bases=[…], workspace="${handle}").`);
    }
