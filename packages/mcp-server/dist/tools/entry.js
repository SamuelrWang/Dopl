...
exports.registerEntryTools = registerEntryTools;
...
function registerEntryTools(register, client) {
    register("dopl_entry", DESCRIPTION, {
        op: zod_1.z
        ...
        title: zod_1.z.string().optional().describe("op=update: new title."),
        summary: zod_1.z.string().optional().describe("op=update: new summary."),
        use_case: zod_1.z.string().optional().describe("op=update: new use case category."),
        complexity: zod_1.z
        ...
        switch (args.op) {
            case "update": {
                const miss = (0, respond_1.missingParams)("update", args, ["entry"]);
                if (miss)
                    return miss;
                return opUpdate(client, args);
            }
            case "check_updates": {
                const miss = (0, respond_1.missingParams)("check_updates", args, ["entry"]);
                if (miss)
                    return miss;
                return opCheckUpdates(client, args.entry);
            }
            case "check_cluster_updates": {
                const miss = (0, respond_1.missingParams)("check_cluster_updates", args, ["slug"]);
                if (miss)
                    return miss;
                return opCheckClusterUpdates(client, args.slug);
            }
        }
    });
}
async function opUpdate(client, params) {
    ...
    return (0, respond_1.ok)(`Updated entry **${label}**.`);
}
async function opCheckUpdates(client, entry) {
    ...
    if (result.has_updates === null) {
        return (0, respond_1.ok)(`**${label}**: ${result.reason || "Update checking not available."}`);
    }
    if (result.has_updates) {
        return (0, respond_1.ok)(`**${label}** (${result.repo}): Updates available.\nRepo last pushed ${result.days_since_push} day(s) ago. You ingested it ${result.days_since_ingestion} day(s) ago.\nConsider re-ingesting with \`dopl_ingest(op='url')\`.`);
    }
    return (0, respond_1.ok)(`**${label}** (${result.repo}): No updates since ingestion (${result.days_since_ingestion} day(s) ago).`);
}
async function opCheckClusterUpdates(client, slug) {
