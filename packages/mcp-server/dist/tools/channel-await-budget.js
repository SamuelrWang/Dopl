...
exports.AWAIT_SHORT_HOLD_MS = exports.AWAIT_MAX_POLLS = exports.AWAIT_MIN_POLL_MS = exports.AWAIT_POLL_MS = exports.AWAIT_HOLD_MARGIN_MS = exports.MCP_ROUTE_MAX_DURATION_MS = exports.AWAIT_HOLD_EXTERNAL_DEFAULT_MS = exports.AWAIT_HOLD_EXTERNAL_MARGIN_MS = exports.EXTERNAL_CLIENT_ABORT_MS = exports.AWAIT_HOLD_DEFAULT_MS = exports.AWAIT_HOLD_CAP_MS = void 0;
exports.resolveAwaitHoldMs = resolveAwaitHoldMs;
exports.resolveAwaitHoldCeilingMs = resolveAwaitHoldCeilingMs;
exports.awaitHoldMs = awaitHoldMs;
 ...
 */
exports.AWAIT_HOLD_CAP_MS = 230_000;
/**
 ...
 */
exports.AWAIT_HOLD_DEFAULT_MS = 215_000;
/**
 ...
 */
exports.EXTERNAL_CLIENT_ABORT_MS = 60_000;
/**
 ...
 */
exports.AWAIT_HOLD_EXTERNAL_MARGIN_MS = 15_000;
/**
 ...
 */
exports.AWAIT_HOLD_EXTERNAL_DEFAULT_MS = exports.EXTERNAL_CLIENT_ABORT_MS - exports.AWAIT_HOLD_EXTERNAL_MARGIN_MS;
/**
 ...
 */
exports.MCP_ROUTE_MAX_DURATION_MS = 300_000;
/**
 ...
 */
exports.AWAIT_HOLD_MARGIN_MS = 60_000;
/**
...
function resolveAwaitHoldMs(raw) {
    const text = (raw ?? "").trim();
    if (!/^\d+$/.test(text))
        return exports.AWAIT_HOLD_DEFAULT_MS;
    const ms = Number.parseInt(text, 10);
    if (!Number.isFinite(ms))
        return exports.AWAIT_HOLD_DEFAULT_MS;
    return Math.min(exports.AWAIT_HOLD_CAP_MS, Math.max(AWAIT_HOLD_FLOOR_MS, ms));
}
/**
 ...
 * route around it.
 */
function resolveAwaitHoldCeilingMs(raw) {
    return /^\d+$/.test((raw ?? "").trim())
        ? resolveAwaitHoldMs(raw)
        : exports.AWAIT_HOLD_CAP_MS;
}
// ── THE ASSEMBLED HOLD, resolved once per server process ─────────────────
//
// ⚠ ONE STATEMENT, SHARED BY BOTH LANES. `channel-ops-await.ts` and
// `channel-ops-await-workspace.ts` are siblings by design (their RESULT
...
const ENV_HOLD_MS = resolveAwaitHoldMs(process.env.DOPL_AWAIT_HOLD_MS);
const ENV_CEILING_MS = resolveAwaitHoldCeilingMs(process.env.DOPL_AWAIT_HOLD_MS);
/** Don't re-issue an inner poll for a sliver of the remaining budget. */
exports.AWAIT_MIN_POLL_MS = 1_000;
/**
 ...
 */
exports.AWAIT_MAX_POLLS = Math.ceil(ENV_CEILING_MS / exports.AWAIT_POLL_MS) + 2;
/**
 ...
 * about getting one.
 */
exports.AWAIT_SHORT_HOLD_MS = 60_000;
/**
 ...
 */
function awaitHoldMs(timeoutMs, runtime) {
    if (timeoutMs !== undefined)
        return Math.min(timeoutMs, ENV_CEILING_MS);
    return (0, identity_1.isDesktopRuntime)(runtime)
        ? ENV_HOLD_MS
        : Math.min(ENV_HOLD_MS, exports.AWAIT_HOLD_EXTERNAL_DEFAULT_MS);
}
