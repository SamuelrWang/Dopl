 ...
 * label. ⚠ **MERGE NOTE (2026-09-18):** the refusal wrappers land on a sibling
 * branch of this same wave and STILL DERIVE THESE TWO QUESTIONS THEMSELVES.
 * Re-point `excerptRefusal` / `unsectionedRefusal` at {@link kbSummaryFault} and
 * {@link kbBodyIsUnsectioned} at the merge, keeping their sentences — otherwise
 * there are two predicates again and this pin guards neither.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.KB_ANY_HEADING_RE = void 0;
exports.foldWords = foldWords;
exports.kbSummaryFault = kbSummaryFault;
exports.kbBodyIsUnsectioned = kbBodyIsUnsectioned;
const knowledge_sections_js_1 = require("./knowledge-sections.js");
 ...
 * no legitimate case.
 */
function kbSummaryFault(excerpt, title) {
    const folded = excerpt ? foldWords(excerpt) : "";
    if (folded === "")
        return "missing";
    if (!folded.includes(" "))
        return "one_word";
    if (folded === foldWords(title))
        return "restates_title";
    return null;
}
/**
 ...
 */
exports.KB_ANY_HEADING_RE = /^[ \t]{0,3}#{1,3}[ \t]+\S/m;
/**
 ...
 * heading.
 */
function kbBodyIsUnsectioned(body, section) {
    if (section !== undefined)
        return false;
    if (body.length <= knowledge_sections_js_1.KB_SECTION_NUDGE_CHARS)
        return false;
    return !exports.KB_ANY_HEADING_RE.test(body);
}
