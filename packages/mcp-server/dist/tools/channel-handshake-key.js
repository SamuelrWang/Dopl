...
exports.HANDSHAKE_PREFIX = void 0;
exports.normalizeHandshakeKey = normalizeHandshakeKey;
exports.malformedHandshakeKey = malformedHandshakeKey;
exports.rewrittenHandshakeKeyNote = rewrittenHandshakeKeyNote;
/** The prefix the protocol's derived key starts with. Mirrors `HANDSHAKE_PREFIX`. */
exports.HANDSHAKE_PREFIX = "thread-open-";
/**
 ...
 */
function normalizeHandshakeKey(clientMsgId, channelId) {
    if (!clientMsgId || !clientMsgId.startsWith(exports.HANDSHAKE_PREFIX)) {
        return { status: "passthrough" };
    }
    const rest = clientMsgId.slice(exports.HANDSHAKE_PREFIX.length);
    ...
    if (seq < 1 || !Number.isSafeInteger(seq))
        return { status: "malformed" };
    const key = `${exports.HANDSHAKE_PREFIX}${channelId}-${seq}`;
 ...
 */
function malformedHandshakeKey(clientMsgId, channelId) {
    return `That \`client_msg_id\` starts with "${exports.HANDSHAKE_PREFIX}" but is not a handshake key — no thread was opened. The form is client_msg_id="${exports.HANDSHAKE_PREFIX}<channelId>-<seq>", and the \`<seq>\` is the seq of the MESSAGE THAT ASKED for this work, as shown on its line in "read" / "await" (\`**#42**\` means seq 42). Yours ends in no seq at all, so two agents racing to open this thread would mint two different keys and you would get two rooms for one job. Re-send it as client_msg_id="${exports.HANDSHAKE_PREFIX}${channelId}-<that seq>". If this thread has nothing to do with a two-agent handshake, use any other idempotency key — the "${exports.HANDSHAKE_PREFIX}" prefix is reserved for it.`;
}
/**
 ...
 */
function rewrittenHandshakeKeyNote(supplied, key) {
