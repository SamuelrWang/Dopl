...
function entryAddress(h) {
    const where = h.path
        ? `${h.baseSlug ? `base \`${h.baseSlug}\` · ` : ""}path ${(0, narration_1.inlineOr)(h.path, "`(unreadable path)`")} · `
        : "";
...
function registerSearchTool(register, client, directory, charge) {
    register("dopl_search", SEARCH_DESCRIPTION, SEARCH_SHAPE, async (args) => {
        ...
        for (const h of entryHits.slice(0, limit)) {
            lines.push(`- ${(0, narration_1.inlineOr)(h.title, narration_1.NO_NAME)} (${entryAddress(h)}) — ${snippet(h.snippet)}`);
        }
