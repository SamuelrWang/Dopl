...
const tool_group_files_1 = require("./tool-group-files");
...
const REGISTRARS = [
    { file: "cluster.ts", register: cluster_1.registerClusterTools },
    { file: "workflow.ts", register: workflow_1.registerWorkflowTools },
    { file: "knowledge.ts", register: knowledge_1.registerKnowledgeTools },
    { file: "skills.ts", register: skills_1.registerSkillTools },
    { file: "chats.ts", register: chats_1.registerChatTools },
    { file: "members.ts", register: members_1.registerMembersTool },
    { file: "map.ts", register: map_1.registerMapTool },
    { file: "search.ts", register: search_1.registerSearchTool },
    { file: "ontology.ts", register: ontology_1.registerOntologyTool },
    { file: "channel.ts", register: channel_1.registerChannelTool },
];
function captureTools() {
    const tools = [];
    const stub = {};
    for (const { file, register } of REGISTRARS) {
        const cap = ((name, description) => {
...
(0, vitest_1.describe)("no tool's headline claims completeness", () => {
    for (const t of TOOLS) {
        (0, vitest_1.it)(`${t.name}: the first sentence promises no census`, () => {
            ...
            (0, vitest_1.expect)(COMPLETENESS.test(opening), `${t.name} opens with a completeness claim: "${opening}"\n` +
                ...
                `the authoritative alternative instead.`).toBe(false);
        });
    }
});
(0, vitest_1.describe)("the two headlines that caused the incident stay corrected", () => {
    (0, vitest_1.it)("dopl_map does not say it returns every anything", () => {
        ...
        (0, vitest_1.expect)(COMPLETENESS.test(headline(d))).toBe(false);
        (0, vitest_1.expect)(d).not.toContain("manifest of the active workspace — every");
        ...
        (0, vitest_1.expect)(d).toContain('dopl_members(op="access_matrix")');
        (0, vitest_1.expect)(d.toLowerCase()).toContain("not an inventory");
    });
    (0, vitest_1.it)("dopl_search does not claim the whole workspace, and names what it skips", () => {
        const d = tool("dopl_search").description;
        (0, vitest_1.expect)(d).not.toContain("Search the whole workspace");
        // The archive is the domain an agent is most likely to assume is covered.
        (0, vitest_1.expect)(d).toContain("CHAT ARCHIVE");
        (0, vitest_1.expect)(d).toContain("dopl_chats");
    });
});
const LEDGER = [
    {
...
(0, vitest_1.describe)("every filtered read op discloses its filter in its own bullet", () => {
    for (const row of LEDGER) {
        (0, vitest_1.it)(`${row.tool}(op="${row.op}") — ${row.filter}`, () => {
            ...
            for (const phrase of row.discloses) {
                (0, vitest_1.expect)(text.includes(phrase), `${row.tool}(op="${row.op}") applies ${row.filter}, but its bullet ` +
                    `does not disclose it — expected to find ${JSON.stringify(phrase)} in:\n${text}`).toBe(true);
            }
        });
        (0, vitest_1.it)(`${row.tool}(op="${row.op}") — the filter it documents still exists in code`, () => {
            ...
            const source = (0, tool_group_files_1.toolGroupSource)(BY_NAME.get(row.tool).file);
            (0, vitest_1.expect)(source.includes(row.proof), `${row.tool}(op="${row.op}") documents ${row.filter}, but ` +
                `${JSON.stringify(row.proof)} is gone from its source. Either the ` +
                `filter moved (update \`proof\`) or it was removed — in which case ` +
                `the description now under-claims and must be corrected too.`).toBe(true);
        });
    }
});
