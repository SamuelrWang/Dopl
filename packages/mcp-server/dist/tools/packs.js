...
exports.registerPacksTools = registerPacksTools;
...
function registerPacksTools(register, client) {
    register("dopl_packs", DESCRIPTION, {
        op: zod_1.z.enum(["list", "list_files", "get_file"]).describe("Operation to perform."),
        pack: zod_1.z
            .string()
            .optional()
        ...
        switch (args.op) {
            ...
            case "list_files": {
                const miss = (0, respond_1.missingParams)("list_files", args, ["pack"]);
                if (miss)
                    return miss;
                return opListFiles(client, args.pack, args.category, args.limit);
            }
            case "get_file": {
                const miss = (0, respond_1.missingParams)("get_file", args, ["pack", "path"]);
                if (miss)
                    return miss;
                return opGetFile(client, args.pack, args.path);
            }
        }
    });
}
...
async function opListFiles(client, pack, category, limit) {
...
async function opGetFile(client, pack, path) {
