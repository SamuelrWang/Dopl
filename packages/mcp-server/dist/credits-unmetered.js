...
exports.NOTE_PREFIX = void 0;
exports.withUnmeteredScope = withUnmeteredScope;
exports.recordUnmetered = recordUnmetered;
exports.unmeteredNote = unmeteredNote;
exports.joinNotes = joinNotes;
exports.resetUnmeteredLogForTests = resetUnmeteredLogForTests;
...
function withUnmeteredScope(run) {
 ...
 */
function unmeteredNote() {
    const reasons = scope.getStore()?.reasons;
    if (!reasons || reasons.size === 0)
        return null;
    const ordered = ["consume_failed", "degraded"];
    const why = ordered
        .filter((r) => reasons.has(r))
        .map((r) => NOTE_TAIL[r])
        .join("; ");
    return `${exports.NOTE_PREFIX} this call was NOT charged — ${why}. Nothing is wrong with your call; tell your operator.`;
}
/** Join the footer's notes. ⚠ Both are optional and either may be the only one. */
function joinNotes(...notes) {
    const kept = notes.filter((n) => typeof n === "string" && n.length > 0);
...
function resetUnmeteredLogForTests() {
    logged.clear();
}
