...
exports.UNTRUSTED_DIRECTORY_NOTE = exports.UNNAMED_WORKSPACE = void 0;
...
function renderWorkspaceGuidance(directory, pin, directoryLoadFailed) {
    ...
    const pinName = pin ? (0, narration_js_1.inlineOr)(pin.name, exports.UNNAMED_WORKSPACE) : "";
...
- Coordinating work across rooms — what has moved, what is running, what is waiting on you -> dopl_status (ONE call: every channel you are in, in every workspace AND every home channel, with the \`workspace=\` handle for each, your own live agent sessions, and anything addressed to you that you have not answered). Pass \`since=<the highest seq you have processed anywhere>\` and it counts what is new from there. Start a check-in here rather than listing workspaces and then channels and then sessions.
