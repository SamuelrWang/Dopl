...
exports.skillAuthoringGuide = void 0;
const call_ref_js_1 = require("../call-ref.js");
const q = { quote: "'" };
const skillAuthoringGuide = () => `# Skill authoring framework
...
- **Reference material goes in knowledge bases, not the skill.** Long specs, lookup tables, schemas, transcripts, big examples — put them in a KB and link from the body as \`[label](dopl://kb/<slug>)\`. The agent loads the KB with \`${(0, call_ref_js_1.callRef)("kb.read_file", {}, q)}\` only when it needs it. The skill stays short and procedural.
...
- **Long reference docs** (API specs, schemas, lookup tables, big example sets, transcripts) → put them in a **knowledge base** and link from the body: \`For the full field list, see [Product specs](dopl://kb/product-specs).\` The agent loads it with \`${(0, call_ref_js_1.callRef)("kb.read_file", {}, q)}\` only when needed.
...
4. **Call \`${(0, call_ref_js_1.callRef)("skill.create", {}, q)}\`** with the metadata (and optional \`folder\`) + an optional initial body. The skill is now real and addressable.
5. **Write SKILL.md procedurally** via \`${(0, call_ref_js_1.callRef)("skill.write", {}, q)}\`. Follow the canonical section order. Keep it short — push reference material into a KB and link it.
...
`;
exports.skillAuthoringGuide = skillAuthoringGuide;
