 ...
 */
function withDoplStatus(handler, client) {
    return async (args) => {
...
function createServer(client, options = {}) {
    ...
    registerTool("read_cluster_knowledge_entry", "Read the full body of one entry inside a knowledge base attached to the given cluster. Returns 404 if the KB isn't attached or the entry doesn't exist. Find the (kb id, entry id) pair via `get_cluster` first.", {
        cluster_slug: zod_1.z.string().describe("Cluster slug"),
        knowledge_base_id: zod_1.z.string().describe("Knowledge base UUID"),
        entry_id: zod_1.z.string().describe("Knowledge entry UUID"),
    ...
    registerTool("prepare_ingest", "Start agent-driven ingestion. The server fetches the URL + follows links (no AI on their side), and returns the raw content + the exact prompts YOU run in your own Claude context. After running the prompts (content_type classify → manifest → README → agents.md → tags, plus vision for any images), call `submit_ingested_entry` with the artifacts. Costs 1 credit total (vs 7 for the legacy `ingest_url`). Prefer this over `ingest_url` for every URL the user wants to save.\n\nThe response includes a complete `instructions` field — follow it step-by-step. If the URL was already ingested, status=\"already_exists\" is returned and you can call `get_setup` directly.", {
        ...
        images: zod_1.z.array(zod_1.z.string()).optional().describe("Optional base64-encoded images to analyze (max 5, each ≤ 10MB)."),
    ...
    registerTool("submit_ingested_entry", "Finalize an agent-driven ingest. Submit the artifacts YOU generated after running the prompts returned by `prepare_ingest`. The server validates the shape, runs embeddings (the only AI call still on our side), persists the entries/tags/chunks rows, and marks status='complete'. Returns { entry_id, slug, title, use_case, complexity, content_type }.\n\nRequired fields come from the steps in the prepare response's `instructions`. Fields are:\n  - entry_id: from prepare response\n  - content_type: from step 1 (content_type classifier)\n  - source_type: from step 1\n  - manifest: entire JSON from step 3\n  - readme: markdown from step 4\n  - agents_md: markdown from step 5 (empty string for content_type='resource')\n  - tags: array from step 6 ({ tag_type, tag_value })\n  - image_analyses: array from step 7 (omit if no images)\n  - content_classification: JSON from step 2 (omit for non-setup/tutorial)\n\nOn success the entry is visible at `<host>/e/<slug>` and searchable by other agents.", {
        ...
        source_type: zod_1.z
            .string()
            .describe("Source type you classified in step 1 (e.g. 'blog_post', 'github_repo', 'news_article')."),
        manifest: zod_1.z
            .object({
            title: zod_1.z
                .string()
        ...
        readme: zod_1.z.string().min(1).describe("Markdown README from step 4."),
        agents_md: zod_1.z
            .string()
            .describe("Markdown agents.md (or key-insights / reference-guide) from step 5. Empty string if content_type='resource'."),
        tags: zod_1.z
        ...
        content_classification: zod_1.z
            ...
            .passthrough()
            .optional()
            .describe("Section classification from step 2. Only for setup/tutorial content_type."),
    }, async (input) => {
    ...
    // ── rename_chat ────────────────────────────────────────────────────
    registerTool("rename_chat", "Rename a chat panel on the user's canvas. Use this when the user asks to rename a chat, or when the initial auto-derived title (first user message truncated) no longer captures the conversation's topic. Purpose-named wrapper over the generic panels PATCH endpoint.", {
        panel_id: zod_1.z.string().describe("Chat panel ID (e.g. 'panel-3')"),
        title: zod_1.z.string().min(1).describe("New chat title"),
    ...
    // ── delete_cluster ─────────────────────────────────────────────────
    registerTool("delete_cluster", "Delete a cluster grouping. Individual entries REMAIN in the KB and on the user's canvas; only the cluster (and its brain + memories) is removed. Use when the user explicitly asks to drop a cluster. Irreversible — confirm intent if the user's phrasing is ambiguous.", {
        slug: zod_1.z.string().describe("Cluster slug from list_clusters"),
    }, async ({ slug }) => {
    ...
    registerTool("get_cluster_brain", "Read the current brain for a cluster — synthesized instructions plus numbered user memories. **Call this on first invocation per session of a cluster's skill** so the body you execute against reflects edits made via the web UI or other agents since the last `sync_skills`. Also call before `update_cluster_brain` (so surgical edits preserve existing text), before `update_cluster_memory` / `delete_cluster_memory` (to get memory IDs), and any time you want to know what durable knowledge already exists for a cluster before adding more.", {
        ...
        if (brain.instructions && brain.instructions.trim().length > 0) {
            ...
            sections.push("## Instructions");
            sections.push("");
            sections.push(`_This brain is empty._ Synthesize one: call \`get_skill_template\` for the canonical prompt, run synthesis in your context against \`get_cluster("${slug}")\`, then call \`update_cluster_brain("${slug}", <result>)\`.`);
            sections.push("");
        }
    ...
    registerTool("delete_cluster_memory", "Remove one specific memory from a cluster's brain by ID. Use when the user reverses a prior preference ('actually, ignore that', 'that's not true anymore') or explicitly says to drop a memory. Call `get_cluster_brain` first to get the memory ID. For REVISING an existing memory rather than deleting it, use `update_cluster_memory`.", {
        slug: zod_1.z.string().describe("Cluster slug"),
    ...
    // ── update_entry ───────────────────────────────────────────────────
    registerTool("update_entry", "Update an entry's editable metadata fields: title, summary, use_case, complexity. Use when the user wants to correct an entry's categorization or description. Does NOT edit README, agents.md, or the source URL — those are regenerated only via re-ingestion. Scoped to the entry's owner and admin callers.", {
        entry: zod_1.z.string().describe("Entry slug or UUID"),
    ...
    // ── check_entry_updates ─────────────────────────────────────────────
    registerTool("check_entry_updates", "Check whether ONE GitHub-sourced entry has new commits since it was ingested. Use when the user asks 'is this still current?' about a specific entry. Non-GitHub entries are skipped (returns a no-op status). For checking every entry in a cluster at once, use `check_cluster_updates` — much faster than looping.", {
        entry: zod_1.z.string().describe("Entry slug or UUID to check"),
    }, async ({ entry }) => {
    ...
    registerTool("add_entry_to_cluster", "Add a single entry to an existing cluster when the user wants to expand a cluster's membership. The brain is NOT auto-regenerated (initial synthesis runs once at cluster creation; later edits are preserved). If the new entry introduces a pattern the brain should reflect, follow up with `update_cluster_brain` — otherwise the addition is silent. To create a brand-new cluster from canvas entries, use `canvas_create_cluster` instead.", {
        slug: zod_1.z.string().describe("Cluster slug"),
        entry: zod_1.z.string().describe("Entry slug or UUID to add to the cluster"),
    ...
    // ── canvas_add_entry ──────────────────────────────────────────────
    registerTool("canvas_add_entry", "Add ONE known knowledge-base entry to the user's canvas by slug or UUID. Use this when you already have the exact entry the user wants to save. For search + add in one step (the common case when the user says 'save some good options for X to my canvas'), use `canvas_search_and_add` instead — it's a batch operation and much faster than looping through search hits.", {
        entry: zod_1.z.string().describe("Entry slug or UUID from search results or get_setup"),
    }, async ({ entry }) => {
    ...
    // ── canvas_remove_entry ───────────────────────────────────────────
    registerTool("canvas_remove_entry", "Take an entry off the user's canvas. The entry stays in the knowledge base — only the canvas panel is removed. Use when the user says 'clear this from my canvas', 'I don't need this anymore', or is pruning their workspace. For permanent deletion from the KB, use `delete_entry` instead (destructive, different scope).", {
        entry: zod_1.z.string().describe("Entry slug or UUID to remove from canvas"),
    }, async ({ entry }) => {
