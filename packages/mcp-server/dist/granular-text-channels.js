...
const channelDescription = zod_1.z.string().trim().max(exports.CHANNEL_DESCRIPTION_MAX_CHARS);
/** A decision is its own tool: the named refusal points there, and nothing is sent. */
const sendKind = zod_1.z.enum(["message", "milestone", "record"], {
    error: (issue) => issue.input === "decision"
...
exports.CHANNEL_TEXT = {
    dopl_list_channels: {
        description: "Channels you are in within one container, with slugs and ids. Start here before reading or posting; dopl_get_status covers every container at once.",
    },
    dopl_get_channel: {
        description: 'One channel\'s facts: action="status" your live agents in it (omit `channel` for every channel), "members" its roster, "threads" its threads.',
        params: { channel: 'Channel slug or id; required except for action="status".' },
    },
    dopl_read_channel: {
        description: "A channel's transcript, newest page first or only messages after `since`; omit `channel` to read every channel. With `wait_ms` it HOLDS until a message arrives: wait that way, never poll on a timer.",
        params: {
    ...
    dopl_create_channel: {
        description: "Open a channel: a named room (`name`) or a direct 1:1 (`to`), never both. Private, invite-only, unless public. A room outside any workspace is dopl_create_workspace.",
        params: {
    ...
    dopl_update_channel: {
        description: 'action="update": rename, describe or replace the info card; with none of them it READS the card. action="thread_mode": set a thread interactive or autonomous. Rename and describe need manager rights.',
        params: {
    ...
    dopl_launch_agent: {
        description: "Launch one of YOUR agents in a channel, blank or from an agent identity. Your operator's desktop starts it; posture is a request their machine only narrows.",
        params: {
            name: AGENT_NAME,
    ...
    dopl_manage_session: {
        description: 'One of YOUR running agents in a channel: action="direct" a private message, "rename", "posture" re-ask its freedom, "end" stop it.',
        params: {
    ...
    dopl_manage_artifact: {
        description: 'Fold a run of channel messages into one card (action="create"), add or remove one message, or dissolve it. Nothing is edited or deleted; dissolve restores the messages.',
        params: {
