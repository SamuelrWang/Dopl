...
exports.SESSION_PIN_TTL_MS = void 0;
exports.readSessionPin = readSessionPin;
exports.writeSessionPin = writeSessionPin;
exports.clearSessionPin = clearSessionPin;
exports.resetSessionPinsForTest = resetSessionPinsForTest;
/**
 ...
 */
exports.SESSION_PIN_TTL_MS = 30 * 60_000;
/**
...
function sweep(now) {
    for (const [key, pin] of pins) {
        if (now - pin.touchedAt >= exports.SESSION_PIN_TTL_MS)
            pins.delete(key);
    }
}
/**
 ...
 */
function readSessionPin(sessionKey, now = Date.now()) {
    if (!sessionKey)
        return null;
 ...
 */
function writeSessionPin(sessionKey, workspaceId, now = Date.now()) {
    if (!sessionKey)
        return false;
    sweep(now);
