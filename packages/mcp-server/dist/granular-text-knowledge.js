...
exports.KNOWLEDGE_TEXT = void 0;
exports.KNOWLEDGE_TEXT = {
    dopl_browse_knowledge: {
        description: 'Knowledge bases without their bodies; open the cheapest view first. action="list_bases" the bases you can read, "tree" a base\'s folders and entries (400 entries a page), "list_dir" one folder, "outline" an entry\'s headings and sizes.',
        params: {
    ...
    dopl_write_entry: {
        description: "Create or overwrite a knowledge entry, or replace one `section`. Overwriting needs `expected_version` from your last read. An entry past ~1.5k chars needs ## headings, one topic each.",
        params: {
    ...
    dopl_manage_knowledge: {
        description: "Bases and folders: create_base, update_base, create_folder (mkdir -p), move_folder, move_entry, publish (a base public to every member, one way), grant (lend a base you made). Nothing here deletes.",
        params: {
    ...
    dopl_list_skills: {
        description: "Active skills you can see, with their triggers, grouped by folder; drafts and skills you have no grant on are absent. Check at every task boundary.",
        params: { folder: "Only skills in this folder." },
    },
    dopl_get_skill: {
        description: 'One skill: view="details" its metadata and SKILL.md body, view="body" the body plus the Version dopl_update_skill needs.',
        params: { detail: 'view="details": "summary" leaves the body out.' },
        required: ["slug"],
        fenced: true,
    },
    dopl_create_skill: {
        description: 'Create a skill: one SKILL.md procedure plus its triggers. Read dopl_get_guide topic="skill_authoring" first.',
        params: {
            name: "Skill name.",
    ...
    dopl_update_skill: {
        description: 'Change a skill: action="update" its metadata, "write" replace the SKILL.md (needs expected_version), "visibility" share it workspace-wide or make it private. Agents edit only skills with agent writes on.',
        params: {
            name: "Skill name.",
            description: "What the skill does.",
            when_to_use: "When to use it: its trigger.",
    ...
    dopl_list_versions: {
        description: 'Version history: resource="entry" or "skill" (revision previews one snapshot), "object" an ontology object, or a whole ontology\'s roll-up.',
        params: {
    ...
    dopl_restore_version: {
        description: "Write an earlier revision of an entry, skill or ontology object back as a new revision. Needs the revision from dopl_list_versions and the current Version.",
        params: {
            path: "entry: the entry path.",
            slug: "skill: skill slug or id.",
