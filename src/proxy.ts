...
import { isAuthEntryRoute, isLoopCountedAuthRoute } from "@/shared/auth/auth-routes";
import { PUBLIC_ROUTES, SELF_AUTH_ROUTES } from "@/shared/auth/public-routes";
import {
  LOGIN_BOUNCE_COOKIE,
  LOGIN_BOUNCE_LIMIT,
  applyTarget,
...
export async function proxy(request: NextRequest) {
  ...
  // / signatures); /_next; /invite/<token> and /link/<token> (signed
  // tokens); /auth/callback (may carry case-sensitive code).
  //
