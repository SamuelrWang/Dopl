export const MANIFEST_PROMPT = `You are analyzing content from a social media post about an AI/automation setup. Your job is to extract structured metadata into a manifest.json format.
...
export function buildManifestPrompt(rawContent: string): string {
  return MANIFEST_PROMPT.replace("{ALL_RAW_CONTENT}", rawContent);
}
