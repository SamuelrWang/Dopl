 ...
 * `AgentTemplate.knowledge` is a field ADDED to an already-persisted payload;
 ...
 */
export const EMPTY_KNOWLEDGE: readonly TemplateKnowledgeRef[] = Object.freeze([]);
 ...
 */
export function scopeKey(scope: TemplateKnowledgeScope): string {
...
export function refKey(ref: TemplateKnowledgeRef): string {
...
export function refToScope(ref: TemplateKnowledgeRef): TemplateKnowledgeScope {
 ...
 *  which is the argument `template-draft.ts › sameIds` already makes for ids. */
export function sameScopes(
  a: ReadonlyArray<TemplateKnowledgeScope>,
  b: ReadonlyArray<TemplateKnowledgeScope>
): boolean {
 ...
 */
export function scopeChipLabel(ref: TemplateKnowledgeRef): string {
 ...
 * lead with the base name; `TemplateKnowledgeRef.toolPath` is that one, and the
