 ...
 *
 * 🔒 ⚠ **READ-STATE HERE IS THE WATERMARK, NOT `channel_mention_reads`.** The inbox
 * above marks mentions read ONE AT A TIME because that list is picked over out of
 * order; a row badge is ONE mark whose clearing act is OPENING THE CHANNEL, so its
 * boundary is `channel_members.last_read_at`, the same watermark the dot takes.
 *
 * ⚠ **THE PER-CHANNEL CUTOFF IS APPLIED IN CODE** (`mention-tally.ts`) — PostgREST
 * cannot express one cutoff per row's channel. `since` is the OLDEST of them, a
 * GLOBAL floor, so every row it drops was already read in its own channel; a
 * MAXIMUM would filter an earlier-read channel's unread mentions out in SQL.
 *
 ...
 * `created_at DESC` discards the OLDEST stamps — the ones nearest their watermark —
 * rather than an arbitrary page.
 *
 * ⚠ TWO COLUMNS. A count that fetched message bodies would put a transcript scan
 * behind a badge.
 */
