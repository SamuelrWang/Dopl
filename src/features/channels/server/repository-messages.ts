...
// 🔴 **`findLastRoomAddressToAgent` IS DELETED (2026-09-18, Samuel's ruling).** It was RR2's one
// read — "who last addressed this agent in this room" — and RR2 is gone with the repair it
// performed (`service-wake-verdict-resilience.ts` carries the argument). It had one caller and
// has none; a read kept for a deleted arm is the shape a future paragraph gets written around.
// ⚠ ITS ORDERING RULE OUTLIVED IT and is stated where it is still used:
// `repository-messages-escalations.ts` cites `seq` DESC for the same reason (seq is unique per
// channel, so "the highest one" is TOTAL and no tie is representable).
