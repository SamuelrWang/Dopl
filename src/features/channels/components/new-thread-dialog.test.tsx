 ...
 *    (`use-channels-selection.ts › requestNewThread`) rather than a counter this file invents.
 *
...
async function openPopup() {
  const view = render(<ChannelsComposer {...composerProps(0)} />);
  view.rerender(<ChannelsComposer {...composerProps(1)} />);
...
describe("Discard and Escape are the same exit, and neither creates", () => {
  it("closes on Discard without a write, and comes back EMPTY", async () => {
    ...
    view.rerender(<ChannelsComposer {...composerProps(2)} />);
 ...
 *
 * ⚠ `use-channels-selection.ts › requestNewThread` IS THE HOP, not a counter this file writes:
...
function ThreadsTabHarness() {
  const sel = useChannelsSelection({});
  ...
  return (
    <>
      ...
      />
      <ChannelsComposer {...composerProps(sel.newThreadSignal)} />
    </>
  );
}
