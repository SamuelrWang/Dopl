...
import { ChannelsAgentPanel } from "./agent-panel";
...
describe("the panel's composer appears only where it can send", () => {
  ...
  it("renders the input on a build that can send", () => {
    ...
    bridge();
    mount();
    expect(screen.getByLabelText("Message New Agent")).toBeTruthy();
...
describe("what the panel's composer sends", () => {
  it("names the INSTANCE the panel was opened on", async () => {
    ...
    mount();
    fireEvent.change(screen.getByLabelText("Message New Agent"), {
  ...
  it("sends on Enter and keeps Shift+Enter for a newline", async () => {
    const message = bridge();
    mount();
    const box = screen.getByLabelText("Message New Agent");
  ...
  it("clears on a real send", async () => {
    bridge();
    mount();
    const box = screen.getByLabelText("Message New Agent") as HTMLTextAreaElement;
