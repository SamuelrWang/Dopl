...
/**
 * THE CHANNEL RECORD PANE'S LOADING SHAPE — the channel surface's OWN two
 * columns (Samuel, 2026-09-13, over the /home pane while a channel loads: *"this
 * is the skeleton for the channel, it doesn't look accurate at all needs to be
 * fixed"*).
 *
 * ⚠ **IT MOVED HERE FROM `apps/desktop-ui/src/pages/home/` ON 2026-09-17**
 * (wave 1 item 6, executed in wave 2). It is the ghost of `channel-surface.tsx`,
 * not of a /home page, and every host of that surface may wear it; leaving it in
 * one page's folder is what made `channels-skeleton.tsx` describe itself as *"a
 * rough fit"* rather than reuse this (F-220). ⚠ **THE MOVE NEEDED A SEAM, NOT A
 * `git mv`:** `SkeletonSurface` was SPA-only, so it moved down to
 * `shared/ui/skeleton-surface.tsx` in the same change and the SPA path re-exports
 * it. `pages/home/channel-record-skeleton.tsx` re-exports THIS, so /home's call
 * site and its byte-share pins did not move either.
 *
