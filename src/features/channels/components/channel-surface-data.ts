 ...
 *
 * ⚠ IT OWNS `useChannelsLive` SO THAT A MOUNT CANNOT FORGET IT. INVARIANTS §7:
 ...
 * coordinator each: `channels-core.tsx` folds its channel-LIST invalidation in
 * through `onDoorbell` (the list is the TREE's read), `channel-surface-
 * standalone.tsx` folds in nothing.
 *
 * ⚠ THE HOOK ORDER IS LOAD-BEARING (lifted verbatim out of `channels-core.tsx`,
...
import type { ChannelsDerivations } from "./derivations";
...
export interface ChannelSurfaceData extends ChannelsDerivations {
