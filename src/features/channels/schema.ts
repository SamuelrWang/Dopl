 ...
 *
 * Mirrors `DISPLAY_NAME_RE` in `src/app/api/user/profile/route.ts`: control
 * characters (the newline that forges a line, and every other C0 / DEL byte),
 * zero-width and bidi-override characters, and the line/paragraph separators
 * some renderers still treat as newlines.
 */
const CHANNEL_TEXT_RE =
  /^[^\u0000-\u001F\u007F\u200B-\u200F\u2028-\u202F\u2060-\u206F\uFEFF]+$/u;

const CHANNEL_TEXT_CHARSET_MESSAGE =
  "cannot contain control, zero-width, or line-separator characters";
...
const ChannelNameSchema = z
  ...
  .min(1)
  .max(120)
  .regex(CHANNEL_TEXT_RE, `Channel name ${CHANNEL_TEXT_CHARSET_MESSAGE}`);

/**
 * `topic` — optional, so `""` stays legal (the create dialog sends the field
 * cleared, and the column is NOT NULL with a `''` default). The regex requires
 * at least one character, hence the explicit empty-string branch rather than a
 * looser pattern: `""` carries nothing to forge with.
 */
const ChannelTopicSchema = z
  .string()
  .trim()
  .max(2000)
  .refine((t) => t === "" || CHANNEL_TEXT_RE.test(t), {
    message: `Channel topic ${CHANNEL_TEXT_CHARSET_MESSAGE}`,
  });
