...
import type { CanvasMembership, Invitation, Role } from "../types";
import { meetsMinRole } from "../types";

interface MemberRow extends CanvasMembership {
  email: string | null;
}
...
/**
 * Members section for the canvas settings page. Shows current members
 ...
 */
export function CanvasMembersSection({ canvasSlug, myUserId, myRole }: Props) {
  ...
  async function refresh() {
    ...
    try {
      const [memRes, invRes] = await Promise.all([
        fetch(`/api/canvases/${encodeURIComponent(canvasSlug)}/members`),
        canManage
          ? fetch(`/api/canvases/${encodeURIComponent(canvasSlug)}/invitations`)
  ...
  async function changeRole(target: MemberRow, role: Role) {
    ...
    try {
      const res = await fetch(
        `/api/canvases/${encodeURIComponent(
          canvasSlug
  ...
  async function removeMemberRow(target: MemberRow) {
    ...
    if (target.userId === myUserId) {
      const ok = window.confirm(
        "Leave this canvas? You won't be able to rejoin without a new invitation."
      );
      if (!ok) return;
    }
    setBusyId(target.userId);
    ...
    try {
      ...
      // If you removed yourself, fall back to the canvas list.
      if (target.userId === myUserId) {
  ...
  async function revokeInvite(invitation: Invitation) {
    ...
    try {
      const res = await fetch(
        `/api/canvases/${encodeURIComponent(canvasSlug)}/invitations/${encodeURIComponent(
          invitation.id
        )}`,
        { method: "DELETE" }
      );
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-white/50 mt-1">
            People with access to this canvas's clusters, brain, and chats.
          </p>
        </div>
        {canManage && (
          <button
