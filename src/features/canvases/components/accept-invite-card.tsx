...
/**
 * Accept-invite landing card. Shows the canvas + inviter context, then:
  ...
  async function accept() {
    ...
    try {
      const res = await fetch(
        `/api/canvases/invitations/${encodeURIComponent(token)}/accept`,
        { method: "POST" }
      );
      if (!res.ok) {
      ...
      const { canvasSlug } = (await res.json()) as { canvasSlug: string };
      router.push(`/canvas/${canvasSlug}`);
      router.refresh();
    } catch (err) {
      ...
      {dead ? (
        <div className="mt-6 rounded-md bg-red-500/10 border border-red-500/20 p-3">
          ...
          <p className="text-[11px] text-white/40 mt-1.5">
            Ask the canvas admin to send a fresh invitation.
          </p>
        </div>
      ) : needsAuth ? (
        ...
        <div className="mt-6 flex flex-col gap-3">
          ...
          >
            {accepting ? "Joining..." : `Join ${status.canvas.name}`}
          </button>
        </div>
      )}
