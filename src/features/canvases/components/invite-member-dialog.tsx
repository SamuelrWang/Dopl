...
const ROLES: Array<{ value: "admin" | "editor" | "viewer"; label: string; hint: string }> = [
  ...
  { value: "admin", label: "Admin", hint: "Editor + invite + canvas settings" },
];

/**
 * Invite a new member to a canvas. Sends a POST that mints a token,
  ...
  async function handleInvite() {
    ...
    try {
      const res = await fetch(
        `/api/canvases/${encodeURIComponent(canvasSlug)}/invitations`,
        {
          method: "POST",
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md bg-[#0a0a0a] border-white/[0.12] text-white">
        <DialogHeader>
          <DialogTitle className="text-white">Invite to canvas</DialogTitle>
