...
/**
 * THE LAUNCH PICKER — the popover that CHOOSES an identity.
 *
 ...
 * normal agent launch, except the identity is pre-selected"*). Three whole
 ...
 *  - **A ROW CLICK HANDS UP THE WHOLE IDENTITY**, not its id — the popup prefills
 ...
 *    filtering.
 *
 * `useAgentIdentities` is MOCKED: what this file is about is what the picker
...
describe("what the popover offers", () => {
  ...
  it("hands a row's WHOLE IDENTITY up, and closes — it starts nothing", () => {
    // 🔒 MUTATION-PROOF: hand up `identity.id` instead of the row and the first expectation
  ...
  it("renders a model chip only when the identity carries a model", () => {
    ...
    // rather than "Default" — a row states what an identity CARRIES.
 ...
 * identity's instructions are another member's text about to run on THIS machine
...
/**
 * ⚠ SOURCE READ, like `./identity-editor-surface.test.tsx › no concave surfaces`. jsdom
 ...
 * `features/agent-identities/{components,lib,hooks,client}` — this names THE NEW
