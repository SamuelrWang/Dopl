 ...
 * `agent_identities` itself and apply the matrix. **That would be a SECOND copy
 * of `canSeeIdentity`**, and this tree has the record of what that costs:
 ...
 * `agent_identities_member_select`) and that pair is documented as having to move
 * together.
 *
 ...
 * pairs are architectural). `channels → agent-identities` is already six imports
 * on the client side (`identity-picker`, `launch-overrides`), so this adds a
 ...
 * OPERATOR's credential** (`main/identity-resolve.js`, spec §3b). A resolver
 ...
 *
 * `agent_identities` has NO name uniqueness, deliberately: a unique index across a
 ...
 *
 * ⚠ THE LIST IS NOT AN ORACLE. It contains only rows `canSeeIdentity` has already
 ...
 * thrown `AgentIdentityNotFoundError` crossing into the channels error mapper
...
export type IdentityRefResolution =
  ...
  | { kind: "not-found" }
  /**
   * ⚠ THE MISS THAT IS NOT A MYSTERY (T35). The ref names an identity the caller
   ...
   * {@link classifyMissingIdentityRef} for why saying so is not an oracle.
   */
  | { kind: "elsewhere"; identity: IdentityElsewhere }
  | { kind: "ambiguous"; matches: IdentityRefMatch[] };

/** An identity the caller holds somewhere else, and the phrase for WHERE. ⚠ The
 ...
 *      "Contract Auditor", and a substring rule turns every added identity into
 ...
 * as `listIdentities` does, because the matrix is a TypeScript predicate over
 * team links — it cannot be expressed as a `WHERE`. The row count is a
 * workspace's identity list; this is a cold path (one call per launch), and the
...
export async function resolveIdentityRef(
  ...
  // spawn-time one (`service-reads.ts › readIdentityById`) now give the SAME
  // answer for the same id: a personal identity launches anywhere its owner is.
...
async function resolveNameInThisTenancy(
  ...
  return {
    kind: "ambiguous",
    // ⚠ NAME-ORDERED ALREADY (`listIdentitiesForWorkspace` orders by name), so the
 ...
 * (T35).
 *
 * An identity read is keyed `(workspace_id, id)`, so `canSeeIdentity` is never
 ...
 * NAME still cannot resolve a tenancy on its own (`agent_identities` has no name
