...
export async function buildBuilderBundle(
  ...
  if (results.length === 0) {
    return {
      ...
      prompt: "",
      instructions:
        "No entries in the knowledge base matched this brief. Tell the user the KB doesn't have relevant prior art yet; suggest ingesting a few URLs (via `dopl_ingest(op=url)`) related to the brief, then retry `build_solution`.",
    };
  }
  ...
  return {
    ...
    instructions: [
      ...
      "Nothing is persisted server-side — this is a synthesis-only tool. If the user wants the composite saved as a new KB entry, run `dopl_ingest(op=url)` on the final artifacts separately.",
    ].join(" "),
  };
}
