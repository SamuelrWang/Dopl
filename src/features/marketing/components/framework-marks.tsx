/**
 * The vendor marks the Framework vignettes are cast with
 * (./framework-windows), each on its own official grid.
 *
 * INLINE SVG, deliberately: the banner must not make a network request for a
 * logo, and an <img> to a brand CDN is both a request and a hotlink. Path data
 * is the vendors' own; only the layout is ours. The one exception is DoplMark
 * at the bottom — our own icon, already shipped as a PNG.
 *
 * Sizing lives entirely in marketing.css (.lp-fw-source-mark /
 * .lp-fw-source-img); nothing here carries a dimension.
 */
...
/** Slack ships from simple-icons as ONE monochrome path. It is split back here
 *  into the four official colour pairs — each pair is a bar plus its pin, and
 *  the quatrefoil reads blue top-left, green top-right, yellow bottom-right,
 *  red bottom-left. Keep the pairs together: a bar without its pin is not the
 *  mark. */
...
/** simple-icons `icons/claude.svg` — the starburst, in Anthropic's coral. The
 *  mark is ONE path; it is not reconstructible by hand, so it is pasted whole. */
const CLAUDE_PATH =
...
/** Gmail's envelope is FIVE pieces on a 88×66 grid — Google's own multicolour
 *  file (Wikimedia `Gmail_icon_(2020).svg`), not the flattened simple-icons
 *  glyph, which is monochrome and would read as a generic mail icon here. */
...
/** The one mark here that is NOT a vector: our own app icon, already shipped as
 *  a PNG for the favicon set and reused by the nav lockup. Plain <img> for the
 *  same reason as the banner — a static public/ file in a box the CSS sizes. */
