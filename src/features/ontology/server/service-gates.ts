 ...
 *
 * The gate lives here and not beside the write: a gate beside its insert is
 * one a later edit reorders past it (the `knowledge/server/
 * service-base-gates.ts` seam, same reason).
 *
 * A gate returns the ROW, and the row's container is where the write lands.
 * A lent ontology lives in the LENDER's container, so callers MUST use
