...
| A33 | Ontology audience | `ontology/server/service-audience.ts › computeAudience` (L127): `link`/`personal` → resolved from share rows | `standard` → **`unrestricted`** (L129) | ❌ two arms | **home-only (the fine-grained half)** | a standard workspace has NO per-ontology sharing: every member sees every board. Parity item #2 | 🔴 high |
