...
| C-2 | Credit bar / credit-spend series | `/home` Overview only — `pages/home/overview-panels.tsx:334` › `CreditsBar`, `:345` `useWorkspaceEntitlements`, `:191` `TokenSpendPanel` | **workspace Overview has no credits surface at all** (`grep -n credit apps/desktop-ui/src/pages/overview/*.tsx` → 0) | home-only | **RULING R-6** | out of this area's centre but it is the only place an operator sees spend |
