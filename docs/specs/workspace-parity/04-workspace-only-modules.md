...
| 19 | **Onboarding** | Three steps — survey → MCP connect → workspace name — plus the welcome popup, the connect banner and the provisioning checklist. | Desktop only: `apps/desktop-ui/src/pages/onboarding/index.tsx` (L54), route `routes.tsx` (L130–134); popup + banner at `app-shell.tsx` (L269–281). API `src/app/api/onboarding/**`. | **None, and none is wanted** — `redirectPath` answers `/{segment}/overview`. | **WS-KEEP** | MEDIUM. It is the first-run path; a parity wave that changes the shell can break it silently. | ⚠ `onboarding/components/welcome-popup.tsx` hardcodes hex (L91, L105) against CLAUDE.md § DESIGN-SYSTEM. **`WS-RESTYLE` on that file alone.** |
...
| 21 | **Tour** | Five-step product tour keyed to `NavSection`. | Desktop workspace shell only: `app-shell.tsx` (L252–255, L283). Steps `tour/tour-steps.ts` (L19+) — ontology, knowledge, skills, chats, … | **None — it never mounts on /home.** | **RULING** (§E-4) | LOW. 4 files, 331 lines, **zero tests**. | ⚠ Its step list is keyed to `NavSection`, so it is coupled to §0.2's four-file rule. A tour of home would need a second vocabulary. |
...
7. **I did not audit the two remaining "old recipe" hex offenders end to end.**
   `onboarding/components/welcome-popup.tsx` (L91, L105) and `auth/components/password-requirements.tsx`
