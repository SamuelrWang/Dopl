...
| A12 | **Notice / guidance layer** | **ABSENT** — /home mounts no `TourProviderCore`, no `JoinRequestNoticesCore`, no `ConnectAgentBanner`, no `WelcomePopup` | `app-shell.tsx:252` / `:269` / `:270` / `:271` | — | n/a | **workspace-only** | RULING E7 | All four already live in `src/features/{tour,workspaces,onboarding}` and are host-agnostic; /home simply never mounts them. A no-code-change parity win. |
...
| A17 | **Ontology** | `pages/home/ontology-panels.tsx:26-63` — mounts the SAME `OntologyView` with `pinnedClusterId` + `onSelectCluster` + `onCreateCluster` + `settingsMenu` + `frameless` | `pages/ontology/index.tsx:44` → the same `OntologyView` with none of those | — | **YES** | **shared-already via props** (second reference implementation) | none | `ontology-view.tsx:57` `pinnedClusterId`, `:60` `onSelectCluster`, `:73` `onCreateCluster`, `:83` `settingsMenu(close)`, `:103` `frameless`. `frameless` is *exactly* the host-supplies-the-surface adapter §F asks for. |
...
| B5 | **`MentionBadge` / `UnreadDot` — the `@ N` pill and the dot** | `src/shared/ui/home-card-marks.tsx`; `home-channel-row.tsx:215-216` | **Deliberately absent.** `Channel.unread` is a boolean and `sidebar-rows.tsx:27-31` forbids inventing a count | n/a until the payload carries a count | HIGH — this is a DATA gap, not a style gap. |
...
| B13 | **`[data-overview-face] .bento` — the deeper card shadow** | `overview-panels.tsx:114` stamps the hook; rule at `globals.css:891` / `kit.css:169` | No workspace surface stamps it | **CSS-only** | LOW — deliberately scoped (Samuel, 2026-09-13). |
...
| B23 | **Presence rings** | `home-channel-row.tsx:185-195` uses a bare `AvatarStack` — **no presence** | `AvatarWithPresence` is used in `channels/member-roster.tsx:47`, `› thread-info-tab.tsx:218`, `› invite-dialog.tsx:239`, `playground/members-pane.tsx:255,357` | **component change** | LOW. `AvatarStack` has no presence variant; `avatar-stack.tsx` docs say `2xs` exists only for this row. |
...
| D8 | **`RelationshipList` → an `ObjectColumn`** | `apps/desktop-ui/src/pages/home/relationship-list.tsx` | **253** (of which ~90 is the row derivation, which should stay) | The generic half (290px column + `WellsColumn` + empty sentence) is what every workspace page's left column wants | the derivation reads `HomeRow` — split it first, exactly as `home-channel-wells.ts:10-15` split set-from-filing |
...
hosts, four knobs: `pinnedClusterId` (`:57`), `onSelectCluster` (`:60`), `onCreateCluster` (`:73`),
