...
7. **Q8 IS ENFORCED ON WRITES AS WELL AS READS, which the spec did not say.** `service.ts › sanitizeEdges` validates edge targets through `service-gates.ts › admittedObjectIds` (the ontology walk) rather than through `repository.ts › filterObjectIds` (the read SCOPE) — the scope holds the LENDER's whole container, so scope-only validation let a lent editor point an edge at a row they cannot see. `› currentRelationships` filters the far end for the same reason. Added at review; pinned and mutation-verified in `service-gates.test.ts › Q8 on a WRITE`.
8. **The ontology payload carries `agentsMayEdit` and `sharedChannelCount`** (`service-reads.ts › mapOntologyRow`), the second only for ontologies the caller OWNS and via ONE batched read (`repository-shares.ts › countSharesForOntologies`). Both are OPTIONAL on `types.ts › Ontology` because a cached snapshot predates them, and the two fallbacks are different answers on purpose (`true` vs `null` — unknown is not empty).
9. **Q6 attribution landed on `ontologies` TOO**, not only on objects, so a rename through a share is attributed the same way an object edit is.
...
- **R7 — `useOntologyRealtime` still subscribes per WORKSPACE.** `20261001130000`'s header records the consequence: a peer mounted on the owner's container now receives frames for a SHARED ontology's rows (the intended reach) and the client-side narrowing is not built.
...
⚠ **ADDENDUM 2026-09-11 (Samuel's wording ruling), AMENDED 2026-09-23:** column = object (an object TYPE), card = item, and the graph's top level is an **ontology** everywhere — UI, code, API, MCP and database (table `ontologies`, since `20261022120000_ontology_vocabulary_rename.sql`). The 2026-09-11 half that kept the old identifiers is superseded; the `column` identifiers (`create_column`, `columnIds`) still stand. INVARIANTS §4A carries the rule and names its gate (`src/features/ontology/vocabulary.test.ts`).

- **ONTOLOGY** = one `ontologies` row. 📌 **ASSUMPTION A1**: Samuel's "multiple ontologies" maps onto the existing ONTOLOGY, not a new container — an ontology is already the addressable board (`src/features/ontology/components/ontology-view.tsx › OntologyView`, one ontology per page).
- **HOME SPACE** = the owner's `kind='personal'` container (INVARIANTS §4A); a **home ontology** is an ontology whose `workspace_id` is that container. 📌 **ASSUMPTION A2**: therefore `supabase/migrations/20260920120000_workspace_kind_personal.sql` and `20260923120000_drop_home_scoped.sql` are PRECONDITIONS of this wave. Applied is a MEASUREMENT joined on the migration NAME, never the filename (INVARIANTS §12).
- **HOME CHANNEL** = the one channel in a `kind='link'` container. **SOLO** = one active member; **SHARED** = two or more. **OWNER** = the ontology's `created_by`; **MEMBER** / **GUEST** = a person in the channel at `member`+ / at `guest` (`src/features/workspaces/types.ts › Role`).
...
**CHOICE: a dedicated `ontology_channel_shares` plus one boolean on the ontology — NOT `resource_grants`.** Reuse is the default answer in this repo, so here are the four reasons it loses:
...
```
ALTER TABLE ontologies ADD COLUMN agents_may_edit BOOLEAN NOT NULL DEFAULT true;
...
|---|---|---|
| `ontologies` | `["canSeeOntology"]` | `ontologies_member_select` → `dopl_ontology_readable` |
...
9. `dopl-desktop-app/main/prompt-framing.js › channelScopeFraming` gains which ontologies this session reaches and at what level, beside `dopl-desktop-app/main/prompt-framing-text.js › HOME_SPACE_KNOWLEDGE_CONFIDENTIALITY`. ⚠ **THIS IS A COMPENSATING CONTROL, NOT A GATE** — INVARIANTS §4A says exactly that about the home-space-reach lane, and reading it as a gate is how this becomes a leak.
...
- **Q4 DELETING A SHARED ONTOLOGY / DELETING A CHANNEL.** *Recommend:* both cascade the share rows by FK. The ontology delete sits behind a confirm naming the channel COUNT and stays app-only (`sessionOnly` on the ontology DELETE route; `dopl_ontology` has no delete op and must not gain one).
...
- **Q8 DOES A SHARED-IN READER SEE OBJECTS OUTSIDE THE ONTOLOGY?** *Recommend:* no — the ontology's membership walk is the boundary, and an object reachable only from another ontology is not in this one.
- **Q9 AN OBJECT IN TWO ONTOLOGIES, ONE OF THEM SHARED.** *Recommend:* a WRITE requires `edit` on EVERY ontology the object belongs to; a READ requires it on ANY. Otherwise `members_level='edit'` in one channel silently edits an ontology that channel cannot see. ⚠ The sharpest consequence of R5, and it needs Samuel's word.

## §8 Risks — where the tree assumes ontology is WORKSPACE-scoped (every one live 2026-09-09)

- **R1** `src/features/ontology/server/service.ts › getSnapshot` — four reads, all keyed on `workspaceId` alone. There is no ontology-scoped read path anywhere in the feature.
...
- **R4** `src/features/ontology/server/repository-projections.ts › listOntologySummaries` / `› listObjectSummaries` — same shape, the narrowed twin.
- **R5** 🔒 **AN OBJECT CAN SIT IN SEVERAL ONTOLOGIES.** `supabase/migrations/20260706120000_ontology.sql` says so in its header and `ontology_memberships` is built for it, so "the objects of one ontology" is a GRAPH WALK, not a column. This is the largest single cost of per-ontology sharing and the source of Q9.
- **R6** That same migration's four RLS policies fence on `is_workspace_member(workspace_id, auth.uid(), 'viewer')` — workspace-wide, no per-ontology arm — and none of the four tables is in `scripts/check-rls-pair-gate.ts › COVERED` today.
...
- **R8** `src/features/ontology/components/ontology-view.tsx › OntologyView` takes ONE `canEdit` boolean for the whole board; per-ontology levels make that per-ontology.
