...
| G19 | "Respond and loop until the goal is met, then STOP" | nothing in the turn path | **NONE** | `maxTurns` on every spawn — the SDK exposes it and Dopl set it nowhere. ⚠ **CORRECTED 2026-09-02: IT SHIPPED AS ONE CONSTANT, NOT "from the launch posture".** `main/runtime/claude/launch-spec.js › SESSION_MAX_TURNS` is a fixed 1000 applied at the single assembly point, so a park or resume cannot shed it and no tool profile can move it — a per-posture ceiling would be a permission axis wearing a budget's clothes. It is a RUNAWAY BACKSTOP far above `session-state.js › DEFAULT_TURN_CAP`, which is the visible limit the reducer enforces and reports; this one only fires for a query that never reaches another `result` event. | **A10** |
...
| **A5** | `v2/a5-builtin-bound` | `dopl-desktop-app/main/runtime/claude/tools.js`, `test/session-profiles.test.mjs` | Replace `builtinTools: []` for `full` with a positive bound. **Remove only tools `grantDecision` classifies as unclassified→gate in every mode including `bypass`** — `Workflow` (21,332 ch alone), `DesignSync`, `Cron*`, `ScheduleWakeup`, `PushNotification`, `Task*`, `Enter/ExitWorktree`, `Monitor`, `ReportFindings`, `SendMessage` — **plus `Agent` and `Skill`** ⚠ **CORRECTED 2026-09-02: `NotebookEdit` WAS ON THIS LIST AND IS NOT REMOVED, AND THE SHIPPED DERIVATION IS RIGHT.** It is a member of `EDIT_TOOLS` (contract A2 names exactly `Write` / `Edit` / `NotebookEdit`), so it classifies at `accept_edits` rather than unclassified→gate — it fails this row's OWN criterion. `FULL_BUILTIN_BOUND` is derived from `BYPASS_TOOLS` and keeps it; removing it would have taken an edit tool off `full` and made the accept_edits grant key name something the session could not run. (see the ruling note below). Keep `Bash`, `Write`, `Edit`, `WebFetch`, `WebSearch`; B7 owns those. Add G21's test. Verify with the capture harness, never by reading the constant | **−~67,400 ch/turn (~16.9k tok)** plus **−8,547 ch** of leaked personal inventory |
...
| **A13** | `v2/a13-contracts` | new `packages/contracts/**` | The shared types package. ⚠ **THIS ROW SAID "imported by nothing yet. Groundwork only" AND WAS STALE ON ARRIVAL (corrected 2026-09-02).** A13 SHIPPED the imports: `src/features/channels/types*.ts`, `workspaces/types.ts`, `agent-templates/types.ts` and both packages' SDK counterparts import and re-export it, and `check-role-drift` / `check-session-health-drift` / `check-message-kind-drift` each lost the halves a re-export cannot make disagree. ⚠ It is TYPE-ONLY and therefore **not built** — "built in CI" was wrong too. ⚠ `@dopl/mcp-server` declares it though it names no symbol from it: `@dopl/client`'s `.d.ts` files import from it, so its `tsc` must resolve it (INVARIANTS §1) | 0 tok; three drift gates shrank rather than being deleted |
