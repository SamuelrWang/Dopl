...
Add `knowledge` to `channels/components/info-panel.tsx › TABS` + `knowledge?: boolean` on `ChannelSurfaceCapabilities`. One component serves operator (`relationship-record.tsx`) and guest (`guest-channel.tsx`) — reads the CHANNEL LANE (§3), shows `visible` grants only; guest sees exactly what the operator sees in that tab. ⚠ "THE SURFACE MUST NOT ISSUE A REQUEST IT WILL GET 403 ON" — the tab's read is guest-floored so safe; add to `guest-surface-reads.test.tsx`.

## 6. The auto_both+full+non-owner warning
`channels/components/settings-agent.tsx` carries the MINIMAL COPY ruling — so NOT a standing explainer: a `ConfirmDialog` (destructive) fired at the moment the combination becomes true, naming the peer. Pure predicate in `agents-controls.ts` (or `posture-warning.ts`): `messageMode==="auto_both" && toolProfile==="full" && roster.some(m => m.userId !== currentUserId)`. Unit-test the predicate. If tight, FILE rather than half-build.
...
- ✅ **M7** Docs, findings, KB sync — **DONE 2026-08-26.** INVARIANTS §3/§4A/§5/§8/§9/§10/§11/§12/§14; ENGINEERING's audience-ceiling stratum (+ the container-property and credential-lock-vs-header-pin arguments); F-323 narrowed, F-329 reopened, F-335/F-336/F-337 filed, F-340/F-341 filed from the handoff block; `node scripts/check-doc-refs.mjs` green; `Dopl Development` KB synced (entry *Home knowledge panels + Agents tab — grants, the audience ceiling, two open rulings*).
Sequencing: M2←M0; M3←M0; M4←M2; M5←M0 (parallel with M3/M4 possible).

## M7 HANDOFF — owed items from earlier milestones — ✅ ALL DISCHARGED 2026-08-26
- ✅ **M4's owed finding is FILED as F-340** — the info-panel tab row is FIVE tabs on a width budget measured for four (DESIGN-SYSTEM: four options with two badges overflowed at 15px pad / 340px; at 380px the four leave ~55px and "Knowledge" needs ~90). M4's tightening (`gap-1`, header `px-2`, only when the fifth is present) and the SCROLLING residual are recorded as shipped state, four options are stated with their blast radius, and the ⚠ **do-not-shorten-the-label** constraint is 🔴-marked in both F-340 and INVARIANTS §5. Samuel's ruling owed; ship as-is meanwhile.
- ✅ **M4's ENGINEERING stratum is WRITTEN** (2026-08-26 — the audience ceiling, extended in this pass with the ceiling-is-a-container-property and credential-lock-beats-header-pin arguments). The Dopl KB sync is the last item of this milestone.
- ✅ **M4's INVARIANTS §5 correction VERIFIED against the tree at M7** — `channel-surface.tsx › ChannelSurfaceCapabilities` still declares exactly three flags, `knowledge` still defaults `false` and still ADDS the tab while `memberManagement`/`selfManagement` default `true` and NARROW. **"May add one, never remove one" reads true.** The width-budget caveat was added beneath it as a sub-bullet.
- **M3 filed F-330** (`canEdit` falls open when no MyAccessProvider is mounted — unreachable-to-harm from both current hosts; fix shape = report PROVIDERLESS distinctly from PENDING, do NOT flip the default closed). Still open, unchanged.
- ✅ **M6's owed INVARIANTS §5 line is WRITTEN** — the tab's one warning is a `ConfirmDialog` fired on the TRANSITION, never a standing banner, stated as the minimal-copy ruling APPLIED rather than an exception to it, so nobody "restores" an explainer paragraph under either select.
- ✅ **M6's flag is FILED as F-341** — `agent-posture.tsx › PostureControls` renders the same two axes but writes the LIVE session mode over the bridge (read per call by `main/session-io.js › grantArgs`), storing nothing, and reaches the warned-about combination unwarned. Three options stated, including why a re-typed predicate would be a second statement of one fact. **Filed, not built** — Samuel may want the warning there too.
