...
**Composer @-chips** (B6): `channels/components/composer-mentions.tsx:87` already offers agents but
...
| `channels/components/view-model-rows.ts:47-53,245,268` | lifecycle kinds → receipt rows; `task_progress` deliberately stays prose | B9 |
| `channels/components/transcript.tsx:125-165` | `system \| receipt \| thread-card \| escalation \| message` | B9 |
...
| 2 | `workspaces/server/service.ts:178-200` `ensureDefaultWorkspace` + `:204-222` `renameDefaultWorkspaceIfUntitled` | `ensureHomeSpace(userId)` — same advisory-lock SELECT-or-INSERT shape, `kind='personal'`, one per user. ⚠ **THIS ROW SAID "Signup still seeds a standard workspace (unchanged, out of the ruling's scope)" AND IT WAS NEVER TRUE OF WHAT SHIPPED** — the row below (`:456`) flagged the contradiction on the day, and Samuel's ruling **R-35** (2026-09-17) settled it: **signup mints the personal container and nothing else.** Both provisioning sites call this function; `createWorkspaceForUser` is explicit-create only. Corrected 2026-09-17; `docs/INVARIANTS.md` §4A is the live statement |
| 3 | `20260802200000_ensure_default_workspace_rpc.sql` + `20260823160000_default_workspace_kind_guard.sql:43-110` (the live `CREATE OR REPLACE`) | `ensure_home_space(p_owner_id)`; both dropped. ⚠ The kind guard exists ONLY to stop a `kind='link'` container becoming a default — with no default it has nothing to guard |
...
| 14 | `src/app/billing/page.tsx:35`, `src/app/api/workspaces/ensure-default/route.ts`, `src/app/auth/callback/route.ts:51`, `workspaces/server/segment.ts:353-380`, `onboarding/server/service.ts:66`, `seed-workspace.ts:25` | all call `ensureHomeSpace`; `/billing` with no segment renders the workspace list instead of guessing; the dedicated `ensure-default` route is deleted |
...
1. `20260920120000_workspace_kind_personal.sql` — widen `workspaces_kind_check` to
   `('standard','link','personal')`; add `ensure_home_space`; **mint one personal container per
...
2. **Dual-write release.** Writes set BOTH `home_scoped` and `workspace_id`; reads prefer
   `workspace_id` behind `TENANCY_HOME_SPACE` (default off, flipped per environment). The 3
...
| G20 / F-450 | "channel work is answered into the channel" | ✅ **CLOSED BY RULING 2026-09-17 (R-36) — THE SET IS SEVEN, AND SEVEN IS WHAT THIS DOC CLAIMS.** It did not become code in Wave B (corrected 2026-09-02, in review — the row said it would), and Samuel ruled that no eighth field is landed. **`scripts/check-session-health-drift.ts` measures SEVEN and is the authority on that number**; this row said "the EIGHTH field was never added", which reads as a field still owed, and no field is owed. ⚠ **The guardrail is prose and stays prose** — recorded here rather than glossed, because the standing rule (`mcp-v2-architecture.md`) is that a prohibition told to an agent has a fence, and this one does not | **RULED — no eighth field** |
...
pays (worst 725) · `agentIdentityFraming`'s claim protocol (~870/turn) · the `homeSpaceShelfNoun` regex
...
| **B9** fan-out narrowed | `v2/b-fanout-narrow` | `dopl-desktop-app/main/{session-dispatch,session-wake-tiers,targeting,listener-messages,session-gate,pings}.js`, `main/session-triage.js`, `main/runtime/claude/triage.js`, `src/features/channels/components/{view-model-rows,transcript}.tsx` | Execute the stored verdict; **delete TRIAGE whole** (354 lines + ~165 of the tiers module + 4 test files); `classify` → two questions; one lane in `listener-messages.js` | the composed drive from B4 re-run against the narrowed desktop; a test that a sibling is NOT fed | `WAKE_TIERS_ENABLED` is already a kill switch; the verdict is stored, so reverting the desktop restores fan-out | F-540..549 | — |
| **B10** composer chips | `v2/b-composer-chips` | `src/features/channels/components/composer-mentions.tsx`, `composer-input.tsx`, `src/features/channels/lib/agent-mentions.ts` | @-chips over the channel's LIVE agents, not just this machine's | the picker offers a peer's agent handle; no operator-only field leaks (`mapPeerSessionStateRow`) | revert one commit | F-550..559 | — |
| **B11** personal container | `v2/b-home-space` | `supabase/migrations/*`, `src/features/knowledge/server/repository-bases.ts`, `src/features/agent-templates/server/repository.ts`, `packages/mcp-server/src/tools/shelf.ts`, `scripts/check-knowledge-type-drift.ts` | Mint one per user; move the 3 `home_scoped` rows; **dual-write for one release** behind `TENANCY_HOME_SPACE` | a stale-cache test per new payload field (`?? EMPTY_X`, INVARIANTS); both read paths agree under the flag | flag off; the column still carries the truth | F-560..569 | `20260920120000_workspace_kind_personal` |
...
| # | Slice | Branch | Slice commits | Merge | Status |
|---|---|---|---|---|---|
| 1 | **B11** personal container | `v2/b-home-space` | `a7a32db9..3a9ef9bb` (5) | `28bc8f8d` | DONE — clean |
...
| 2 | `ensureHomeSpace`; *"signup still seeds a standard workspace"* | ⚠ **THE PARENTHESIS IS THE ONE PLACE THIS SLICE CONTRADICTS THE SPEC.** Row 14 repoints the auth callback at `ensureHomeSpace`, and nothing else in the signup path creates a workspace — so a new user gets ONE personal container and no standard workspace, which is what the brief's acceptance test asks for and what "all workspaces are just normal workspaces" means. The starter corpus seeded into the container until 2026-09-10, when Samuel's *"drop seed content"* ruling made a fresh personal container EMPTY — see INVARIANTS §4; `20260920120000`'s header had asked for that from the start. ~~**Recorded, not glossed: if Samuel meant signup to keep minting a standard workspace, one call comes back.**~~ ✅ **HE DID NOT — RULED 2026-09-17 (R-35):** *"New users should not be getting a workspace. It should be the home space."* The call does not come back, and the home space is now PERMANENT as well as sole (`docs/INVARIANTS.md` §4A). **This row was right and rows 2 / §7's summary were the drift**; recording the contradiction instead of glossing it is what made the question answerable three weeks later. |
| 3 | both SQL objects dropped | as written — and `default_workspace_of` is RENAMED to `home_space_origin_of` rather than inlined, because it is a live dependency of the mint and F-560's exact revert reads it (F-630) |
...
| 14 | all call `ensureHomeSpace`; `/billing` renders the list; route deleted | as written. ⚠ The route's deletion overrides **F-164**'s standing condition (a minimum-version floor that does not exist) — transferred to **F-631**, unmeasured |
...
(B15's) each swap an import for `isOwnHomeSpace` (never adopted: it had no caller anywhere and was deleted
...
| ↳ ⚠ **MEASURED AT THE BATCH-3 INTEGRATION: TWO, NOT ONE — AND BOTH ARE PROSE BY RULING SINCE 2026-09-17.** G18 was already prose by ruling; **G20/F-450 was prose BY DEFAULT and is prose by RULING now** (R-36): the session-health set is **SEVEN** fields, `npx tsx scripts/check-session-health-drift.ts` is what holds that number, and no eighth is landed. The wording here claimed an eighth was missing, which is a field owed; none is | 6 | **2** | — |
...
which is no longer a default but a first workspace~~ ⚠ **WRONG AND CORRECTED 2026-09-17 (R-35): signup
seeds NOTHING but the personal container** — this sentence recorded the same decision the §3 row 2
parenthesis did, and the slice that implemented it did the opposite on purpose (`:456`); and the personal container is **not in the rail** —
