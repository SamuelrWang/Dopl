...
| `GET …/{id}/members` | `loadVisibleChannel` | Both member rows incl. peer `email`, `displayName`, `avatarUrl`, presence; own-only fields (`notifyScope`, `agentToolProfile`, `myFavoritedAt`) nulled for the peer by `dto.ts › mapMemberRow` | Accepted (R7 / F-299) for `email`; the per-member preference scrub holds |
