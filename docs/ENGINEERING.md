...
- **`sessionOnly: true`.** Rejects ALL OAuth tokens (403 `SESSION_REQUIRED`) regardless of scope — for destructive or permission-mutating routes an agent must never reach. Applied to 15 routes: user delete; workspace delete; member role-change + remove; invitation create + revoke; join-request approve; billing checkout / portal / upgrade-to-team; access-matrix PUT; teams members add + remove; teams access PUT; join-link POST; oauth-grant revoke.
