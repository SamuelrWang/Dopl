...
- **30-day trash retention.** `/api/cron/purge-trash` (daily, `requireCronSecret`-gated) hard-deletes rows with `deleted_at < now() - 30d` across ALL soft-delete tables (knowledge, skills, workflows, chats, ontology). It supersedes the old `knowledge-trash-purge` cron (removed). FK cascade handles children; `RETENTION_DAYS` is the single knob.
