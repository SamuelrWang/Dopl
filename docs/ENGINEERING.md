...
- **THE WEB SIDE'S WATCH LIST — RE-MEASURED 2026-08-08, and the old text here named the wrong file as the tightest.** ⛔ **ALL THREE FILES THIS PARAGRAPH NAMES ARE DELETED (2026-08-18) — `session-card.tsx` and the `lib/group-thread*` family at wiring plan Phase 5, `channel-pane.tsx` and `channels-view-core.tsx` at the Phase 12 cutover (INVARIANTS §5). Read it as the record of a correction, never as today's watch list; do NOT re-derive a current "tightest file" from it.** The lesson is what survives, and it is stated in the last sentence. *(Historical, 2026-08-08.)* It said `channel-pane.tsx` was 495; it is **493**. The tightest hand-written file in the web tree is `src/features/channels/components/session-card.tsx` at **495**, which this list had never mentioned, and `channel-pane.tsx` is second at 493. `channels-view-core.tsx` is **474 as of 2026-08-11** (it was 447 at this 2026-08-08 remeasure), not the 436 recorded at the end of the split that produced it (and not the 439 `eslint.config.mjs`'s own removal comment claims) — it took F-174's three-control popover wiring and F-178's `pending` threading in the same night. Note the pattern the correction exposes: **a file named on a watch list gets watched, and the file beside it grows past it unrecorded.** The desktop cluster below is the worked example of where that ends. ⚠ **And it happened again, in the other direction: the channels tree's tightest file today crossed the cap at a MERGE, where neither parent was over — see REFACTOR-FINDINGS F-225.** Re-derive the current list rather than reading a row: `find src packages \( -name node_modules -o -name dist \) -prune -o \( -name '*.ts' -o -name '*.tsx' \) -print | xargs wc -l | awk '$1>450 && $2!="total"'`.
...
| `src/features/channels/lib/group-thread.test.ts` | ⛔ **RETIRED 2026-08-18** (file DELETED with the `group-thread*` family, wiring plan Phase 5) | ✅ **UNDER CAP as of 2026-08-08 — was 983**, the largest test file in the tree. Split along its host's seams into four siblings (`-status` 421, `-pairs` 338, `-reopen` 235, `-render` 107). The row said "split by describe when next edited" for three remeasures; it was edited, and this time the split came with it. |
...
| `src/features/channels/lib/group-thread.ts` | ⛔ **RETIRED 2026-08-18** (file DELETED, wiring plan Phase 5 — an agent's run state stopped being transcript content; INVARIANTS §5) | ✅ **UNDER CAP as of 2026-08-08 — was 819, "the fastest-growing file in the tree: 621 → 819 in five days."** Split by reason-to-change: `group-thread-markers.ts` (176, the reserved metadata markers, read strictly), `group-thread-render.ts` (162, the card's read side), `group-thread-types.ts` (131, the render model), `group-thread-draft.ts` (127, the accumulator). The grouping state machine stayed WHOLE under §2's reducer carve-out — the split ran on the seams the file already had, not on a line count — and every public name is re-exported, so no importer changed. |
...
- **The summary is a distinct type, not the full type with empty arrays** *(the rule and its one-line reason are INVARIANTS §9)*. What is worth keeping here is the SHAPE that made it cheap: `OntologySummary` (mirrored in `packages/dopl-client/src/ontology-types.ts`) is structurally a subset of `OntologySnapshot`, so a render that reads only names accepts either.
...
- **WHY the model is approve-in AND approve-out** (the rule itself is INVARIANTS §6 — do not restate it here). Because the decision is a ROW rather than an in-process promise, the same request is answerable on **either** the web surface or the desktop's native dialog, and first answer wins; an in-process promise would have bound the decision to the machine that raised it, which is the whole thing v1.2 was undoing.
- **Trust is evaluated SERVER-SIDE ONLY, and the reason is an incident** (rule: INVARIANTS §6). `createConsentRequest` looks up the rule and returns the row already born `auto_allowed` (`decided_by='trust'`); the desktop keeps NO local trust cache — a stale cache let a just-revoked rule still auto-allow, and worse, made the desktop PATCH a still-pending row as `decided_by='web'`, i.e. fabricate a human decision. `auto_allowed` rows are recorded precisely so the audit filter (`status=decided|all`) can answer "your agent ran N times without asking you".
...
⛔ **THREAD CLOSING WAS REMOVED IN FULL ON 2026-08-18** (channels-v2 Phase 4; see the entry of that name in §8, and INVARIANTS §5). Everything below about propose-then-confirm, the human-only close lane, the proposal's re-raisable key, the two status transitions echoing, and the stale sweep's close proposals is HISTORY — the code is deleted and the MCP ops left the enum. **The prose/milestone half of this section is LIVE and unaffected in its RULE:** an agent still writes messages and milestones and nothing else, and the credential seam that enforces it is untouched (INVARIANTS §5). ⚠ **BUT ITS RENDERING HALF IS NOT — corrected 2026-08-18.** Two paragraphs below this banner name `splitSessionEntries`, `lib/group-thread.ts`, `substantiveEndBody`, `computeSummary`, `session-card` and `activity-event-row` in the PRESENT tense, and **every one of those is deleted** — the whole `lib/group-thread*` family and the session card went at wiring plan Phase 5, `activity-event-row.tsx` at the Phase 12 cutover. **What renders a terminal marker today is `channels/components/view-model-rows.ts › toReceiptRow` over `lib/message-receipt.ts › lifecycleReceiptStatus`: one slim muted line, `task_started` dropped unconditionally, the LABEL from the calm flag and never from the body** (INVARIANTS §5). The rules those paragraphs teach — status by FLAG never by string, a body is renderable when it says something — SURVIVED the renderer that taught them; the mechanism did not. Read every renderer name below as history.
...
⚠ **THE POPOVER IS GONE (2026-08-19, Samuel, live review) — THE FIVE RULES ABOVE ARE NOT.** The three-control split, the arm-is-not-a-preference heading, the one-value-per-channel arm, the `read_only` fallback and the grants-not-withholds copy all survived verbatim into `channels/components/settings-agent.tsx`, where every control is INLINE: the complaint was that a containment setting you have to click twice to READ is a containment setting nobody reads. What died with the chrome is only the drill-down — the option descriptions render under their option now instead of behind an `OptionPanel`, which is the same rule `permission-preset-row.tsx` already followed. `channel-settings-popover.tsx`, `channel-folder-control.tsx` and both their suites were deleted; the copy is pinned in `channels/components/settings-tab.test.tsx` (root) and `apps/desktop-ui/src/features/channels/settings-agent.test.tsx` (bridge + DOM, two-surface arm case included). **Current state: INVARIANTS §5.**

⚠⚠ **AND THE COPY WENT LATER THE SAME DAY — SAMUEL'S MINIMAL-COPY RULING (2026-08-19, live review, third ruling of that day). "WE SHOULD NOT BE EXPLAINING EVERYTHING TO THE USER."** The paragraph above is the one that aged: inlining every explanation produced a tab that read as documentation, and the fix for *nobody reads a setting behind two clicks* turned out not to be *print the manual under it*. **The rule now: on that tab a row is a NAME + a CONTROL, plus at most a few-word secondary line; no paragraph-style `text-caption` block anywhere.** What this SUPERSEDES, named rule by named rule — the rest of the section stands:
...
the same way** — race-proofing in the database, `created` returned so the caller knows whether THIS
call won — so the entry is kept as the argument for that shape rather than as a map of the
tree. ⚠ **AND THE ONE THING `created` WAS READ FOR IS GONE SINCE 2026-09-10**: it gated the
starter-corpus seed, and Samuel's *"drop seed content"* ruling made a fresh personal container empty,
so the flag is returned and read by nobody. The shape is still right — a caller with a real
first-mint side effect can read it again. Current state: INVARIANTS §4.
...
no gain. **What shipped instead is the READER** — `channels/components/view-model.ts ›
...
were rebuilt on the new header (`channels/components/channel-manage.tsx`), and
...
and it is the single reason the router-free `ChannelsCore` takes a `Link` prop.
...
⛔ **SUPERSEDED THE SAME DAY, 2026-08-18 — Phase 10 SHIPPED. Current state is INVARIANTS §11
(`app-windows.js`'s registry); do not read the paragraph below as a live constraint.** Samuel took
option (a): the sender binding was WIDENED from "the main window" to "an app-owned window in the
registry", and the pop-out thread window landed on it. The paragraph is kept because the argument it
records is the one the ruling answered, and because "what a phase deliberately did not do" is exactly
the kind of sentence that outlives its own deadline — this one by hours.

*(Historical.)* **Phase 10's pop-out thread window stays parked**, and it is parked on a security
...
oversight. ⚠ **REVERSED 2026-09-14** — Samuel: *"nothing should be named channels v2, it's
just channels now"*. The folder was folded up into `src/features/channels/components/` and
every `ChannelsV2*` symbol renamed; the paragraph below is the 2026-08-18 reasoning, not
the live layout (INVARIANTS §5). Route strings, the nav label, the deep-link key and `CHANNELS_PAGE` carry
no `v2`; `src/features/channels/components/` is a component-family name
under a feature already called `channels`, and renaming the whole directory to remove
a word would have moved every symbol anchor in INVARIANTS on the same day the surface
changed. (The file COUNT is a measurement and lives in §5 with its date — this
paragraph carried a bare "~28" and `pages/channels/index.tsx` a bare "27"; both were
already wrong by the review.) The rule the tree now runs on is written into §5 so the next reader does not
...
`main/ui-sync.js` sat at EXACTLY 500 lines and `channels-core.tsx` within thirty of
...
`channels-core.tsx` was the only thing that knew how to be a channel. The home page needed the
...
owns the reads, the derivations and the ONE `useChannelsLive` call, **so that a mount cannot forget
...
host; `channels-core.tsx` is the first.
...
## 2026-09-01 — The cap that was six: raising the ceiling a release valve had already made safe

**Samuel's ruling, verbatim:** *"im thinking to just increase the agent number to like 15 or
something. because 6 is too limiting. As long as we give agents the ability to end other agents
that would be useful. Because like my current setup, if I have an orchestrator agent, that agent
should be able to spin other agents, and then end them when they're not useful anymore."*

`session-windowless.js › MAX_CONCURRENT_SESSIONS` goes **6 → 15**, and the interesting part is not
the number — it is that **two of the three arguments for 6 had already expired and nobody had gone
back to check.**

- **THE ARGUMENT THAT EXPIRED FIRST: nothing could give a slot back.** A cap is the only backstop a
  population that cannot shrink itself has, and until `main/agent-self-ops.js` shipped `end_agent`
  on 2026-08-31 the only ways down from the ceiling were a settle or the operator's own hand. An
  orchestrator that can *hire* but not *fire* needs a low ceiling. One that can do both needs
  headroom instead. **The release valve landed one day before the cap moved, and it is what makes
  the raise a different decision rather than the same one made louder.**
- **THE ARGUMENT THAT EXPIRED SECOND: the memory multiplier.** §11's "every per-session bound is
  MULTIPLICATIVE against this number" is still true and still governs. But the term that made it
  BITE was the narration ring, and the 17 GB dev-RSS incident it caused was closed on 2026-08-30 by
  `session-narration.js › RING_CHAR_BUDGET` — the ring is bounded by CHARACTERS now, not only by
  entries, so its worst case is flat per session instead of `PROSE_CAP`-shaped. **15 × the bounded
  ring is smaller than the 6 × unbounded ring that actually broke.** The 2026-08-30 fix bought the
  headroom; this wave is the first thing to spend it.
- **WHAT ACTUALLY LIMITS THE BUILD NOW, stated so the next retune measures instead of guessing:**
  N live SDK child processes (each a real `claude` process with its own RSS) and API spend. Neither
  is bounded by this constant. If 15 turns out to be wrong it will be wrong for one of those two
  reasons — **do not reach for the ring again.**

**TWO OTHER NUMBERS HAD TO MOVE WITH IT, AND BOTH WERE FOUND BY ASKING "WHAT ELSE SAYS SIX".**
Neither is derived from the cap in code — deliberately, per each file's own header — which is
exactly why raising one meant revisiting the others by hand.

- **`launch-budget.js › MAX_CHAINED_LAUNCHES` 12 → 30.** At 12, an orchestrator staffing a single
  channel to the NEW cap is refused at its **thirteenth** worker inside the 15-minute window. The
  rate ceiling would have silently become the real cap, at 12, on the one lane the concurrency
...
docblock now states instead is the constraint that must actually hold: `SESSION_REPORT_MAX >
MAX_CONCURRENT_SESSIONS`, because the wire set is live-only and zod validates the ARRAY — a cap
raised past that bound would 400 the WHOLE push for a busy machine, unretryably.

### A test that pinned a DECISION rather than a PROPERTY

`test/session-park.test.mjs` asserted `REAL_MAX === 6` under the note *"Samuel's multi-machine
ruling: the number does not go up"* (F-272). It was the only red test in the suite and it was red
for the right reason — but it is the wrong SHAPE of test, and that is worth recording. **The
comment above it claims the case exists so the harness cannot agree with itself about a ceiling
production does not enforce — a property. The assertion underneath pinned a policy literal, which
told nobody anything about the code and failed the moment the policy changed.** It now asserts what
the comment claims: the ceiling is *parsed from production source* and is a positive integer. The
superseded ruling is recorded in `REFACTOR-FINDINGS.md` at F-272 rather than deleted, because
F-272's actual fix — `startResume` ENFORCES the ceiling instead of documenting +1 headroom past it
— is unaffected and true at any value.

### The Windows CI failure this wave finally paid off, and it was never about the cap

CI had been red on the last three pushes to master for **two unrelated causes**, one of which had
nothing to do with any code being wrong:

1. **`desktop app (node 22) → Lint`** — four files over the 500-line cap
   (`channel-prefs.js` 514, `session-io.js` 502, `session-state-push.js` 508,
   `test/session-state-push.test.mjs` 507). Already fixed by in-flight work in the tree; all four
   measured under the cap before this wave committed.
2. **`node 22 / windows-latest`** — `frame-palette.test.ts` failed with
   `expected '"use client";\r\n\r\nimport type…' to contain 'SECTION_PANEL_GROUND =\n  "border…'`.
   **The code was correct and the assertion was comparing against a differently-encoded copy of
   it.** The test is a SOURCE PIN: it `readFileSync`s a real source file and asserts it contains a
   multi-line literal. On a Windows runner git's default `core.autocrlf=true` checks that file out
   with CRLF, so **every `\n` in every expected literal misses by one invisible byte.**

⚠ **IT IS A CLASS, NOT ONE TEST.** There are ~27 source-pinning tests across `src/` and
`apps/desktop-ui/src/` (`grep -rl readFileSync`), and every one is a latent Windows failure the
moment it asserts across a line break. Fixing them one `.replace(/\r\n/g,"\n")` at a time is a bug
waiting on the next author to forget. **The fix is a repo-root `.gitattributes` with
`* text=auto eol=lf`** — it fixes the checkout, so it fixes all of them and everything written
after. **Zero churn, measured before adding:** `git grep -Il $'\r'` matched nothing, i.e. every
text file was already LF in the index; the file changes what a Windows working tree GETS, never
what is stored. `text=auto` (not a blunt `text`) keeps git's binary detection, and the binary
extensions are listed explicitly so no heuristic ever gets a vote on the `.icns` app icon.
`frame-palette.test.ts` keeps a local normalization as the belt: **a test that reads source must
not depend on how the reader's git is configured.**

**THE LESSON THIS BOUGHT, AND IT IS THE ONE THE LOCAL GATES KEEP RE-TEACHING:** every gate in the
"definition of green" passed on a Mac while CI was red, because the failure was a property of the
