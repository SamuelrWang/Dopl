...
- **Known name clash (tracked, F-081).** The `ChannelThread` COMPONENT (`components/channel-thread.tsx`) is the whole channel detail pane and predates this round; the `ChannelThread` TYPE is one exchange. They compile together only because a type-only import never collides with a local value. A comment at the import says so. Renaming the component (→ `ChannelPane`) and `MessageThread` (→ the transcript) is deferred.
