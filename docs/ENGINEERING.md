...
- **`signOut()` tears down THREE credentials, not two.** The Supabase blob, the cookie jar, and — new in Q5 S2 — the **MCP device token**, which is a separate 90-day `dopl.read`+`dopl.write` bearer living in electron-store (`mcpDeviceToken` / `…Plain`) AND in `userData/mcp-spawn.json`. `mcp-config.clearDeviceToken()` removes both copies and the memoized in-process one. **This teardown is LOCAL ONLY: `/api/auth/mcp-device-token` is mint-only, so the token stays valid server-side until it expires or is revoked from the web "Connected apps" list (F-085).** The user-scope `dopl` entry in the CLI's own config is likewise not touched.
