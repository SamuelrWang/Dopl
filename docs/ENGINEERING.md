...
- **MCP tool `dopl_channel`** (`packages/mcp-server/src/tools/channel*.ts`) — ops `list` / `open` (create; `open{direct:true, member}` opens a DM, see v1.5) / `invite` / `post` / `read` / `await` / `list_tasks` / `get_task` (both READ, added v1.8) / `create_task` / `close_task` / `set_task_mode` (the three task-write ops added in v1.5). `read`+`await` are the listener loop (learn the latest seq, then re-issue `await` from the last seq processed). `@dopl/client` has the matching methods and the parity test is wired. `read`/`await`/`list_tasks`/`get_task` pass the ref straight to the route (no `listChannels` on the poll loop); `invite`/`post` still pre-resolve the channel via `listChannels` (F-055).
...
#### v1.8 additions — MCP task-read + create_task idempotency (2026-07-28)
...
- **Task READ ops.** `list_tasks` (`channel`) and `get_task` (`channel`,`task`) — the first way to enumerate/inspect tasks over MCP (previously agents could create/close by id but not discover them). `list_tasks` reuses the existing `GET /tasks` + `listChannelTasks`; `get_task` adds `GET /tasks/[taskId]` + `getChannelTask(ctx, channelId, taskId)` (404 for a task not in the channel — id unprobeable). `@dopl/client` gains `listChannelTasks`/`getChannelTask`; handlers `opListTasks`/`opGetTask` render readably (task list / detail block) through the `_dopl_status` footer. Both are **READS** — added to `READ_OPS.dopl_channel` in `parity.test.ts` (NOT `WRITE_OPS`, NOT `sessionOnly`); `KNOWN_*_DRIFT` stays `{}`.
- **`create_task` idempotency.** `create_task` accepts `client_msg_id` (like `post`). Server-side dedup mirrors `channel_messages`: nullable `channel_tasks.client_msg_id` + partial unique index `(channel_id, client_msg_id) WHERE client_msg_id IS NOT NULL`; `createTask` (`service-tasks.ts` since v2.6) returns the existing task on a `client_msg_id` hit (and on a 23505 insert race) **without re-posting the initial request** — so a retried create can't double-create the row OR double-spawn the responder window. Wired through `TaskCreateSchema`, `POST /tasks`, `repository-tasks.findTaskByClientId`, `@dopl/client.createChannelTask`, and `opCreateTask`.
...
- **Description clarifications (agent-facing, no behavior change).** `CHANNEL_DESCRIPTION` now warns (1) in a live desktop **session window** only `op=post` to the session's OWN channel runs un-prompted — every other op may raise an operator Allow/Deny; (2) if the counterparty's replies already arrive as new turns (desktop-run session), do NOT call `await` (await is only for a standalone listener loop).
- **Tests.** `service-reads.test.ts` (list/get + get 404), `service-tasks.test.ts` (create_task idempotency: dedup returns same task with no re-insert/re-post, first-send inserts+posts once, unique-race convergence), `channel-ops.test.ts` (list_tasks/get_task render, get_task 404, opPost bad-task mapping). Root suite 591 green; MCP parity 52 green, zero drift.
