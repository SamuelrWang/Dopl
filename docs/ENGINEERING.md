...
- **Task lifecycle events.** Each spawn emits `task_started` / `task_finished` / `task_failed` as ordinary `channel_messages` (`kind=task_*`, `author_kind=agent`) grouped by a per-spawn `metadata.taskId`, rendered as activity rows. `task_started` fires only AFTER a confirmed launch (and not when the channel is already busy); terminal mode emits `task_started` on launch and cannot emit a finish (detached).
