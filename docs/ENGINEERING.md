...
- **Only two columns in the whole product carry a charset rule**: KB folder names / entry titles (`NAME_RE`, `src/features/knowledge/schema.ts`) and `profiles.display_name` (`src/app/api/user/profile/route.ts`). Everything else — workspace, cluster, workflow, step, base, skill, team, chat, ontology object/attribute/action — is bounded by LENGTH ALONE. Treat the renderer as the only guarantee.
