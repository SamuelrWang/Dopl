...
**Every user gets one container.** `kind='personal'`, minted by `ensure_home_space` with an
owner membership and a one-per-owner unique index; the `home_scoped` shelf moves into it by
`created_by`. Dual-write behind `TENANCY_HOME_SPACE` (default **off**) — the column still
carries the truth.
...
5. ⚠ **Replay is SKIPPED — Docker is down** (`docker info` fails), as it has been for all of wave A
   ...
   trigger does: F-461's grant probes and F-563's four home-space claims are still owed.
...
| **B14** default workspace off | `ensureDefaultWorkspace` → `ensureHomeSpace`; `findDefaultWorkspaceForUser` → `findSoleOwnedStandardWorkspace`, billing-only; both `resolveHomeScope` copies repointed; `20260802200000` + `20260823160000`'s guard dropped; `default_workspace_of()` — ⚠ **NOT "for the revert only": it is a LIVE DEPENDENCY of `ensure_home_space`**, which mints a container FROM today's default and reads it for the name and `created_at` (corrected 2026-09-02, in review). Dropping it before that mint stops running is a `CREATE OR REPLACE` away from an apply failure, not a tidy-up |
...
| `home-space.ts` — flipping the flag ON HID every row written before it | `d7758ae3` — the read stops asking the flag (F-590) |
