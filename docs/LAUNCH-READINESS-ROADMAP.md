...
- Realtime events trigger wholesale refetch amplification: any channel event fires **four full reads** (`channels-view-core.tsx`, deleted 2026-08-18 — the same shape lives on in `components/channels-v2/channels-v2-core.tsx › refetchRef`, now five reads); "doorbell, never content" policy is defensible for RLS but DL-030 was rejected, not fixed. (DL-031 — the 82.7%-of-DB realtime problem — IS fixed: ref-counted shared channel registry + one desktop socket + publication trimmed to 22 tables.)
