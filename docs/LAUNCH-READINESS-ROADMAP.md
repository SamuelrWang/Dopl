...
| **`CRON_SECRET` unset in Vercel** (per `api/cron/stale-threads/route.ts:44-48`) → all four cron jobs answer 503 → **trash never purges, seats never reconcile**. Verify live env before launch | P2 (operational) | S |
