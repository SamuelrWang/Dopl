...
- Realtime events trigger wholesale refetch amplification: any channel event fires **four full reads** (`channels-view-core.tsx`, deleted 2026-08-18 — the same shape lives on in `channels/components/live.ts › useChannelsLive`, now five reads, which the core hands it as `refetchAll`; the ref that held it lived in `channels-core.tsx` until the Phase 10 split); "doorbell, never content" policy is defensible for RLS but DL-030 was rejected, not fixed. (DL-031 — the 82.7%-of-DB realtime problem — IS fixed: ref-counted shared channel registry + one desktop socket + publication trimmed to 22 tables.)
...
1. Create-channel selects an id not yet in the list → wrong channel shown, then jumps (`channels-view-core.tsx`, deleted 2026-08-18; the surviving create path is `channels/components/channels-core.tsx › onCreated`).
