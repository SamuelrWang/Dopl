...
**Scope note:** canvas, workflows, and configuration pages are excluded from UX/perf findings. ✅ **The retirement is DONE as of 2026-08-07** (phases 1–7, in the working tree — see [RETIREMENT-UNWIRING-PLAN.md](RETIREMENT-UNWIRING-PLAN.md) §STATUS): the three pages are unrouted and their four MCP tools no longer register, so no finding below can be reached through them. Their API routes stay open on purpose (D3) and were security-checked anyway — all authenticated (`withWorkspaceAuth` + `minRole: "member"`). ✅ **MIGRATIONS: NOTHING IS PENDING (measured against production 2026-08-11, `supabase migration list --linked`: 157 local files = 157 remote history rows, zero drift).** This line has been corrected twice — it said "three unapplied" on 2026-08-07 and named a different unapplied set on 2026-08-08 — and both corrections themselves went stale, which is the lesson: **deploy state is a measurement, not a fact a doc can hold.** Everything through the 2026-08-10 security wave (`20260810140000`) is applied; the F-167 renames were reconciled before the 2026-08-09 push and never re-applied. Do not trust this sentence either: run `supabase migration list --linked` before reasoning about deploy state.
...
✅ **AND THE REMAINING-SITES WORK IS DONE TOO (2026-08-08).** This item said "the ~80 remaining write sites (chats, ontology, members, channels' own lifecycle writes) are now one config each" — all four are converted:
   ...
   Four rules came out of it and are in ENGINEERING §7 (5–8): merge-never-replace when the response is narrower than the cache; a feature's READS must be on `useApiQuery` before its writes adopt the layer; `CREATE_RESOLVE` for server-minted-id-plus-instant-render; `pendingRow` on a CONTROL is what closes toggle races. Original diagnosis (⚠ **`channels-view-core.tsx` was DELETED at the v2 cutover, wiring plan Phase 12, 2026-08-18** — the line numbers below are dropped because they resolve to nothing; the diagnosis is kept as the record of what the layer was built against):
   - `src/features/channels/components/channels-view-core.tsx` — `await postMessage()` (response contains the created message, **discarded**) → `await refetchMessages()` (re-downloads the full 200-message page) → `void refetchChannels()`. User's text sits in the composer through all of it.
...
| ⛔ **MOOT 2026-08-18 — the route is DELETED** (wiring plan Phase 4: its whole body was a close proposal, and threads no longer close). ~~`stale-threads` per-row insert; correct form is `.upsert(…, { onConflict: "channel_id,client_msg_id", ignoreDuplicates: true })`~~ | P2 | — |
...
- Realtime events trigger wholesale refetch amplification: any channel event fires **four full reads** (`channels-view-core.tsx`, deleted 2026-08-18 — the same shape lives on in `components/channels-v2/live.ts › useChannelsV2Live`, now five reads, which the core hands it as `refetchAll`; the ref that held it lived in `channels-v2-core.tsx` until the Phase 10 split); "doorbell, never content" policy is defensible for RLS but DL-030 was rejected, not fixed. (DL-031 — the 82.7%-of-DB realtime problem — IS fixed: ref-counted shared channel registry + one desktop socket + publication trimmed to 22 tables.)
...
Full trace: composer "Request" mode → Send → `setSending(true)` dims the button → **2 serial network hops with zero other pixel change** (POST creates the thread, response discarded; then full messages refetch with `keepPreviousData` deliberately holding the old list) → SessionCard finally appears → auto-scroll → "Working…". Fix = §2.1. ⚠ **The three adjacent weak controls this row used to name are DELETED, not fixed** — "Open thread" on the session card, the session pill's "Open" (no busy state at all) and the thread popover's close/complete/failed/reopen went with `session-card.tsx`, `session-pills-bar.tsx` and `thread-panel.tsx` in wiring plan Phases 4-5 (2026-08-18). The replacement surface is the v2 right panel's Agents tab, and its controls (pause / end / open window) are a fresh feedback question, not this one carried over. The pending-request decision (`src/features/channels/components/launch-panel.tsx › LaunchPanel`, which replaced `consent-card.tsx` in wiring plan Phase 8 — the button now reads **Launch agent**) is already genuinely optimistic — the in-feature reference.
...
1. Create-channel selects an id not yet in the list → wrong channel shown, then jumps (`channels-view-core.tsx`, deleted 2026-08-18; the surviving create path is `components/channels-v2/channels-v2-core.tsx › onCreated`).
...
Fixes: make `PageLoading` render a shape; adopt the **orphaned** `TwoPaneListSkeleton` (`src/shared/ui/skeleton.tsx:108` — built for exactly knowledge/chats/skills/members, used by nothing) on those pages; transcript skeleton for chats detail (`detail-pane.tsx:190`); members console **resolved 2026-08-19**: the v2 console (`src/features/members/components/members-v2/members-v2-view.tsx`) renders `TwoPaneListSkeleton` on cold arrival, so the false "No members yet." empty state is gone; dedupe the two hand-rolled skeleton clones (`channels-skeleton.tsx:11-33`, `doc-pane-chrome.tsx:19-34`) into the kit; `CheckoutSkeleton` (`embedded-checkout.tsx:225-238`) is the quality bar.
