...
### F-067: A failed consent PATCH from the notification Allow action is silent — ✅ RESOLVED (2026-08-10)
- Location: `dopl-desktop-app/main/consent.js:149-151` (the three constants), `:153-169` (`patchDecision`), `:213-224` (`submitDecision`, the entry point); callers `main/trigger.js:230`, `main/trigger-headless.js:133`, `main/session-consent.js:199,202`
- Found during: Channels consent redesign (desktop v1.4, 2026-07-27)
- Severity: smell (silent failure — no operator signal)
- ✅ **RESOLVED 2026-08-10.** Tests: `dopl-desktop-app/test/consent-decision-signal.test.mjs` (NEW).
- **THE FIX IS THE THREE-VALUED OUTCOME, not the notification.** `patchDecision` used to collapse every non-ok answer to `false`, and every caller dropped it on the floor. A boolean cannot carry the one distinction that matters here, because the benign case and the broken case are BOTH "not ok". It now returns `DECISION_OK` (the server recorded our decision) / `DECISION_SETTLED` (409 — someone already decided) / `DECISION_FAILED` (it did not land). The notification is downstream of getting that distinction right.
- **`submitDecision` is the single entry point, and all four decision call sites use it** — `trigger.js` (notification Allow), `trigger-headless.js` (headless Allow), and `session-consent.js` in-window Accept AND Deny. It never rejects, so a notification-action callback can fire-and-forget it without an `await` or a `.catch` and still not produce an unhandled rejection. Only `DECISION_FAILED` notifies; success and 409 are both silent.
- **The 409 is silent BECAUSE it is the only 409 the route emits.** `http-mapping.ts:80` maps `ConsentAlreadyDecidedError` → 409 `CONSENT_ALREADY_DECIDED`, and the server CASes on `status='pending'`, so a 409 means the web list (or the other surface) won the race. Re-notifying there would tell the operator something broke when their own click is what answered it. **A 404 is deliberately `failed`, not `settled`** — the row was hard-deleted under us, the request will never run, and silence is the exact bug this finding is about.
- **RESIDUAL, and it is the reason the web list stays load-bearing: the notification is best-effort.** `notifyDecisionFailed` returns `null` when `Notification.isSupported()` is false and swallows a constructor or `show()` throw, and an OS that suppresses notifications (Do Not Disturb, Focus, a revoked permission) loses the signal with no fallback. The diag line still records it. **So the web Pending Requests list remains the only GUARANTEED recovery path** — this fix raises the odds the operator hears about a failed decision, it does not guarantee it. Do not delete the web list on the strength of this entry.
- Status: **resolved** (residual noted: notification delivery is best-effort, not guaranteed)
...
### F-100: `op="members"` lets an agent walk any PUBLIC channel and dump display names and EMAILS — ✅ CLOSED FOR THE MCP SURFACE (2026-08-10)
- Location: `packages/mcp-server/src/tools/channel-render.ts:409-420` (`formatMemberLine`, now taking `callerIsAdmin`), called at `channel-ops-read.ts:435` from `opMembers` (`:414`); threaded from `server.ts:226` → `channel.ts:109,217`
- Found during: the N-party wave review (2026-07-31)
- Severity: question (owner call) — **it was the entry in this file with the largest data-exposure surface**
- ✅ **CLOSED FOR THE MCP SURFACE 2026-08-10 — the first of the two proposed resolutions, entitlement-scoped rather than dropped outright.** `formatMemberLine` computes `emailAllowed = callerIsAdmin || isSelf` and falls back to the email ONLY then; every other member renders as name + id + role, and a name-less member renders **by id alone**, never by email. `isAdmin` is threaded from the boot status ping through `createServer` → `registerChannelTool` → `opMembers` → `formatMemberLine`, and **defaults to `false` at every hop** (`channel.ts:109` `isAdmin = false`, `server.ts:226` `options.isAdmin ?? false`, both function params `callerIsAdmin = false`) — so a boot ping that failed to resolve the flag scrubs rather than leaks. Fail-closed by default value, which is the only way a defaulted boolean is safe. Pinned by `channel-addressing.test.ts` (member vs. admin rendering asserted against the same roster).
- **The enumeration itself is UNCHANGED and that was deliberate.** `listChannels` still ORs `visibility.eq.public` (`src/features/channels/server/repository.ts:48`), so an agent can still walk every public channel and list who is in it. What it can no longer do is harvest the PII. The second proposed resolution — gate `op="members"` on membership rather than visibility — was not taken.
- ⚠ **RESIDUAL, and it is a deliberate asymmetry worth stating plainly: the WEB roster still shows email to any member.** The MCP surface is now strictly TIGHTER than web. That is accepted, not an oversight — the agent is the better enumerator and was the actual finding — but it means the two rosters no longer render the same workspace the same way, and `packages/mcp-server/src/tools/members.ts:176` already carries a comment about that parity. **Closing the gap means the same admin-or-self scrub on the web `/members` DTO**; until someone does it, do not "restore parity" by loosening the MCP side.
...
- Status: **closed for MCP** (residual: the web roster DTO is unscrubbed, accepted)
...
**Open, deliberately not closed here** (one of the two closed 2026-08-10; the M4 wiring is all that survives)**:**

- ~~**`coldKeys` is a second copy of `use-chat-writes.ts`'s `ifCold`.**~~ ✅ **CLOSED 2026-08-10.** Promoted to `coldKeys(client: QueryClient, keys: QueryKey[]): QueryKey[]` in `src/shared/hooks/use-api-mutation.ts:121-128`, beside `patchCache`. Both local copies are deleted: `use-thread-writes.ts` and `use-chat-writes.ts` now import it (`:5` in each), and **`ifCold` no longer exists anywhere in `src/`** — grep returns zero hits, so the name cannot be revived by accident. Callers are `use-chat-writes.ts:156,195,258,292` and `use-thread-writes.ts:181`. **Named for what it RETURNS** — the subset of `keys` that is cold — because the result is spread into an `invalidate` list rather than branched on, which is why the chats name (`ifCold`, a predicate's name) was the one dropped.
...
## F-186 — `members-tab.tsx` carries two `@deprecated` no-op props waiting on a `shared/` owner — ✅ RESOLVED (2026-08-10)

- Location: `src/features/members/components/members-tab.tsx` (both props deleted) and `src/shared/layout/settings-modal/sections/members-section.tsx` (both arguments deleted)
- Found during: the members write-layer conversion (13 writes)
- Severity: smell (dead surface)
- Description: the conversion to `useApiMutation` made both props redundant — the cache patch is the notification now — but they were still declared, still passed by callers, and still did nothing. A prop that exists and does nothing is worse than one that was deleted: the next reader wires it up and gets silence.
- ✅ **RESOLVED 2026-08-10, fully — both ends in one change, which is the only way this was ever going to be safe.** `onChanged` and `onInvitationsChanged` are gone from `MembersTab`'s `Props`, and the two arguments are gone from the `MembersSection` call site. **The deletion also removed what fed them**: `useMembers` and `useTeams` no longer destructure `refresh` at all, so there is no orphaned refresher left behind to tempt a re-wire. `useInvitations`'s `refresh` DOES survive and is correct to — `InviteDialog` is not converted to the mutation layer and still needs it. A comment at the call site now states why the roster and teams have no post-write refresh (the writes patch the very caches these reads render from).
- **The `shared/`-owner problem it was opened for turned out to be the whole cost.** The finding sat open not because the change was hard but because a feature-scoped wave could not reach `shared/`; given an owner spanning both, it was the estimated size — `members-tab.tsx` +0/−13, `members-section.tsx` +6/−7, the additions being the explanatory comment rather than logic.
- Status: **resolved**
...
## F-188 — The consent inbox was not workspace-scoped: the sidebar badge counted OTHER workspaces' pending requests (2026-08-10) — ✅ RESOLVED

- Location: `src/features/channels/server/repository-collab.ts:79-115` (`ConsentListOpts`, `listConsentRequests`); `server/consent-service.ts:245-259` (the caller that threads `ctx.workspaceId`); `hooks/use-consent-inbox.ts` (the docblock that asserted the wrong bound)
- Tests: `src/features/channels/server/consent-service.test.ts`
- Found during: **user report** — a pending request raised in workspace B showed while viewing workspace A
- Severity: **bug (shipped, user-visible, cross-workspace leak of existence)**
- Status: **resolved.** No schema, no migration, no new idiom.

**WHAT WAS WRONG.** `repository-collab.listConsentRequests` filtered on `operator_user_id` alone. A `channel_consent_requests` row carries `operator_user_id` and **nothing else that identifies whose workspace raised it**, so an operator-only filter returns that operator's pending rows from EVERY workspace they belong to. The read runs under `supabaseAdmin()` — the service-role client — so **RLS was not a backstop**, and RLS would have scoped to the operator anyway, which is not the same bound.

**THE USER-VISIBLE PATH.** `apps/desktop-ui/src/components/app-shell/app-shell.tsx:83` calls `useConsentInbox` and passes `consentCount={consentRequests.length}` into `AppSidebarCore` (`:153`), which renders the pending badge at `app-sidebar-core.tsx:128-135`. The badge is always mounted, so the wrong count was on screen continuously, not only on the channels page.

**THE CLIENT WAS ALREADY CORRECT — THIS WAS PURELY THE SERVER READ.** Both the TanStack query key and the realtime subscription filter were already workspace-scoped. That is precisely why it survived: every layer a reader would think to check looked right, and the one that was wrong is the one nothing pointed at.

**FIX: `workspaceId` is a REQUIRED field on `ConsentListOpts`, not an optional one.** That is the whole design decision. An optional `workspaceId` fixes today's caller and lets the next one forget; a required argument makes the omission a type error at every future call site. `consent-service.listConsentRequests` threads `ctx.workspaceId`. **`listTrustRules` in the same file has always filtered on both columns** — this is that shape restored, not a new invariant, which is the strongest evidence it was an oversight rather than a design.

**⚠ THE REUSABLE LESSON, and the reason this is written at length: A COMMENT ASSERTING WHERE A FILTER LIVES IS NOT EVIDENCE THE FILTER EXISTS.** `use-consent-inbox.ts` claimed *"RLS scopes the stream to the operator's own rows."* That was **wrong twice** — the list read runs under the service role so RLS does not apply at all, and operator-scoping was never workspace-scoping even where RLS does apply. Anyone auditing the inbox read that sentence and moved on. The docblock is corrected in place and now names the file the bound actually lives in, plus the instruction not to restate a filter's location from memory. **This is the same class as this file's own documented failure mode — entries verified against other entries rather than against code.** When a comment tells you a constraint is enforced somewhere else, open that somewhere else.
