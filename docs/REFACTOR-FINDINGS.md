...
- **The other MCP tools were NOT swept.** `dopl_kb` / `dopl_skill` / `dopl_workflow` / `dopl_ontology` / `dopl_chats` splice `${x.name}` / `${x.title}` into narration the same way (e.g. `chats.ts:404` `# ${chat.title}`). Lower reach — those are the caller's OWN workspace objects, not a cross-user channel — but a shared workspace has multiple authors, so this is the same class one blast radius down. **Not in scope tonight; worth its own round.**
