...
- Description: five items deliberately deferred out of the v1.5 round (first-class tasks, direct channels, engagement mode). Two HIGH DM-invariant bugs + a suppression-binding gap were caught and FIXED in-round (the soft-deleted-DM revive path and the DM membership/visibility immutability enforcement; the `task-reply` verdict binding to `taskTarget === author`); the items below are what was left for later.
  - **1. Threads never auto-close (rot `active`).** No lifecycle closes a `channel_tasks` row — `status` stays `open` until an agent (or human) explicitly calls `close_thread` / `PATCH …/tasks/[taskId] {op:"close"}`. An agent that finishes but skips the close leaves the thread reading `active` forever (the web overlay maps `open → active`); there is no timeout, no close-on-`task_finished`, and no sweep. So a channel accretes perpetually-"active" threads that no one closed.
...
### F-082: Two stale claims corrected in `docs/ENGINEERING.md` §8 (both were actively dangerous)
- Location: `docs/ENGINEERING.md` §8 "Channels" (the v1.1 addressing bullet and the v1.9 Session Window bullet), `src/features/channels/server/service-writes.ts:261`, `dopl-desktop-app/main/session-profiles.js`
- Found during: six-agent audit + contract v3.0 Track B doc pass (2026-07-30)
- Severity: bug (documentation, but load-bearing — agents act on this doc instead of re-reading the code)
- Description: two claims in §8 had drifted from the code, in the unsafe direction. Both are now corrected in place with an explicit "this used to say X, and X is false" note so a future session cannot re-derive the old rule.
  - **(1) The v1.9 shadow rule said own-channel `post` is pre-approved.** False since v2.5 D2 / FIX H1: `dopl_channel` is in NEITHER `allowedTools` NOR `disallowedTools` on any profile, so every channel op reaches `canUseTool`; `isOwnChannelPost` only selects the op-scoped grant key. A session trusting the old text would re-introduce the cross-user exfiltration path H1/D2 closed (read a local file, post it to a peer, no operator click). The same false claim was echoed in the v1.8 "description clarifications" bullet AND in the shipped `CHANNEL_DESCRIPTION`, so agents were planning against an inverted cost model; the tool description is rewritten too (see §8 v3.0).
  - **(2) The v1.1 bullet said `authorKind` derives from `ctx.source`.** It never has: `service-writes.ts` computes `input.authorKind ?? (ctx.source === "agent" ? "agent" : "user")`, so an explicit body value WINS. This is load-bearing rather than a bug — the desktop peer-post path posts with `authorKind:"agent"` over a cookie session — but documenting it as credential-derived invites treating it as an authentication fact. Corrected to: caller-assertable within `user | agent`, `author_user_id` always `ctx.userId` (identity not spoofable), `system` route-schema-rejected.
- Proposed resolution: fix-now — done. Both corrected in this pass, and the `authorKind` half is now pinned in BOTH directions plus "identity stays `ctx.userId`" (`server/service-writes.test.ts`, "postMessage — addressing + author derivation"), so a future "harden this" edit cannot quietly turn the `??` into a hard derive and break the desktop peer-post path. The gate half has no equivalent test on this side of the repo: nothing in the web/MCP tree can assert the desktop's `canUseTool` behavior, so the shadow-rule claim is guarded only by the desktop's own suites (contract v2.9 invariant tests) plus the correction note in §8.
- Status: open (kept one cycle per the F-042/F-043 precedent — the fixes are landed; delete on the next prune)
...
### F-084: Cookie-jar trust was asserted, not enforced (`auth-cookies.js` filtered by NAME only)
- Location: `dopl-desktop-app/main/auth-cookies.js` (`readCookieSession`, `getSessionCookieHeader`), `dopl-desktop-app/main/auth-actions.js` (`isAppUrl` → `isAppOrigin`), `dopl-desktop-app/main/index.js` (`will-navigate`)
- Found during: Q5 adversarial review of the Q4 auth round (2026-07-31) — item S3
- Severity: bug (security posture; the false claim was the sole justification for skipping a CSRF nonce)
- Description: `auth-cookies.js`'s header comment claimed the jar is "writable only by our own origin", and `auth-state.rebuildBlobFromCookieSession` cites exactly that to adopt a session out of the jar with NO pending-auth nonce. The claim was false twice over. (1) Both readers filtered `cookies.get({url: APP_ORIGIN})` by cookie NAME only, and that call returns every cookie VISIBLE to the URL — including `.usedopl.com` DOMAIN cookies, which any sibling subdomain can set. (2) `isAppUrl` admitted `usedopl.com` and every `*.usedopl.com` host for IN-WINDOW navigation, so such a page ran inside the app window; it also gated `maybeBeginAuth`, which appends our login-CSRF nonce as `?state=` — handing that nonce to a host we do not control. Also fixed alongside: `readCookieSession`'s `JSON.parse` was unguarded, so a torn/duplicate chunk set threw out of the function and every caller logged it as a jar-read FAILURE rather than "nothing usable".
- Proposed resolution: fix-now — done. `isOurAuthCookie` requires HOST-ONLY for the `APP_ORIGIN` host (leading dot stripped, then `===`, so `.usedopl.com` fails) and is used by both readers; `isAppOrigin` (exact origin) replaces `isAppUrl` at both call sites; the parse returns `null`; the header comment now states the real, enforced guarantee. Tests: `test/auth-signed-in.test.mjs` pins the mechanism (both readers use the predicate, the predicate keys on `APP_HOST`, the name-only filter is gone, the loose predicate has no call site left) rather than the prose it replaced.
- Status: open (kept one cycle per the F-042/F-043 precedent — landed; delete on the next prune)
...
### F-086: The two credentials were never compared, so a foreign cookie jar could overwrite the operator's blob
- Location: `dopl-desktop-app/main/auth-state.js` (`signedInFrom`, `rebuildBlobFromCookieSession`, `identityMismatch`), `dopl-desktop-app/main/channel-listener.js` (`reconcileInner`)
- Found during: Q5 adversarial review (2026-07-31) — item S4
- Severity: bug
- Description: `signedInFrom` returned `source:'both'` whenever a stored blob and a fresh cookie identity both existed, with no check that they name the same user, and `rebuildBlobFromCookieSession` adopted purely on `exp`. A jar belonging to a DIFFERENT account therefore only had to be newer to overwrite the signed-in operator's blob — after which `getUserId()`, the listener's cached `myUserId` and the Realtime `'stored'` candidate were all routing one account's channel traffic under the other's identity, with nothing in the log saying so. Compounding it, `channel-listener` resolves `myUserId` once and caches it for the life of the process (`if (!myUserId)`), so whichever identity resolved first stuck.
- Proposed resolution: fix-now — done. `signedInFrom` takes `blobUserId` and reports `mismatch` + `source:'conflict'` (still `signedIn`, because a usable credential does exist); the rebuild refuses to adopt across a mismatch and diags it loudly; `auth.identityMismatch()` is re-exported and `reconcileInner` nulls `myUserId` each pass while the conflict lasts, so the identity is re-resolved rather than assumed. An unreadable id on either side is treated as ignorance, not disagreement (fails toward the old behavior). Truth table in `test/auth-signed-in.test.mjs`.
- Status: open (kept one cycle — landed; delete on the next prune)

### F-087: The legacy-thread registry was a broad,never-expiring consent suppressor
- Location: `dopl-desktop-app/main/targeting.js` (LEGACY-THREADS block: `noteMyLegacyThread`, `knownLegacyReply`)
- Found during: Q5 adversarial review (2026-07-31) — item S5
- Severity: bug
- Description: a registry hit downgrades an inbound reply to `'task-reply'` — a passive banner with NO consent row, no gate and no Accept. It recorded EVERY addressed own message, and since v2.6 the server auto-addresses every post in a DM, so in the ordinary 1:1 case that is literally every message the operator sends, each minting a legacy id. Meanwhile `metadata.taskId` remains caller-settable for a legacy id with no participation check (F-083 leaves legacy ids un-gated for threading). A peer could therefore stamp a genuinely NEW request with an old legacy id of mine and have it silently suppressed for as long as the process lived — entries had a 500-item cap but no expiry.
- Proposed resolution: fix-now — done. Only an OPENER (a message carrying no inbound taskId) records an entry, which still covers multi-turn because the legacy id is derived from the opener's seq and every later turn carries that id; entries carry `at` and expire after `LEGACY_THREAD_TTL_MS` (6h), dropped on read; the cap stays as the memory bound; `nowMs` is injectable so the TTL is a truth table. Every miss still fails toward `'trigger'`. The real fix for the wire half is server-side participation gating for legacy ids, which is F-083's third bullet.
- Status: open (the desktop half landed; the server half stays with F-083)

### F-088: Refresh amplification — no single-flight on `refresh()`, and the cookie jar went empty mid-write
- Location: `dopl-desktop-app/main/auth.js` (`refresh` / `refreshInner`, `ensureFresh`), `dopl-desktop-app/main/auth-cookies.js` (`writeSessionCookies`), `dopl-desktop-app/main/listener-io.js` (`listChannels`, `listChannelsWithRetry`)
- Found during: Q5 adversarial review (2026-07-31) — item S6
- Severity: bug
- Description: three independent pieces composed into one path from a transient failure to a real sign-out. (1) Only `getAccessTokenInfo` was cooldowned; `ensureFresh()` — the listener's per-workspace enumeration-retry path and `getAuthCookie`'s repair branch — called `refresh()` directly with no coordination. Supabase ROTATES the refresh token on use, so N concurrent loops meant one winner and N-1 400s, and the 400 handler DROPS the stored blob. (2) `writeSessionCookies` cleared the jar and THEN set the chunks, so across that await the jar was empty and every concurrent reader saw "signed out" and kicked its own repair. (3) `listChannelsWithRetry` ran the refresh dance after EVERY failure, so a 500 or a dropped socket rotated the token for no reason. Net: one transient 5xx across N loops could clear the session.
- Proposed resolution: fix-now — done. `refresh()` is a single-flight wrapper around `refreshInner()`; `writeSessionCookies` writes every chunk first and then prunes only the names the new session does not occupy; the retry ladder repairs the session only on an AUTH-shaped (401) failure.
- Status: open (kept one cycle — landed; delete on the next prune)

### F-089: Self-heal had a rate bound but no total bound (`loop=miss` reconciled every 60s forever)
- Location: `dopl-desktop-app/main/listener-heal.js` (`createReconcileHealer.onLoopMiss`, `MISS_GIVE_UP_COUNT`), `dopl-desktop-app/main/channel-listener.js` (`lastGoodWorkspaceIds`)
- Found during: Q5 adversarial review (2026-07-31) — items S7 + S8
- Severity: smell (wasted work + a permanent oscillation; no data loss)
- Description: two "forever" loops. (S7) A realtime wake for a channel with no loop re-enumerated at most once per 60s window, but nothing capped the TOTAL — so a channel that legitimately has no loop (the operator was removed, it was archived under a still-subscribed topic, the DTO hides it) drove a full reconcile every 60 seconds for as long as the app ran, each pass correctly re-deriving a set that still did not contain it. (S8) `lastGoodWorkspaceIds` was assigned only when the enumeration returned a NON-EMPTY set, so an operator who left every workspace kept the stale set forever and `shouldReapplyWorkspaces` re-subscribed push to workspaces they are no longer in on every 5-minute pass, permanently.
- Proposed resolution: fix-now — done. `MISS_GIVE_UP_COUNT` (5) caps miss-triggered passes per channel, with one escalation log; a parked channel does not even consume the window, and `healer.stop()` (a listener restart) resets it. Giving up on the SELF-HEAL is not giving up on the channel — the periodic 5-minute reconcile still re-derives the whole set. `lastGoodWorkspaceIds` is now assigned unconditionally: a successful enumeration of nothing is an ANSWER (a FAILED one returns before that line). Also in this round: `enumerationRetryDelay`'s `ladder` parameter was dead (no caller, no test ever passed one) and is dropped rather than left reading as configurable.
- Status: open (kept one cycle — landed; delete on the next prune)

### F-090: The blob was rewritten on EVERY probe when either `exp` was unreadable
- Location: `dopl-desktop-app/main/auth-state.js` (`rebuildBlobFromCookieSession`)
- Found during: Q5 adversarial review (2026-07-31) — item S9
- Severity: smell (F-072 class: a read must never become a write storm)
- Description: the "don't rewrite a fresher blob" guard was one `&&` chain — `stored && storedExp != null && cookieExp != null && storedExp >= cookieExp`. When either `exp` was unreadable (`jwtExp` returns null for anything that is not a Supabase JWT), the whole condition was false and the function fell through to `persist()`, so every probe rewrote the encrypted store.
- Proposed resolution: fix-now — done. An explicit `if (stored && (storedExp == null || cookieExp == null)) return false;` refuses to overwrite a blob we cannot compare against. Scoped to `stored` on purpose: with NO blob there is nothing to compare and the rebuild is the whole point of the Q4 fix, so that path is unchanged (pinned by a test in both directions).
- Status: open (kept one cycle — landed; delete on the next prune)
...
- Residual 2 — **PER-SERVER `timeout` BLAST RADIUS.** The desktop writes `timeout: 280_000` on every Dopl MCP entry it owns: `mcp-spawn.json` (`mcp-config.spawnConfigBody`), the SDK path (`sdk-loader.js`), and now the CLI's user-scope config (`mcp-cli-entry.patchCliEntryTimeout` — `claude mcp add` has no `--timeout` flag, and `ensureMcpConfig` does remove+add on every fresh mint, so a hand-patched value was destroyed each cycle). That value applies to EVERY call to the `dopl` server, not just `await`: a genuinely hung short op now hangs a session for up to 280s instead of 60s before it aborts. Accepted for now — the hold is the only op that can legitimately take minutes, and a 60s cap on it breaks the wake outright — but it is the reason the transport fix is the real one and these entries are belt-and-braces. If the transport fix is confirmed in production, consider dropping the per-server timeouts entirely rather than keeping two mechanisms.
- Residual 3 — `dopl-desktop-app/main/sdk-loader.js` still carries a comment saying `/api/mcp` "answers with enableJsonResponse". Left untouched to avoid a concurrent-edit collision during this round; correct it on the next pass through that file.
- Status: open (residuals 1-3)
...
- Resolution (b): the rule is wired into `eslint.config.mjs` at the same severity and options as the desktop config, with the pre-existing offenders listed explicitly rather than hidden behind a blanket ignore. **New code is capped; the backlog stays visible.** DO NOT ADD TO THAT LIST — split the file instead. As of 2026-07-31 the exempted files are, by size: `src/shared/supabase/types.ts` (2290, GENERATED by the Supabase CLI — this one is not a refactor candidate), `src/features/channels/lib/group-thread.test.ts` (837), `packages/mcp-server/src/server.ts` (781), `src/features/skills/components/skill-view.tsx` (759), `packages/dopl-client/src/client.ts` (695), `src/features/knowledge/server/seed-fixtures-data.ts` (670), `src/shared/layout/settings-modal/sections/plans-billing.tsx` (566), `src/features/channels/lib/group-thread.ts` (561), `src/features/billing/server/webhook-handler.test.ts` (542), `src/features/ontology/server/repository.ts` (538), `src/features/workspaces/server/invitations.ts` (534), `src/features/channels/components/channels-view.tsx` (534), `src/features/teams/server/repository.ts` (509), `src/features/billing/components/upgrade-modal.tsx` (506). `src/shared/auth/mcp-oauth.ts` (exactly 500) is deliberately NOT exempted, so the next line added to it fails the lint.
- Proposed resolution: refactor the list down one file at a time, outside a hardening round. `types.ts` should get a permanent exemption comment (generated); the rest are ordinary splits.
- Status: open (rule landed; 14-file backlog outstanding)
...
**RESIDUAL 1 — `withUserAuth` still does a network `getUser()` per API request, and it is now the largest remaining consumer.** `src/shared/auth/with-auth.ts`'s session branch calls `supabase.auth.getUser()`. It is out of scope here (A2/B2 own that file) and it is NOT the same call as the middleware's — a route handler wants a verified identity it can hand to the service layer. But the same `getClaims()` swap applies to it (`claims.sub` is the `userId` those wrappers inject), and after this change it accounts for essentially all remaining GoTrue traffic. Do it in a round that owns that file, with the same two traps in mind (the throw, and the refresh).
...
- Status: **CLOSED 2026-07-31.** The read-op bullets landed in `602d8cb`; the `channel-ops-write.ts` bullet, plus three sites this list never named, landed in the write sweep — see **F-099**.
...
**The Q1 sweep finished on the WRITE ops — and turned up a sixth value class the enumeration had never named.** F-095 listed five sites and fixed the read half; the completeness review then found the same defect alive in the write ops, which had never been enumerated at all. Closed 2026-07-31 (uncommitted). What actually shipped:

- **`opCloseThread` — the headline.** `**${thread.title}**` was raw, and **closing is permitted to the thread's TARGET**, so the ordinary shape is a peer opening a thread, titling it, addressing it to me, and my agent's own close confirmation printing the peer's 200-char, newline-tolerant title as our narration. Now a code span **and** framed with `UNTRUSTED_THREAD_HEADER`.
- **`ch.name` at 14 sites** across `channel-ops-write.ts` / `channel-ops-threads.ts`. `resolveChannelOr` resolves PUBLIC channels the caller was never invited to, so the reach is lower than `opList`'s but non-zero.
- **`profiles.display_name` at ~10 sites**, via `member.label` / `toLabel` — including one **outside every ops file**, in `resolveMemberOr`'s own pending/deactivated error, which is why a per-file sweep missed it. Neutralized AT THE SOURCE in `memberLabel` so a call site added later cannot reintroduce it.
- **`metadata.taskId` (NEW — Q1-E, not in F-095, and it is a READ-op miss).** A non-UUID `taskId` is stored **verbatim**: `resolvePostMetadata` runs its lookup + participation gate only inside `if (isUuid(callerTaskId))` (`service-writes-metadata.ts:236-245`) and the route's `metadata` is a bare `z.record(z.string(), z.unknown())` — no length, charset or newline rule on any value. It was spliced raw in two places, both outside the body's framing and its 2-space indent: the 8-char tag on the message line's **head** (`x\n## OWN` is enough), and the **full-length** id in the legend, inside a hand-built code span that one backtick opens. F-095 quoted the fact that produces this while missing the consequence.

Also: charset rules for `channels.name` / `channels.topic` in `src/features/channels/schema.ts` (they were length-bounded and nothing else), mirroring `DISPLAY_NAME_RE`; `channel-ops-write.ts` split at the §2 cap (see ENGINEERING §18). Tests +18 MCP (`channel-narration-write.test.ts` NEW, 15; +3 in `channel-narration.test.ts`), +4 root. **Every new assertion was verified to FAIL on revert** — neutralizer disabled ⇒ 27/27 fail; headers stripped ⇒ 3/3 framing tests fail; schema reverted ⇒ 2/2 fail.
...
- ~~**The other MCP tools were NOT swept.**~~ **CLOSED by F-101 (same night).** The guess in this bullet — "lower reach, the caller's OWN workspace objects" — was **wrong on two counts**, which is the reason the round was worth doing rather than deferring. `dopl_chats` reads are cross-user BY DESIGN (`visibility: "public"` shares a chat workspace-wide and `op="list"` returns other members' alongside your own). And the widest-reach string in the product turned out not to be in a tool file at all: `workspaces.name`, spliced into the MCP `instructions` block and the `_dopl_status` footer of every response by `server.ts`. See **F-101**.
...
- Status: **code CLOSED** (uncommitted). Migration open (written, not applied). Cross-tool sweep open. **Ships with the SERVER** — `npm run build:packages` done, needs a server push.
...
- **F1 — the MCP lane keyed the trigger rule on `is_direct`; `classify` keys it on MEMBER COUNT.** `targeting.js:152` fires the implicit trigger on `memberCount === 2` + membership and never reads `is_direct`, so a two-member channel opened as a CHANNEL (what `op="open"` with a name gives you) was told its unaddressed messages reach nobody. The rule was restated in three files (`channel-ops-write.ts`, `channel-ops-read.ts`, `channel.ts`) and wrong in all three; the web (`GROUP_CHANNEL_MIN_MEMBERS`) and ENGINEERING §N-PARTY had it right. **Resolution:** ONE module, `packages/mcp-server/src/tools/channel-addressing.ts`, that all three read, with each fact cited to the file that owns it. What makes the caller's OWN post safe to call unheard is the LOOP BRAKE (`:146`) plus the fact that every MCP post is agent-authored (`source: auth.agentTokenId ? "agent" : "user"`), NOT the channel's size — so the note says that instead of generalizing.
- **H3 — "nobody was woken by it" was rendered directly above "the other side reads this as a continuation".** The three pre-classify routes (`listener-messages.js:36-38`) read no `to_user_id` at all: a first-class (uuid) thread tag feeds the counterparty's live session or reopens their settled one. The note's remedy (re-post with `to=`) manufactured a duplicate request → a second consent prompt and a second run, i.e. the 1.7.14 incident shape. **Resolution:** a threaded post gets its own note that says WAIT, not re-post; a LEGACY (`task-<channel>-<seq>`) tag routes to no session (`firstClassTaskId` requires a uuid) and keeps the plain note.
- **H2a — the `await` "not for you" notice told an agent to ignore its own answer.** The canonical reply here is UNADDRESSED (`channel-post.js#postResult` posts with no `toUserId`; `prompt-framing.js#deliveryCall` teaches the call with no `to`), and `resolveDirectPeer` stamps `to_user_id` only for DMs — so in exactly the N>=3 case the notice exists for, a requester awaiting its own answer was told it was somebody else's traffic. **Resolution:** the CONDITION is unchanged (it is all the op can decide without a round-trip on the wake path); the wording no longer equates "nothing names you" with "none of this is yours", and it points at the thread tags. The guardrail it replaces — do not adopt another member's request — is kept verbatim in spirit.
- **H2b — the outbound approval card stopped naming the DM recipient.** In a DM the agent is correctly told it needs no `to`, so `addressed` is falsy and `to` is main's bound-counterparty fill; the wave read that as "no recipient named" — while the SERVER addresses the post to the peer. At N=2, which is all live traffic today, the card understated what a human was approving. **Resolution:** the session now carries `direct` (the server's own `isDirect`, plumbed through `trigger.js` / `session-dispatch.js` / `session-park.js`×3 / the durable record) and the post surface carries `directChannel`; both renderer surfaces name the recipient on that flag OR on `addressed`. Strict `=== true` at every hop, and the field is stamped only when true, so a launch shape that lacks it keeps the channel-level wording and the non-direct payload stays byte-identical.

Tests: `channel-addressing-rule.test.ts` NEW (22 assertions incl. a disk pin of the web's `GROUP_CHANNEL_MIN_MEMBERS`), 4 rewritten in `channel-addressing.test.ts`, +6 desktop (`session-addressee-truth`, `session-dispatch`, `session-store`, `session-open-thread`). **Mutation-verified:** reverting the MCP copy fails 17; reverting the desktop `direct` plumbing fails 6; reverting the store field fails 1.
...
- Status: **code CLOSED** (uncommitted). The `members` reach is an OPEN PRODUCT CALL. **Ships with the SERVER** (`packages/mcp-server`) and the DESKTOP.
...
- **ONE definition, MOVED not copied.** `neutralizeInline` / `inlineOr` / `INLINE_TEXT_MAX` now live in `packages/mcp-server/src/tools/narration.ts`; `channel-shared.ts` **re-exports** them, so no channel module changed and there is still exactly one implementation. Guarded two ways: function IDENTITY (`expect(channelNeutralize).toBe(neutralizeInline)` — a copy passes a behavioural test and fails this) and a source scan asserting `narration.ts` is the only file in `src/tools/` declaring `function neutralizeInline`.
- **`workspaces.name` / `.description` — the highest reach found, and higher than the channel's.** `z.string().min(1).max(120)` / `.max(2000)`, length only, **no charset rule** (`src/features/workspaces/schema.ts`; the `display_name` regex added for profiles has no counterpart here). Set by whoever OWNS a workspace, and a workspace enters your directory the moment you accept an invitation or a join link — the author need share no other context with you at all. That is not "another member of your workspace", it is "the owner of a workspace you joined". It was spliced RAW into the two most trusted surfaces in the protocol: the MCP **`instructions`** block, and the **`_dopl_status` footer on EVERY successful tool response** — where `active_workspace: "${name}"` sat inside double quotes, so one newline bought a second, invented `_dopl_status` key. Six sites in `server.ts`, all neutralized; the footer gained the immutable `id=` beside the renameable slug; `UNTRUSTED_DIRECTORY_NOTE` frames the table ahead of the names.
- **`dopl_chats` — cross-user by design, which F-099 did not realise.** `chats.ts:404`'s `# ${chat.title}` (the site F-099 named) is a real H1 built from a 200-char title, and `op="list"` rows literally read `shared by <another member's display name>`. Title, owner, project, folder and deliverable labels are values now; overview, learnings, transcript summaries and `verbatim` stay intact as bodies. `UNTRUSTED_ARCHIVE_HEADER` fires **only when a rendered chat is workspace-shared** — `visibility === "private"` proves the chat is the caller's own, and a header on every result trains agents to skip headers. `chats-render.ts` split out at the §2 cap (chats.ts was at 491).
- **`dopl_members` renders the SAME `profiles.display_name` column the channel pass flagged**, from the same `listWorkspaceMembers()` call, with no framing — and `op="get"` built a `## ` heading out of it, `op="teams"` a `### ` out of `teams.name`. Worse, `op="list"` printed a member-typed name with **no user id beside it at all**, which the channel work had already ruled out. Fixed at both ends, plus team names/descriptions and every access-matrix resource name.
- **kb / skill / workflow / cluster / ontology / map / search**: every name, title, label, path, slug-echo and search snippet routed through the neutralizer. `dopl_map` matters most of the group — the server instructions tell the agent to call it FIRST, before its first substantive reply, and a base description with a newline started a line of the agent's opening picture of the workspace.
- **`descSuffix` (`knowledge-ops-read.ts`) was a hand-rolled neutralizer that didn't neutralize.** Its `\s*\n+\s*` → space pass misses **U+0085 (NEL)**, which is not in JavaScript's `\s` class, and it touched no markdown at all. Deferring to the shared helper is the point of having one; bound moves 120 → `INLINE_TEXT_MAX` (160).
...
**REPORT-ONLY — a schema call, deliberately not made here (no prod DDL this round):** only **two** columns in the entire product carry a charset rule — KB folder names / entry titles (`NAME_RE`) and `profiles.display_name`. `workspaces.name`/`.description`, `clusters.name`/`.description`, `workflows.name`/`.description`, workflow step `title`/`description`, `knowledge_bases.name`/`.description`, folder `description`, entry `excerpt`, `skills.name`/`.description`/`.when_to_use`, `teams.name`/`.description`, `chats.title`/`.overview`, and every ontology `name`/`purpose`/`subtitle`/`label` are bounded by LENGTH ALONE, so newlines and `##` are legal in all of them. The renderer is now the guarantee, which is the right primary defence; a `DISPLAY_NAME_RE`-style charset rule on the short LABEL columns (workspace, cluster, workflow, base, skill, team names) would be a cheap second layer. **`workspaces.name` is the one worth doing first**, on reach alone.

Tests: **+27 MCP** (`narration.test.ts` 12, `tool-narration.test.ts` 6, `tool-narration-graph.test.ts` 9, sharing `narration-fixtures.ts`); 179 → 206. Root suite unchanged at 965. Two pre-existing assertions were rewritten, not deleted, where a deliberate format change invalidated them (`server.test.ts` footer quoting ×3, `knowledge-ops.test.ts` `descSuffix` bound ×2). **Mutation-verified: 28 reverts, 28 test failures, 0 vacuous.** The harness itself needed two fixes before it could be trusted — vitest `-t` is a REGEX (filters containing `(` `+` matched nothing and read as "passed"), and passing a filter through a shell lets BACKTICKS in a test name run as command substitution (same silent zero-match). Five "survivors" were harness artifacts; all five died once the filter was escaped and `spawnSync`'d with an argv array.

- Status: **code CLOSED** (uncommitted). Charset-rule question **ANSWERED — see F-102** (14 columns bounded, migration written and NOT applied). **Ships with the SERVER** — `npm run build:packages` done, `dist` idempotent, forgery re-verified against the SHIPPED dist build. Dopl "Dopl Development" KB sync still pending.
...
- **ONE definition, and the two that already existed were MOVED onto it.** `SAFE_LABEL_RE` + `safeLabelMessage` + `safeLabel(subject, max)` + `safeOptionalLabel(subject, max)` live in `src/shared/lib/safe-label.ts`. `features/channels/schema.ts` (`CHANNEL_TEXT_RE`) and `app/api/user/profile/route.ts` (`DISPLAY_NAME_RE`) now import it — same class character for character, same error sentence. Three copies of a charset rule drift, and the copy that drifts is the one that stops rejecting.
- **14 columns bounded**: `workspaces.name`+`.description`, `clusters.name`, `knowledge_bases.name`, `skills.name`+`.folder`, `workflows.name`, `workflow_steps.title`, `teams.name`, `chats.title`+`.project`, `chat_folders.name`, `ontology_clusters.name`, `ontology_objects.name`.
- **THE FINDING THAT CHANGES THE PRIORITY.** F-101 framed this as a cheap second layer. For eight of the twelve tables it is the FIRST layer: `authenticated` holds INSERT/UPDATE on all of them **and** each carries a permissive `public` write policy — `clusters_editor_update`, `knowledge_bases_editor_update`, `skills_editor_update`, `workflows_editor_update`, `workflow_steps_editor_update`, `ontology_clusters_editor_update`, `ontology_objects_editor_update`, `teams_admin_write`. Any workspace **editor** can PATCH those names straight through PostgREST with the anon key and never touch a route. The zod schema there is the layer that produces a readable error; the CHECK is the layer that holds. Only `workspaces`, `chats` and `chat_folders` have zero write policies (service-role only), i.e. the `channels` case.
- **`app/api/clusters/[slug]` PATCH had NO schema at all** — hand-rolled `typeof body.name === "string"`, no length bound, while the sibling POST capped at 120. Create and rename disagreed and rename was the one with no opinion. It parses `ClusterNameSchema` now; `description` keeps its existing CLIP-don't-reject semantics on purpose.
- **User-facing copy.** `validationResponseBody(err)` (`shared/api/parse-json.ts`) promotes the first zod issue's own message into `error.message`; the two workspace body routes use it, so renaming a workspace to something illegal shows "Workspace name cannot contain control, zero-width, or line-separator characters" instead of "Request body failed validation".
...
Tests: **+331** in `src/shared/lib/safe-label.test.ts` (root 965 → 1296). Table-driven: 17 bounded fields x (11 rejected classes + 7 legitimate names). **Mutation-verified in BOTH directions** — reverting the bound to a permissive regex fails 198/199 rejection cases (the survivor is the empty-string case, which a length rule still catches); narrowing the rule to ASCII fails 126/127 acceptance cases. The second direction is the point: a rule that rejects `Müller's Team` or `研究` would be worse than no rule.

- Status: **code CLOSED** (uncommitted). **DDL NOT APPLIED — owner decision.** Jsonb ontology labels and the description textarea are OPEN. Dopl "Dopl Development" KB sync still pending.
