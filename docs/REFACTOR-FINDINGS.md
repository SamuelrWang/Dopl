...
- Status: **code CLOSED** (uncommitted). Charset-rule question OPEN (report-only, needs a schema decision). **Ships with the SERVER** — `npm run build:packages` done, `dist` idempotent, forgery re-verified against the SHIPPED dist build. Dopl "Dopl Development" KB sync still pending.
