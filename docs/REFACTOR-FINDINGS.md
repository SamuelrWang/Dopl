...
- **Open follow-ups (NOT done this batch):**
  ...
  2. **Ontology has no web trash/restore UI** — workflows/skills/chats have one; cluster restore is MCP-only. The delete-confirm modal's "restore from trash" copy currently points at the MCP path only.
