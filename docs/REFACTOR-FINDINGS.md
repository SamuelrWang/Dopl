...
- **F-202 and F-203 assigned 2026-08-18**, from Phase 1 (the read model) — the second SQL expression of the thread activity clock that the new view creates, and the tracked `packages/*/dist` that a plain rebuild proved stale. **F-204 and F-205 assigned 2026-08-18**, from Phase 2 (the three-column shell on real reads) — the `react-hooks/refs` rule that reports one channels core and not its twin, and the fixture-driven Tags inbox whose scroll target names no stored message. **The next free id is F-206.**
