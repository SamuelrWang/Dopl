...
- Proposed resolution: add `DELETE /api/auth/mcp-device-token` (or a `revoke` op) that revokes the caller's tokens by label, `sessionOnly` like the mint, and have `signOut()` fire it best-effort before the local teardown. Until then the local teardown plus this note is the honest state. Deliberately NOT built in the Q5 round (desktop-scoped work).
- Status: open
