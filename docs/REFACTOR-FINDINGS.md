...
- **The shape.** The stale sweep's whole body was a close PROPOSAL, so it went with thread closing (INVARIANTS §5, §13) — the route, its `vercel.json` entry and `repository-tasks.ts › listStaleOpenThreads` are all deleted in this change. **The RPC is still in the database.** Deleting the CALLER is not a migration; deleting the function is, so the code half was taken and this half was filed, exactly as the plan directs
...
- The v2 core was fixed at the time (the refs are assigned in a `useEffect`, the shape `shared/hooks/use-api-mutation.ts › useRefetchGate` already uses for its own `runRef`), and the older file was deliberately left alone as the then-shipping page. **The disagreement is now gone BY DELETION, not by diagnosis** — which resolves the symptom and leaves the cause exactly where it was
...
## F-210 — The composer's @-autocomplete is inert, and its match rule is WIDER than the resolver's (2026-08-18)

- Found during: Phase 6 of `docs/CHANNELS-V2-WIRING-PLAN.md`, aligning the composer's picker with the one parser
- Location: `src/features/channels/components/channels-v2/composer.tsx › mentionQuery` / `› MentionPopover` vs `src/features/channels/lib/mentions.ts › resolveMentions`
- **Two separate problems, same seam:**
  - **The popover has no selection handler.** It renders candidates and highlights the first, and nothing clicks, arrows or Tabs one into the draft — the user reads a name and types it themselves. That is an inert control on a wired surface, which Samuel's interaction-completeness ruling treats as worse than an absent one
  - **It suggests on `memberLabel(m).toLowerCase().includes(query)` — a SUBSTRING match — while the resolver is lowercase EXACT equality against a handle set.** So the picker can offer a person the message will not tag. Worked case: typing `@Tay` suggests "Diana Taylor" (substring of the label) and `@Taylor` resolves to NOBODY (not a handle; the handles are `diana`, `dianataylor` and the email local part)
- **Why the resolver must NOT be widened to match the picker.** Substring resolution would tag `@dan` at everybody whose name contains "dan", and a mention decides whose inbox a message lands in. The asymmetry is correct in principle — a picker suggests, a human confirms — and it is only wrong because there is nothing to confirm WITH
- Proposed resolution, in order: (a) give the popover a real selection (click + ArrowUp/Down + Tab/Enter) that INSERTS a handle the resolver accepts — which makes the rules agree by construction rather than by keeping two rules in step; (b) only then consider narrowing the suggestion filter. ⚠ (b) alone leaves the control inert, and (a) alone is already the whole fix
- Not blocking Phase 6: an @-tag typed by hand resolves correctly today, and the stamp, the inbox and the tint are all driven by the resolver
- Status: open
...
- **`src/shared/version/latest-release.test.ts › "honours an injected clock"`** — fails roughly 1 run in 2..3, INCLUDING in isolation (measured 2026-08-18: 1 fail / 2 isolated runs), so it is not merely parallel-load contention; it races a module-level cache against its injected clock. First flagged by the Phase 1 pass, reproduced at the wave-2 join
...
- Proposed resolution: decide whether historical agent attribution is a product requirement. If yes, resolve `author_agent_id` in `channels-v2/view-model.ts › toMessageRow` off a channel-scoped roster read and restore the two deleted modules from git. If no, retire the route and the service read in one change — and leave the strip list alone either way
...
## F-221 — ✅ RESOLVED ON SIGHT: two copies of the IPC sender guard disagreed, with the more privileged one on the lenient side (2026-08-18)

- Found during: Phase 10 of `docs/CHANNELS-V2-WIRING-PLAN.md`, widening the sender binding to the app-window registry
- The tree keeps TWO copies of "is this sender an app window's top frame?" on purpose — `main/ui-bridge.js` (which REJECTS) and `main/channel-dir-ipc.js` (which returns each op's own refusal value, and whose pure block is sliced by `test/channel-ipc-sender.test.mjs`, so de-duplicating it would break the test that guards it)
...
- Status: RESOLVED (kept as the record of why the two copies are now byte-comparable, and of the standing hazard: **two copies of one predicate drift, and nothing lints that**)
