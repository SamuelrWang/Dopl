...
## F-202 — Two SQL expressions of ONE activity clock (2026-08-18)

- Location: `supabase/migrations/20260807160000_channel_tasks_stale_activity.sql › channel_tasks_stale` (the cron's RPC) and `supabase/migrations/20260818120000_channel_tasks_activity_view.sql › channel_tasks_activity` (the read model's view)
- The duplication is DELIBERATE and is the lesser evil, not an oversight. Both carry the identical lateral over `channel_messages` — newest message tagged `metadata.taskId`, excluding a close proposal, falling back to `channel_tasks.created_at` — because the alternative on the day was rebasing the live cron's RPC onto a brand-new view in the same change that first introduced it. **`docs/INVARIANTS.md` §5 now states the rule that binds them: touch one, touch both**
- Why it is debt anyway: two statements of one definition drift, and the drift is invisible — the sweep would idle a thread the sidebar shows as live, or the reverse, with no error on either side
- The merge is already scheduled by the plan, not by this finding: `docs/CHANNELS-V2-WIRING-PLAN.md` Phase 4 removes the stale sweep's close proposals and (probably) the cron, at which point `channel_tasks_stale` loses its only caller and the proposal-exclusion predicate goes inert in BOTH. **Then: drop the RPC in a migration, and delete the `closeProposed` clause from the view in the same one** — a view still excluding a message kind that no longer exists is a trap for the next reader
- Interim rule if the sweep OUTLIVES Phase 4: rewrite `channel_tasks_stale` to select from `channel_tasks_activity` rather than re-deriving. It needs `anchor_seq` as well, so the view grows one column; that is one derivation with two readers, which is the shape this finding wants
- Status: open (scheduled — Phase 4 of the wiring plan)
...
## F-207 — The composer's @-autocomplete is inert, and its match rule is WIDER than the resolver's (2026-08-18)
...
## F-208 — Mentions are per-channel and unbackfilled, so a tag you have not clicked into is invisible (2026-08-18)
...
## F-209 — `check-doc-refs` is RED at HEAD: INVARIANTS §3 describes a sign-in surface this tree does not have (2026-08-18)

- Found during: Phase 6 of `docs/CHANNELS-V2-WIRING-PLAN.md`, running the gates. ⚠ **Pre-existing and unrelated to that change** — measured by stashing the whole Phase 6 diff and re-running: the same single failure, the same anchor. Re-measure the same way before acting
- The gate: `node scripts/check-doc-refs.mjs` reports ONE dead symbol anchor, at `docs/INVARIANTS.md` line 78, naming a constant `LOOP_COUNTED_AUTH_ROUTES` that `src/shared/auth/auth-routes.ts` does not export. Exit FAIL. ⚠ The anchor is deliberately NOT written in `path › symbol` form anywhere in this entry — the checker resolves that form in every doc, so quoting the broken anchor would add a SECOND dead ref and make this finding part of the failure it reports
- **The disagreement is bigger than the one missing symbol.** §3 states that the sign-in form is ONE page at `/authenticate` (2026-08-16) with `/login` and `/signup` kept as 307 REDIRECTORS, all three named in `AUTH_ENTRY_ROUTES`, and the Q4 loop breaker counting `/login` AND `/authenticate` together via that missing constant. **The tree says something else:** the exported `AUTH_ENTRY_ROUTES` in `src/shared/auth/auth-routes.ts` is exactly `["/login", "/signup"]` — two routes over one `LoginScreen`, no `/authenticate` anywhere — and its own docblock argues the loop breaker stays `/login`-ONLY, "a path nothing redirects to cannot be one end of a cycle", pointing at `./login-bounce.ts`
- **MEASURED CAUSE (2026-08-18): the sign-in rework is UNCOMMITTED WORK IN THE MAIN CHECKOUT, not a fiction and not a landed change.** `git status` in the main tree shows `src/app/login/page.tsx` and `src/app/signup/page.tsx` DELETED and the `features/auth` screen files modified, none of it committed — so every worktree branched off the last commit (this one included) sees the OLD two-route shape while `docs/INVARIANTS.md` §3 already describes the NEW one. **The doc is ahead of the commit, not ahead of the product.** Re-measure with `git status` in the main checkout; do not quote this
- **SO THE FIX IS A SEQUENCING FIX, NOT A DOC REWRITE.** §3 becomes true the moment that work commits, and rewriting it now to match the committed tree would have to be reverted by the same change. ⚠ **What is genuinely wrong is that a doc describing uncommitted code was committed** — it makes a repo-wide gate red for every branch that does not carry the working copy, which is every branch
- Two acceptable resolutions, both belonging to whoever owns that working copy: land the `/authenticate` change (the gate goes green on its own), or back §3's paragraph out until it does
- ⚠ **Do NOT "restore" the missing constant to make the gate pass** — the script says so in its own failure text, and inventing the constant would make the doc's claim true by construction while the middleware still counts one path
- Deliberately not touched in Phase 6: it is an auth-surface question with a security property attached (which paths a loop breaker counts), reached by a channels change only because the gate is repo-wide
