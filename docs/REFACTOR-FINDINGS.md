...
- Both files write a ref in the render pass, the same way, for the same reason (a realtime callback needs the LATEST refetch closure without resubscribing): `src/features/channels/components/channels-view-core.tsx › ChannelsViewCore` (`refetchRef.current = () => {…}`, `membersRefetchRef.current = …`) and, until this change, `src/features/channels/components/channels-v2/channels-v2-core.tsx › ChannelsV2Core`
- **Measured 2026-08-18:** `npx eslint src/features/channels/components/channels-view-core.tsx` → exit 0, zero diagnostics. The identical lines in the v2 core → **two `react-hooks/refs` errors**. Re-measure with those two commands; do not quote the outcome
- The v2 core was fixed here (the refs are assigned in a `useEffect`, the shape `shared/hooks/use-api-mutation.ts › useRefetchGate` already uses for its own `runRef`). **`channels-view-core.tsx` was deliberately left alone** — it is the SHIPPING page, this change is read-only and scoped to v2, and editing a live surface to satisfy a rule that is not reporting it is a change with no failing gate behind it
...
- Status: open (v2 half fixed; the shipping page, the second-rule instance and the cause are all open)
...
- **The design (`apps/desktop-ui/src/pages/channels-v2/MAPPING.md` § Agents tab & the agent view) draws THREE lanes in one feed. One of them wired; the other two are named here rather than faked, because a fabricated line in this panel is a claim about what a real machine did:**
