...
### F-099: `channels.name` / `.topic` charset CHECKs — migration written, NOT applied
...
- Status: open (migration unapplied + the jsonb labels)
...
- **NOT VERIFIED against a live database.** Samuel's gate. **This is the top deploy blocker in this file: until it is applied, delete looks broken to any user with two windows open.**
- Status: open (migrations written, unapplied)
...
- Status: **resolved in code; the deploy step is Samuel's.** Migration written, NOT applied.
