...
## F-207 — `scripts/check-doc-refs.mjs` has been failing on an INVARIANTS symbol anchor that names nothing (2026-08-18)

- Found during: Phase 8 of `docs/CHANNELS-V2-WIRING-PLAN.md`, running the doc-anchor gate on a change that touched neither auth nor that section
- **The break.** `docs/INVARIANTS.md` §3 carries a symbol anchor into `src/shared/auth/auth-routes.ts` for a constant named `LOOP_COUNTED` + `_AUTH_ROUTES` (spelled in two halves here on purpose — a live copy of the anchor would fail this very check). `grep -rn "LOOP_COUNTED_AUTH_ROUTES" src` returns **nothing** — the constant exists only in the sentence describing it. The checker reports it as a symbol anchor naming something that is not there, which is precisely the failure mode that rule was added to catch
- **It is PRE-EXISTING, not this phase's.** `check-doc-refs` exits non-zero on a clean tree at this branch point; Phase 8 touched `INVARIANTS.md` §6/§7/§11 and no auth file. Recorded rather than fixed, because fixing it means deciding what the loop breaker ACTUALLY counts today, which is an auth read this phase had no business doing
- ⚠ **The tempting wrong fix, named: do not add the constant to `auth-routes.ts` to make the gate pass.** The doc's own instructions say so. Either the breaker really does count both paths under a different name — in which case repoint the anchor — or it does not, in which case the §3 bullet is describing a guarantee the code does not provide, and that is a security-shaped claim, not a docs typo
- ⚠ **A red gate that predates a change trains the next agent to skip the gate.** This one has been failing long enough that a phase running it found it as background noise
- Status: open (needs an auth read: what does the Q4 loop breaker count)
