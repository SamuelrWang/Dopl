...
| **3 — Agent surface** | `HIDDEN_TOOLS` guard in `packages/mcp-server/src/server.ts` unregisters `dopl_workflow` / `dopl_workflow_admin` / `dopl_cluster` / `dopl_cluster_admin` (D1/D2) → **14 tools in `tools/list`**; `buildInstructions` rewritten; `dopl_map` 5→3 domains, `dopl_search` 4→3 groups; the skill template's (a module deleted 2026-09-24, F-726) "Following a workflow" section gone; `members.ts` workflow-row filter; `tool-profiles.js` `DOPL_SAFE_TOOLS` 11→9 with the two retired admins moved to `RETIRED_DOPL_TOOLS` (**still hard-denied** — a name off a deny list becomes *unclassified*, which resolves to `gate`). `dist/` rebuilt. New suite: `packages/mcp-server/src/retirement.test.ts`. |
...
| Overview "Workflows" stat card | `src/features/workspaces/components/overview-stats-core.tsx` (⛔ the whole module went off disk 2026-08-19 when the Overview page was rebuilt as a static dashboard — this row's card had already gone with the 2026-08-07 hide), rendered from `apps/desktop-ui/src/pages/overview/index.tsx` | Remove card + test |
| Product tour steps 2 (Canvas) + 3 (Workflows) — step 2 **navigates** to /canvas | ⛔ MOOT — the whole tour (`src/features/tour/**`) went off disk 2026-09-17 with the guidance layer (Samuel's ruling R-49) | Nothing to do |
...
| Generated skill template (the SKILL.md users install) | The skill template module (a module deleted 2026-09-24, F-726), its "## Following a workflow" section | Rewrite in lockstep with instructions (its docblock required it) |
...
| Bootstrap agent prompt teaches WORKFLOW as one of three primitives | ⛔ MOOT — `buildBootstrapPrompt` was deleted 2026-09-17 with the welcome popup that was its only reader (R-49); `bootstrap-prompt.ts` keeps `buildConnectPrompt` alone |
| Welcome popup "…and run workflows", workspace-name step, create-workspace dialog copy | ⛔ the popup went off disk 2026-09-17 (R-49); the other two stand — `workspace-name-step.tsx:32`, `create-workspace-dialog-core.tsx:88` |
...
3. **Agent surface** — `HIDDEN_TOOLS` guard covering `dopl_workflow`, `dopl_workflow_admin`, `dopl_cluster`, `dopl_cluster_admin`; `buildInstructions` rewrite; `dopl_map` + `dopl_search` sections/counts (5→3 domains, 4→3 groups); skill template (a module deleted 2026-09-24, F-726) rewrite; incidental descriptions; `members.ts:315` workflow-row filter (D7); `tool-profiles.js`. **Rebuild `packages/mcp-server/dist`.** Fix `parity.test.ts:142-270` + 4 desktop session/tool-profile tests.
