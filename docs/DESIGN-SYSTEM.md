...
Samuel's ruling (docs/specs/agent-colors.md; `channels-v2/message-box-agent.tsx`):
...
- **The bar** — full width, the SAME colour, holding the attribution pill on the
  **LEFT** and nothing on the right. Its geometry is one constant,
  `message-box-agent.tsx › AGENT_BAR`, which `channels-v2/agent-stream-sent-box.tsx`
...
- **The dot** — `channels-v2/agent-color-dot.tsx`, `size-2`, `aria-hidden`, drawn
...
- **The filter** — `channels-v2/transcript-filter.tsx`, immediately LEFT of the
  ...
  predicate: `channels-v2/agent-box-rule.ts › agentBoxOf`.
