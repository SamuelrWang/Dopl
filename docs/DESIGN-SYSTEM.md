...
channel should be that font styling"*; `channels-v2/message-pane-header.tsx`,
`channels-v2/channel-single-column.tsx`, pinned in
`channels-v2/channel-single-column.test.tsx`). ⚠ **NEVER RE-TYPED** — a hand-written
...
`channels/components/channels-v2/agent-window-shell.tsx`) with the agent's view a
...
Pinned in `channels-v2/agent-window-frame.test.ts` and
...
| `InfoResizeHandle` (`channels/components/channels-v2/info-resize-handle.tsx`) | **THE DRAG HANDLE ON A COLUMN DIVIDER (Samuel, 2026-09-13: *"a vertical black line … a little thick, rounded at the edges, and centered. If I hover over it, it should show the left and right arrows"*)** — a `w-1 h-10 rounded-full bg-text-primary` pill centred on the rule, flanked on HOVER-or-FOCUS-or-DRAG by 12px `ChevronLeft`/`ChevronRight` in `text-text-secondary`, over an INVISIBLE `w-3` `cursor-col-resize` strip. ⚠ **THE WRAPPER IS `w-0`**, so the handle is a zero-width flex sibling and the row's box math does not move — and it sits OUTSIDE `.channel-info-slide`, whose `overflow: hidden` would clip the left half of a centred pill. ⚠ **`z-[2]` IS BETWEEN TWO EXISTING LAYERS**: above the info column (`z-index: 1`) and below the agent overlay (`z-20`), which covers the column this would resize. ⚠ **HOVER IS REACT STATE, NOT `group-hover:`** — the arrows are a fact in the DOM, which is what lets `info-resize-handle.test.tsx` assert the behaviour instead of reading a class back as a string. ⚠ **THE PILL'S CENTRE IS THE LINE'S CENTRE, WHICH IS HALF A BORDER RIGHT OF THE COLUMN'S EDGE (Samuel, 2026-09-13: *"right now it's sitting to the left. I want it to be perfectly on the vertical line"*).** The wrapper is `w-0`, so `left-1/2` resolved to 0 — the divider's FIRST pixel, not its middle — and half the 4px pill sat on the transcript. The strip is now `left-[calc(var(--channel-divider-w,1px)/2)] -translate-x-1/2`. ⚠ **THE `1px` IS A FALLBACK, NOT THE WIDTH**: the divider is `channels-v2/info-panel.tsx`'s `border-l border-border-default`, a kit hairline on the workspace channels page and **2px inside /home's record pane**, where `apps/desktop-ui/src/pages/home/home.module.css › .frame` widens that shape AND declares `--channel-divider-w: 2px` in the rule block beside it so the pair cannot drift (pinned: `pages/home/channel-divider.test.ts`). **A literal here is the bug.** The width itself, its two limits and the per-device memory are `channels-v2/use-info-resize.ts`; this row is the FACE. ⚠ **ONE consumer and it is a SHARED surface** — `channels-v2/channel-surface.tsx` renders it, so the workspace channels page and the desktop /home pane get it from one mount point (there is no third host of that surface). |
| `AgentMessageRow` (`channels/components/channels-v2/message-row-agent.tsx`) | **WHAT AN AGENT'S POST LOOKS LIKE (Samuel, 2026-09-13: *"it's actually really hard to understand which agent is posting what … there's a specialized kind of box UI … I want to change that to a more modern UI"*)** — a FLAT row: a 24px avatar slot (`Avatar size="xs"`, the operator's photo with a `Bot` badge, or a `Bot` MARK in the same circle when there is no photo — an initial letter on an agent row is a picture of a person posting), then a one-line header `name · time` (`text-body font-medium text-text-primary` · an `aria-hidden` middot · `text-caption text-text-muted`), then the body in the transcript's own `MESSAGE_BLOCK`/`MESSAGE_TEXT`, `gap-1`. ⚠ **IT SUPERSEDES `AttributionPill` FOR AGENT ROWS ONLY** — the 2026-08-22 "one header idiom for the whole transcript" ruling stands for HUMANS, who keep the `.bento` capsule byte for byte, and for the thread / milestone / escalation CARDS, which are cards on purpose. ⚠ **IT MUST NOT CARRY `data-attribution-pill`**: /home repaints that hook in a RAISED PILL face (`pages/home/home.module.css`), which would re-box the thing this removed. The sender hook is `data-agent-sender`, on the pressable name AND its inert span. ⚠ **A RUN COLLAPSES THE HEADER, AND EXPIRES AFTER 5 MIN** (`view-model-rows.ts › AGENT_RUN_WINDOW_MS`); the avatar slot is held EMPTY on a continuation so the body keeps its edge. ⚠ **`border-l-2` IS ALWAYS RESERVED** and only its colour moves — the agent's own tone (`bits.tsx › agentAccent`, fill and ink neutralised after it) while the agent is live, `border-transparent` once it has ended or when this machine does not know it — or the transcript reflows 2px the moment an agent stops. ⚠ **THE SIDE IS STILL `author_user_id`** (INVARIANTS §5), expressed as `flex-row-reverse` rather than `items-end`; the name wording is still `attribution-pill.tsx › attributionName`, by import. Pinned in `channels-v2/message-row-agent.test.tsx`. |
