...
`agent-identities/components/identity-section.tsx › IDENTITY_NAME_TEXT`
(`text-title font-medium text-text-primary`) — the identity card's name, the /home
...
⚠ **NOT `IDENTITY_NAME_TEXT_LG`** (18px, `shared/ui/section-heading.ts`), whose one
...
| `[data-frame-skin]` (**unlayered**) | 🔒 **THE ACCOUNT PALETTE SKIN — the app's since 2026-09-17 (Samuel's ruling R-38: the workspace pages adopt /home's frame model and palette, *"the two surfaces must match"*).** Six rules that carry the palette INTO the shared channel surface: both divider utilities recoloured to `--home-panel-line`, the structural `border-l` / `border-b` widened to 2px with `--channel-divider-w` declared beside them, `[data-attribution-pill]` on the raised-light face, `[data-composer-panel]` on `--home-panel`. ⚠ **IT IS AN ATTRIBUTE, AND THE HOSTS ARE A LIST OF THREE**: `pages/home/index.tsx` (the record pane), `› home-skeleton.tsx` (its ghost) and `channels/components/channels-core.tsx` (the workspace channels page). The guest web lane does NOT wear it and keeps the neutral kit hairlines. ⚠ **OUTSIDE `@layer components` ON PURPOSE** — it overrides Tailwind UTILITIES on components the host does not own, and a layer beats specificity outright — which also means **`check-css-token-drift.ts` cannot see it**: the class half compares the layer only, so this block is mirrored BY HAND in `kit.css` like the auth-adjacent rules (F-074). ⚠ **THREE RULES SELECT ON TAILWIND UTILITY CLASS NAMES** in `features/channels/components/`; if those change the skin degrades to the neutral hairline — the old look, not a broken one. ⚠ **IT REPLACED THREE COPIES OF ITSELF**: `pages/home/home.module.css › .frame` (fenced to /home), the `.lp-demo-record` port in `src/features/marketing/marketing.css`, and — for the sixth rule, `[data-section-panel]` — the workspace-page default that carried a hairline. **Do not re-fence it and do not port it again.** Pinned: `pages/home/channel-divider.test.ts` (both kit copies, the 2px pair, and the three hosts). |
...
- **Section**: 🔒 ⚠ **THERE IS ONE SECTION PATTERN ON THE DESKTOP AND IT IS FLAT
  ...
  - **Section box** (`SectionBox`, `src/shared/ui/section-box.tsx`): 14px-radius
    ...
    from `src/app/playground/page.tsx`); `SECTION_BOX_INSET` survives on the
    FROZEN settings surfaces (INVARIANTS §15) and on
    `channels/components/composer-launch-panel.tsx`, where the pressed-in stack
    is its own standing ruling (2026-08-26: *"FILL ONLY. The concave shadow stack
    is deliberate and stays"*). **A new desktop section that mounts either is the
