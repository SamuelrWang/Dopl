...
| `.graph-substrate` | Dotted world surface (24px pitch) behind a board or graph view. THE dot recipe. ⚠ **THE ONTOLOGY BOARD LEFT IT ON 2026-09-10** (Samuel: the grid under the lanes read as *"a gray canvas, on top of a white panel … double panel"*) — `kanban-board.tsx` and `ontology-skeleton.tsx` compose nothing, and the live board is the page panel itself. The one consumer left is the playground's ontology pane (`features/playground/components/panes/ontology-pane.tsx`, measured 2026-09-10: `grep -rn 'graph-substrate' src apps`). |
| `.kanban-substrate` | Modifier on `.graph-substrate`: halves the pitch to 12px so a lane board's geometry lands on the grid (288px lanes = 24 tiles, 12px gutter = 1, 24px board padding = 2), and `background-attachment: local` so the dots cover the whole scrollable area and travel with the lanes. ⚠ **NOT ON THE LIVE ONTOLOGY BOARD SINCE 2026-09-10** — see the row above; the playground pane is the only caller. Do not put it back on `kanban-board.tsx`: the white panel is a ruling, and `pages/home/ontology-panels.test.tsx` asserts neither class is in the tree. |
...
| `.graph-node` / `.graph-node-selected` / `.graph-node-target` | Graph card resting / selected-ring / connect-drop-target surfaces. ⚠ **DEAD — ZERO consumers, measured 2026-08-30** (`grep -rn 'graph-node' src apps packages \| grep -v '\.css:'` → no hits). This row said *"only the Workflows cards are left"*; the Workflows page was **deleted 2026-08-11**, and the last caller went with it. Kept until a deliberate deletion pass, and named here so the next reader does not compose a group that paints nothing. ⚠ Do NOT take the substrates down with them: `.graph-substrate` / `.kanban-substrate` are still live, though their caller is now the playground pane alone (the ontology board dropped them 2026-09-10). |
