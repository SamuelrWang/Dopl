...
| `.channel-info-slide` | The channels-v2 info column's slide shell (2026-08-24) — 0 ↔ 340px on a 200ms width transition, `overflow: hidden`. WIDTH, not transform: the column is a flex sibling of the message pane, and a transform would slide it over the transcript instead of handing the space back. The panel inside keeps its own `w-[340px] shrink-0` so it never squashes mid-slide. ⚠ Coupled to `channel-surface.tsx › INFO_SLIDE_MS`, which keeps the panel mounted one transition past close, and to the `prefers-reduced-motion` block that turns the transition off. |
...
| `.seg-track`     | Flat pill track for `SegmentedControl variant="track"`. Not `.concave-track` (that one is pressed-in and hosts tab pills); this is a page-header selector and its FILL comes from the consumer's `bg-*` token utility — the value in the kit is only a fallback. Active option stays `.raised-tab`. ⚠ Its 3px pad is arithmetic, not taste: at `size="lg"` it turns 30px options into the 36px control height the header's other controls share, and it does not scale with them — under 3px a track stops reading as a track. |
