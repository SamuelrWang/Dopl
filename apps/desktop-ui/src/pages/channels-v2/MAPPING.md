...
**`requested` is a MOCK-SIDE status.** The shipping model has no such task
status: there it is an open thread whose consent rows are still `pending`, and
an addressee who never answers is a thread that never starts. The mock needs one
word for that state to give it a glyph, a chip and a filter home.
...
- **ACTIVE and REQUESTED threads are rows in this tree**, nested one indent step
  ...
  - **Glyph legend**: `Bot` = active, an agent is party to it and working;
    `Clock` = requested, addressed and waiting on consent. `Clock` beat
    `CircleDashed` on two counts — at 13px a dashed ring's gaps fall under a
    pixel and it mushes into a fuzzy dot beside the crisp `Bot`, and it is the
    SAME glyph the card's pending pill and the thread view's strip use, so one
    shape means "waiting on approval" in all three columns.
...
- **The "New agent thread" panel is the ONLY way to raise an agent request.**
  The plain composer is human chat, full stop — the intent pill (chat/request)
  dies with this.
- **The DM implicit trigger is REMOVED.** No auto-addressing in a 2-person DM;
  a DM agent request goes through the same explicit panel. (Port note: this
  retires the fail-quiet DM auto-address behavior, INVARIANTS §5.)
...
Composer wiring (`channel_message_insert` RPC path), addressing ENFORCEMENT
(the new-thread panel draws the shape; nothing validates it, and the plain
text area can still be sent unaddressed — INVARIANTS §5), the request fan-out
into one thread per addressee, the consent DECISION surfaces themselves (the
mock renders each decision's outcome on the card, never the Allow/Deny control
or the trust settings behind it), close = propose-then-confirm flow,
optimistic writes / `client_msg_id` idempotency.
