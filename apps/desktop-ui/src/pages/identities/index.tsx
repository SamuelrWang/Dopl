...
/**
 * `/:workspaceSegment/agents` — persistent agent IDENTITIES: three scope panels
 * (Private / Team / Public) over a card grid, and the editor behind them.
 *
 * ⚠ ONLY A SEAM. The whole surface is `@/features/agent-identities/`, already
 ...
 *
 * ⚠ NO DETAIL ROUTE. An identity is edited in a modal, not at a URL, so there is
 * no `agents/:identityId` row — and the deep-link hand copy in
 * `dopl-desktop-app/main/deep-link-target.js › WORKSPACE_PAGES` therefore wants
 * `agents: false`, not `true`.
 *
 * ⚠ THE LOADING STATE IS THIS PAGE'S OWN SHAPE (`./identities-skeleton.tsx`) — a
 * 52px header over three scope panels on `IdentityGrid`'s card grid. It was the
 ...
 * identity read — and the second one used to paint the shared page ghost, so
